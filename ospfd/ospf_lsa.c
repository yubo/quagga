...
static void ospf_stub_router_check(struct ospf_area *area)
{
	 ...
	 */
	if (CHECK_FLAG
	    (area->stub_router_state, OSPF_AREA_WAS_START_STUB_ROUTED))
		return;
...
static void
ospf_summary_lsa_body_set(struct stream *s, struct prefix *p, u_int32_t metric)
{
	struct in_addr mask;
		...
		if (IS_DEBUG_OSPF_NSSA)
			zlog_debug
			    ("ospf_translated_nssa_refresh(): no Type-7 found for "
			     "Type-5 LSA Id %s", inet_ntoa(type5->data->id));
		return NULL;
	}
...
void ospf_nssa_lsa_flush(struct ospf *ospf, struct prefix_ipv4 *p)
{
	...
	for (ALL_LIST_ELEMENTS(ospf->areas, node, nnode, area)) {
		if (area->external_routing == OSPF_AREA_NSSA) {
			if (!
			    (lsa =
			     ospf_lsa_lookup(area, OSPF_AS_NSSA_LSA, p->prefix,
					     ospf->router_id))) {
...
void ospf_external_lsa_refresh_type(struct ospf *ospf, u_char type, int force)
{
	...
	if (type != DEFAULT_ROUTE)
		if (EXTERNAL_INFO(type))
			...
			for (rn = route_top(EXTERNAL_INFO(type)); rn;
			     rn = route_next(rn))
				if ((ei = rn->info))
...
static struct ospf_lsa *ospf_router_lsa_install(struct ospf *ospf,
						struct ospf_lsa *new,
						int rt_recalc)
{
...
static struct ospf_lsa *ospf_summary_lsa_install(struct ospf *ospf,
						 struct ospf_lsa *new,
						 int rt_recalc)
{
		...
		for (rn = route_top(nbr->oi->nbrs); rn; rn = route_next(rn))
			if ((onbr = rn->info) != NULL)
				if (onbr != nbr->oi->nbr_self
					...
					ospf_ls_upd_send_lsa(onbr, self,
...
void ospf_refresher_register_lsa(struct ospf *ospf, struct ospf_lsa *lsa)
{
	...
	if (lsa->refresh_list < 0) {
		...
		if (LS_AGE(lsa) == 0 &&
			...
			delay =
			    OSPF_LS_REFRESH_TIME - LS_AGE(lsa) -
		...
		current_index =
		    ospf->lsa_refresh_queue.index + (quagga_time(NULL)
						     -
		...
		if (IS_DEBUG_OSPF(lsa, LSA_REFRESH))
			zlog_debug
			    ("LSA[Refresh]: lsa %s with age %d added to index %d",
