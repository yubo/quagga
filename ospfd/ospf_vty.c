...
DEFUN(router_ospf,
      router_ospf_cmd,
      "router ospf", "Enable a routing process\n" "Start OSPF configuration\n")
{
	vty->node = OSPF_NODE;
	vty->index = ospf_get();
...
ALIAS(no_ospf_router_id,
      ...
      "no router-id", NO_STR "router-id for the OSPF process\n")
...
static void
ospf_vl_config_data_init(struct ospf_vl_config_data *vl_config, struct vty *vty)
{
...
static struct ospf_vl_data *ospf_find_vl_data(struct ospf *ospf, struct ospf_vl_config_data
					      *vl_config)
{
	...
	if (area->external_routing != OSPF_AREA_DEFAULT) {
		if (vl_config->format == OSPF_AREA_ID_FORMAT_ADDRESS)
			vty_out(vty, "Area %s is %s%s",
				inet_ntoa(area_id),
				area->external_routing ==
				OSPF_AREA_NSSA ? "nssa" : "stub", VTY_NEWLINE);
		else
		     ...
		     struct ospf_vl_config_data *vl_config)
{
	...
	if (vl_config->auth_key) {
		...
		if (ospf_crypt_key_lookup
		    (IF_DEF_PARAMS(ifp)->auth_crypt, vl_config->crypto_key_id)
		    != NULL) {
      ...
      VLINK_HELPSTR_IPADDR)
{
	...
	for (i = 2; i < argc; i++) {
		...
		case 'a':
			if (i > 2
			    || strncmp(argv[i], "authentication-", 15) == 0) {
...
ALIAS(ospf_area_vlink,
    ...
    ALIAS(ospf_area_vlink,
      ...
      "area (A.B.C.D|<0-4294967295>) virtual-link A.B.C.D "
      "(authentication|)", VLINK_HELPSTR_IPADDR VLINK_HELPSTR_AUTHTYPE_SIMPLE)
    ...
    ALIAS(ospf_area_vlink,
      ...
      "(authentication-key|) AUTH_KEY",
      VLINK_HELPSTR_IPADDR
      VLINK_HELPSTR_AUTHTYPE_SIMPLE VLINK_HELPSTR_AUTH_SIMPLE)
      ...
      "Stub's advertised default summary cost\n")
{
	...
	VTY_GET_OSPF_AREA_ID_NO_BB("default-cost", area_id, format, argv[0]);
	VTY_CHECK_INTEGER_RANGE("stub default cost", argv[1], 0,
				OSPF_LS_INFINITY);
...
ALIAS_DEPRECATED(no_ospf_timers_throttle_spf,
    ...
    DEFUN(ospf_neighbor,
      ospf_neighbor_cmd,
      "neighbor A.B.C.D", NEIGHBOR_STR "Neighbor IP address\n")
{
	struct ospf *ospf = vty->index;
	...
	if (argc > 1)
		VTY_GET_INTEGER_RANGE("neighbor priority", priority, argv[1], 0,
				      255);

	if (argc > 2)
		VTY_GET_INTEGER_RANGE("poll interval", interval, argv[2], 1,
				      65535);
      ...
      "Neighbor IP address\n" "Dead Neighbor Polling interval\n" "Seconds\n")
{
	...
	if (argc > 1)
		VTY_GET_INTEGER_RANGE("poll interval", interval, argv[1], 1,
				      65535);

	if (argc > 2)
		VTY_GET_INTEGER_RANGE("neighbor priority", priority, argv[2], 0,
				      255);
...
ALIAS(ospf_neighbor_poll_interval,
    ...
    DEFUN(no_ospf_neighbor,
      ...
      "no neighbor A.B.C.D", NO_STR NEIGHBOR_STR "Neighbor IP address\n")
{
	struct ospf *ospf = vty->index;
	struct in_addr nbr_addr;
      ...
      "Unset refresh timer\n" "Timer value in seconds\n")
{
	...
	if (argc == 1) {
		VTY_GET_INTEGER_RANGE("refresh timer", interval, argv[0], 10,
				      1800);
...
static void show_ip_ospf_area(struct vty *vty, struct ospf_area *area)
{
	...
	if (CHECK_FLAG(area->stub_router_state, OSPF_AREA_IS_STUB_ROUTED)) {
		...
		vty_out(vty,
			...
			VTY_NEWLINE);
		if (CHECK_FLAG
		    (area->stub_router_state, OSPF_AREA_ADMIN_STUB_ROUTED))
			vty_out(vty,
...
DEFUN(show_ip_ospf,
      show_ip_ospf_cmd, "show ip ospf", SHOW_STR IP_STR "OSPF information\n")
{
	struct listnode *node, *nnode;
	struct ospf_area *area;
	struct ospf *ospf;
	...
	ospf = ospf_lookup();
	if (ospf == NULL) {
		vty_out(vty, " OSPF Routing Process not enabled%s",
			VTY_NEWLINE);
		return CMD_SUCCESS;
	}
	...
	vty_out(vty, " OpaqueCapability flag is %s%s%s",
		...
		" (origination blocked)" : "", VTY_NEWLINE);
#endif /* HAVE_OPAQUE_LSA */
			   ...
			   struct interface *ifp)
{
	...
	vty_out(vty, "  ifindex %u, MTU %u bytes, BW %u Kbit %s%s",
		ifp->ifindex, ifp->mtu, ifp->bandwidth,
	...
	for (rn = route_top(IF_OIFS(ifp)); rn; rn = route_next(rn)) {
		...
		vty_out(vty, "  Router ID %s, Network Type %s, Cost: %d%s",
			...
			ospf_network_type_str[oi->type], oi->output_cost,
			VTY_NEWLINE);
	...
	for (ALL_LIST_ELEMENTS_RO(ospf->oiflist, node, oi)) {
		...
		if (oi->type == OSPF_IFTYPE_NBMA) {
			...
			for (ALL_LIST_ELEMENTS_RO(oi->nbr_nbma, nd, nbr_nbma))
				if (nbr_nbma->nbr == NULL
				    || nbr_nbma->nbr->state == NSM_Down)
					show_ip_ospf_nbr_nbma_detail_sub(vty,
									 oi,
									 nbr_nbma);
		}
	}
		...
		if (self == 0 || IS_LSA_SELF(lsa)) {
			...
			case OSPF_AS_NSSA_LSA:
				...
				vty_out(vty, " %s %s/%d [0x%lx]",
					IS_EXTERNAL_METRIC(asel->
							   e[0].tos) ? "E2" :
					"E1", inet_ntoa(p.prefix), p.prefixlen,
...
static int show_as_external_lsa_detail(struct vty *vty, struct ospf_lsa *lsa)
{
	if (lsa != NULL) {
		...
		vty_out(vty, "        External Route Tag: %lu%s%s",
			(u_long) ntohl(al->e[0].route_tag), VTY_NEWLINE,
			VTY_NEWLINE);
	}
...
#if 0
static int show_as_external_lsa_stdvty(struct ospf_lsa *lsa)
{
	struct as_external_lsa *al = (struct as_external_lsa *)lsa->data;

	/* show_ip_ospf_database_header (vty, lsa); */

	zlog_debug("  Network Mask: /%d%s", ip_masklen(al->mask), "\n");
...
int (*show_function[]) (struct vty *, struct ospf_lsa *) = {
	NULL, show_router_lsa_detail, show_network_lsa_detail, show_summary_lsa_detail, show_summary_asbr_lsa_detail, show_as_external_lsa_detail, show_func_dummy, show_as_nssa_lsa_detail,	/* almost same as external */
#ifdef HAVE_OPAQUE_LSA
	    NULL,		/* type-8 */
...
ALIAS(no_ip_ospf_cost,
    ...
    ALIAS(no_ip_ospf_cost,
      no_ospf_cost_cmd,
      "no ospf cost", NO_STR "OSPF interface commands\n" "Interface cost\n")
...
DEFUN(no_ospf_redistribute_source,
      ...
      NO_STR REDIST_STR QUAGGA_REDIST_HELP_STR_OSPFD)
{
	struct ospf *ospf = vty->index;
	int source;
...
DEFUN(no_ospf_default_metric,
      ...
      "no default-metric", NO_STR "Set metric of redistributed routes\n")
{
	struct ospf *ospf = vty->index;
      ...
      "Administratively applied, for an indefinite period\n")
{
	...
	for (ALL_LIST_ELEMENTS_RO(ospf->areas, ln, area)) {
		...
		if (!CHECK_FLAG
		    (area->stub_router_state, OSPF_AREA_IS_STUB_ROUTED))
		...
		if (rn->info) {
			...
			for (ALL_LIST_ELEMENTS_RO
				...
				vty_out(vty, "%s [%d] area: %s",
					(or->path_type ==
					 OSPF_PATH_INTER_AREA ? "IA" : "  "),
					or->cost, inet_ntoa(or->u.std.area_id));
				/* Show flags. */
				vty_out(vty, "%s%s%s",
					(or->u.
					 std.flags & ROUTER_LSA_BORDER ? ", ABR"
					 : ""),
					(or->u.
					 std.flags & ROUTER_LSA_EXTERNAL ?
					 ", ASBR" : ""), VTY_NEWLINE);
	...
	for (ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		...
		if (area->shortcut_configured != OSPF_SHORTCUT_DEFAULT)
			vty_out(vty, " area %s shortcut %s%s", buf,
				...
				[area->shortcut_configured], VTY_NEWLINE);
		...
		for (rn1 = route_top(area->ranges); rn1; rn1 = route_next(rn1))
			if (rn1->info) {
				...
				if (!CHECK_FLAG
				    (range->flags, OSPF_AREA_RANGE_ADVERTISE))
					vty_out(vty, " not-advertise");

				if (CHECK_FLAG
				    (range->flags, OSPF_AREA_RANGE_SUBSTITUTE))
	...
	for (ALL_LIST_ELEMENTS_RO(ospf->vlinks, node, vl_data)) {
		...
		if (vl_data != NULL) {
			...
			if (vl_data->format == OSPF_AREA_ID_FORMAT_ADDRESS)
				...
				sprintf((char *)buf, "%lu",
					(unsigned long int)
					ntohl(vl_data->vl_area_id.s_addr));
			oi = vl_data->vl_oi;
			...
			if (IF_DEF_PARAMS(vl_data->vl_oi->ifp)->auth_simple[0]
				...
				vty_out(vty,
					...
					buf, inet_ntoa(vl_data->vl_peer),
					IF_DEF_PARAMS(vl_data->vl_oi->
						      ifp)->auth_simple,
					VTY_NEWLINE);
			/* md5 keys */
			for (ALL_LIST_ELEMENTS_RO
			     (IF_DEF_PARAMS(vl_data->vl_oi->ifp)->auth_crypt,
			      n2, ck))
				vty_out(vty,
					" area %s virtual-link %s"
					" message-digest-key %d md5 %s%s", buf,
					inet_ntoa(vl_data->vl_peer), ck->key_id,
					ck->auth_key, VTY_NEWLINE);
...
static int config_write_ospf_distribute(struct vty *vty, struct ospf *ospf)
{
	...
	if (ospf) {
		...
		if (ospf->default_originate != DEFAULT_ORIGINATE_NONE) {
			...
			if (ospf->dmetric[DEFAULT_ROUTE].type ==
...
static int config_write_ospf_distance(struct vty *vty, struct ospf *ospf)
{
	...
	if (ospf->distance_intra
	    || ospf->distance_inter || ospf->distance_external) {
	...
	for (rn = route_top(ospf->distance_table); rn; rn = route_next(rn))
		if ((odistance = rn->info) != NULL) {
			vty_out(vty, " distance %d %s/%d %s%s",
				...
				rn->p.prefixlen,
				odistance->
				access_list ? odistance->access_list : "",
				VTY_NEWLINE);
		}
	return 0;
}
		...
		for (ALL_LIST_ELEMENTS_RO(om->iflist, node, ifp))
			if (OSPF_IF_PARAM_CONFIGURED
				...
				vty_out(vty, " %spassive-interface %s%s",
					IF_DEF_PARAMS(ifp)->passive_interface ?
					"" : "no ", ifp->name, VTY_NEWLINE);
			}
		for (ALL_LIST_ELEMENTS_RO(ospf->oiflist, node, oi)) {
			if (!OSPF_IF_PARAM_CONFIGURED
			    (oi->params, passive_interface))
				continue;
