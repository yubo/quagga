      ...
      "OSPF Link State Acknowledgment\n" "OSPF all packets\n")
{
	...
	if (argc >= 2) {
		if (strncmp(argv[1], "s", 1) == 0)
			...
			flag =
			    OSPF_DEBUG_SEND | OSPF_DEBUG_RECV |
			    OSPF_DEBUG_DETAIL;
	}
...
ALIAS(no_debug_ospf_packet,
    ...
    DEFUN(debug_ospf_ism,
      debug_ospf_ism_cmd,
      "debug ospf ism", DEBUG_STR OSPF_STR "OSPF Interface State Machine\n")
{
	if (vty->node == CONFIG_NODE) {
		if (argc == 0)
			DEBUG_ON(ism, ISM);
...
ALIAS(debug_ospf_ism,
    ...
    DEFUN(no_debug_ospf_ism,
      ...
      "no debug ospf ism",
      NO_STR DEBUG_STR OSPF_STR "OSPF Interface State Machine")
{
	if (vty->node == CONFIG_NODE) {
		if (argc == 0)
			DEBUG_OFF(ism, ISM);
...
ALIAS(no_debug_ospf_ism,
    ...
    DEFUN(debug_ospf_nsm,
      debug_ospf_nsm_cmd,
      "debug ospf nsm", DEBUG_STR OSPF_STR "OSPF Neighbor State Machine\n")
{
	if (vty->node == CONFIG_NODE) {
		if (argc == 0)
			DEBUG_ON(nsm, NSM);
...
ALIAS(debug_ospf_nsm,
    ...
    DEFUN(no_debug_ospf_nsm,
      ...
      "no debug ospf nsm",
      NO_STR DEBUG_STR OSPF_STR "OSPF Neighbor State Machine")
{
	if (vty->node == CONFIG_NODE) {
		if (argc == 0)
			DEBUG_OFF(nsm, NSM);
...
ALIAS(no_debug_ospf_nsm,
    ...
    DEFUN(debug_ospf_lsa,
      debug_ospf_lsa_cmd,
      "debug ospf lsa", DEBUG_STR OSPF_STR "OSPF Link State Advertisement\n")
{
	if (vty->node == CONFIG_NODE) {
		if (argc == 0)
			DEBUG_ON(lsa, LSA);
...
ALIAS(no_debug_ospf_lsa,
    ...
    DEFUN(debug_ospf_zebra,
      ...
      "debug ospf zebra", DEBUG_STR OSPF_STR "OSPF Zebra information\n")
{
	if (vty->node == CONFIG_NODE) {
		if (argc == 0)
...
ALIAS(no_debug_ospf_zebra,
    ...
    DEFUN(debug_ospf_event,
      ...
      "debug ospf event", DEBUG_STR OSPF_STR "OSPF event information\n")
{
	if (vty->node == CONFIG_NODE)
...
DEFUN(debug_ospf_nssa,
      debug_ospf_nssa_cmd,
      "debug ospf nssa", DEBUG_STR OSPF_STR "OSPF nssa information\n")
{
	if (vty->node == CONFIG_NODE)
...
DEFUN(no_debug_ospf_nssa,
      ...
      "no debug ospf nssa", NO_STR DEBUG_STR OSPF_STR "OSPF nssa information\n")
{
	if (vty->node == CONFIG_NODE)
...
static int config_write_debug(struct vty *vty)
{
	...
	if (IS_CONF_DEBUG_OSPF(zebra, ZEBRA) == OSPF_DEBUG_ZEBRA)
		...
		if (IS_CONF_DEBUG_OSPF(zebra, ZEBRA_INTERFACE))
			vty_out(vty, "debug ospf zebra interface%s",
				VTY_NEWLINE);
		if (IS_CONF_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE))
			vty_out(vty, "debug ospf zebra redistribute%s",
				VTY_NEWLINE);
