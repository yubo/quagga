...
static void ospf_mpls_te_config_write_if(struct vty *vty,
					 struct interface *ifp);
...
static void ospf_mpls_te_lsa_schedule(struct mpls_te_link *lp,
				      enum sched_opcode);
...
int ospf_mpls_te_init(void)
{
	...
	rc = ospf_register_opaque_functab(OSPF_OPAQUE_AREA_LSA, OPAQUE_TYPE_TRAFFIC_ENGINEERING_LSA, ospf_mpls_te_new_if, ospf_mpls_te_del_if, ospf_mpls_te_ism_change, ospf_mpls_te_nsm_change, ospf_mpls_te_config_write_router, ospf_mpls_te_config_write_if, NULL,	/* ospf_mpls_te_config_write_debug */
					  ospf_mpls_te_show_info, ospf_mpls_te_lsa_originate, ospf_mpls_te_lsa_refresh, NULL,	/* ospf_mpls_te_new_lsa_hook */
...
static void ospf_mpls_te_foreach_area(void (*func)
				       (struct mpls_te_link * lp,
					enum sched_opcode),
...
set_linkparams_link_type(struct ospf_interface *oi, struct mpls_te_link *lp)
{
	...
	lp->link_type.header.length =
	    htons(sizeof(lp->link_type.link_type.value));
...
static void build_link_subtlv_link_id(struct stream *s, struct mpls_te_link *lp)
{
...
static void build_link_subtlv_max_bw(struct stream *s, struct mpls_te_link *lp)
{
...
static int ospf_mpls_te_lsa_originate(void *arg)
{
	...
	if (OspfMplsTE.status == disabled) {
		zlog_info
		    ("ospf_mpls_te_lsa_originate: MPLS-TE is disabled now.");
	...
	for (ALL_LIST_ELEMENTS(OspfMplsTE.iflist, node, nnode, lp)) {
		...
		if (!is_mandated_params_set(lp)) {
			zlog_warn
			    ("ospf_mpls_te_lsa_originate: Link(%s) lacks some mandated MPLS-TE parameters.",
			     lp->ifp ? lp->ifp->name : "?");
			continue;
		}
...
static void
ospf_mpls_te_lsa_schedule(struct mpls_te_link *lp, enum sched_opcode opcode)
{
	struct ospf_lsa lsa;
	struct lsa_header lsah;
	u_int32_t tmp;
...
show_vty_link_subtlv_unrsv_bw(struct vty *vty, struct te_tlv_header *tlvh)
{
	...
	for (i = 0; i < 8; i++) {
		...
		if (vty != NULL)
			vty_out(vty,
				"  Unreserved Bandwidth (pri %d): %g (Bytes/sec)%s",
				i, fval, VTY_NEWLINE);
		else
			zlog_debug
			    ("    Unreserved Bandwidth (pri %d): %g (Bytes/sec)",
			     i, fval);
	}
...
static void ospf_mpls_te_config_write_if(struct vty *vty, struct interface *ifp)
{
	...
	if ((OspfMplsTE.status == enabled)
		...
		if (fval >= MPLS_TE_MINIMUM_BANDWIDTH)
			vty_out(vty, " mpls-te link max-bw %g%s", fval,
				VTY_NEWLINE);

		ntohf(&lp->max_rsv_bw.value, &fval);
		if (fval >= MPLS_TE_MINIMUM_BANDWIDTH)
			vty_out(vty, " mpls-te link max-rsv-bw %g%s", fval,
				VTY_NEWLINE);
      ...
      "Link metric for MPLS-TE purpose\n" "Metric\n")
{
	...
	if ((lp = lookup_linkparams_by_ifp(ifp)) == NULL) {
		vty_out(vty, "mpls_te_link_metric: Something wrong!%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
      ...
      "Bytes/second (IEEE floating point format)\n")
{
	...
	if ((lp = lookup_linkparams_by_ifp(ifp)) == NULL) {
		vty_out(vty, "mpls_te_link_maxbw: Something wrong!%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	if (sscanf(argv[0], "%g", &f2) != 1) {
		vty_out(vty, "mpls_te_link_maxbw: fscanf: %s%s",
			safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}

	if (ntohs(lp->max_bw.header.type) == 0 || f1 != f2) {
      ...
      "Bytes/second (IEEE floating point format)\n")
{
	...
	if ((lp = lookup_linkparams_by_ifp(ifp)) == NULL) {
		vty_out(vty, "mpls_te_link_max_rsv_bw: Something wrong!%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	if (sscanf(argv[0], "%g", &f2) != 1) {
		vty_out(vty, "mpls_te_link_max_rsv_bw: fscanf: %s%s",
			safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}
      ...
      "Priority\n" "Bytes/second (IEEE floating point format)\n")
{
	...
	if ((lp = lookup_linkparams_by_ifp(ifp)) == NULL) {
		vty_out(vty, "mpls_te_link_unrsv_bw: Something wrong!%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	if (sscanf(argv[0], "%d", &priority) != 1) {
		vty_out(vty, "mpls_te_link_unrsv_bw: fscanf: %s%s",
			safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}
      ...
      "32-bit Hexadecimal value (ex. 0xa1)\n")
{
	...
	if ((lp = lookup_linkparams_by_ifp(ifp)) == NULL) {
		vty_out(vty, "mpls_te_link_rsc_clsclr: Something wrong!%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}

	if (sscanf(argv[0], "0x%lx", &value) != 1) {
		vty_out(vty, "mpls_te_link_rsc_clsclr: fscanf: %s%s",
			safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}
