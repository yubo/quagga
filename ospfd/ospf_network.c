	...
	if (ret < 0)
		zlog_warn
		    ("can't setsockopt IP_DROP_MEMBERSHIP (fd %d, addr %s, "
		     "ifindex %u, AllSPFRouters): %s", top->fd,
			 ...
			 ifindex)
{
	...
	if (ret < 0)
		zlog_warn
		    ("can't setsockopt IP_DROP_MEMBERSHIP (fd %d, addr %s, "
		     "ifindex %u, AllDRouters): %s", top->fd,
		     inet_ntoa(p->u.prefix4), ifindex, safe_strerror(errno));
	else
		zlog_debug
...
ospf_if_ipmulticast(struct ospf *top, struct prefix *p, unsigned int ifindex)
{
	...
	ret =
	    setsockopt(top->fd, IPPROTO_IP, IP_MULTICAST_LOOP, (void *)&val,
		       len);
	if (ret < 0)
	...
	val = 1;
	ret =
	    setsockopt(top->fd, IPPROTO_IP, IP_MULTICAST_TTL, (void *)&val,
		       len);
	if (ret < 0)
...
int ospf_sock_init(void)
{
	...
	ret =
	    setsockopt(ospf_sock, IPPROTO_IP, IP_HDRINCL, &hincl,
		       sizeof(hincl));
	if (ret < 0) {
		int save_errno = errno;
		if (ospfd_privs.change(ZPRIVS_LOWER))
