...
int
ospf_register_opaque_functab(u_char lsa_type,
			     u_char opaque_type,
			     int (*new_if_hook) (struct interface * ifp),
			     int (*del_if_hook) (struct interface * ifp),
			     void (*ism_change_hook) (struct ospf_interface *
						      oi, int old_status),
			     void (*nsm_change_hook) (struct ospf_neighbor *
						      nbr, int old_status),
			     void (*config_write_router) (struct vty * vty),
			     void (*config_write_if) (struct vty * vty,
						      struct interface * ifp),
			     void (*config_write_debug) (struct vty * vty),
			     void (*show_opaque_info) (struct vty * vty,
						       struct ospf_lsa * lsa),
			     int (*lsa_originator) (void *arg),
			     struct ospf_lsa *(*lsa_refresher) (struct ospf_lsa
								* lsa),
			     int (*new_lsa_hook) (struct ospf_lsa * lsa),
			     int (*del_lsa_hook) (struct ospf_lsa * lsa))
{
...
static struct ospf_opaque_functab *ospf_opaque_functab_lookup(struct ospf_lsa
							      *lsa)
{
	struct list *funclist;
...
static struct opaque_info_per_type *register_opaque_info_per_type(struct
								  ospf_opaque_functab
								  *functab, struct
								  ospf_lsa
								  *new);
static struct opaque_info_per_type *lookup_opaque_info_by_type(struct ospf_lsa
							       *lsa);
static struct opaque_info_per_id *register_opaque_info_per_id(struct
							      opaque_info_per_type
							      *oipt, struct ospf_lsa
							      *new);
static struct opaque_info_per_id *lookup_opaque_info_by_id(struct
							   opaque_info_per_type
							   *oipt, struct ospf_lsa
							   *lsa);
...
static void
opaque_lsa_config_write_router_callback(struct list *funclist, struct vty *vty)
{
	struct listnode *node, *nnode;
	struct ospf_opaque_functab *functab;
...
opaque_lsa_originate_callback(struct list *funclist, void *lsa_type_dependent)
{
	...
	for (ALL_LIST_ELEMENTS(funclist, node, nnode, functab))
		if (functab->lsa_originator != NULL)
			if ((*functab->lsa_originator) (lsa_type_dependent) !=
			    0)
				goto out;
	rc = 0;
out:
...
void ospf_opaque_nsm_change(struct ospf_neighbor *nbr, int old_state)
{
	...
	if (old_state != NSM_Full && nbr->state == NSM_Full) {
		if (CHECK_FLAG(nbr->options, OSPF_OPTION_O)) {
			if (!CHECK_FLAG
			    (top->opaque, OPAQUE_OPERATION_READY_BIT)) {
				if (IS_DEBUG_OSPF_EVENT)
					zlog_debug
...
void ospf_opaque_lsa_originate_schedule(struct ospf_interface *oi, int *delay0)
{
	...
	if (IS_OPAQUE_LSA_ORIGINATION_BLOCKED(top->opaque)) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug
			    ("ospf_opaque_lsa_originate_schedule: Under blockade.");
	...
	if (!list_isempty(ospf_opaque_type9_funclist)
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug
			    ("Schedule Type-9 Opaque-LSA origination in %d sec later.",
			     delay);
		oi->t_opaque_lsa_self =
		    thread_add_timer(master, ospf_opaque_type9_lsa_originate,
				     oi, delay);
	...
	if (!list_isempty(ospf_opaque_type10_funclist)
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug
			    ("Schedule Type-10 Opaque-LSA origination in %d sec later.",
			     delay);
	...
	if (!list_isempty(ospf_opaque_type11_funclist)
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug
			    ("Schedule Type-11 Opaque-LSA origination in %d sec later.",
			     delay);
	...
	if (!list_isempty(ospf_opaque_type10_funclist)
		...
		for (ALL_LIST_ELEMENTS
		     (area->opaque_lsa_self, node, nnode, oipt)) {
			/* 
			 * removed the test for
			 *   (! list_isempty (oipt->id_list))   * Handler is already active. *
			 * because opaque cababilities ON -> OFF -> ON result in list_isempty (oipt->id_list)
	...
	if ((oipt = lookup_opaque_info_by_type(lsa)) == NULL) {
		...
		if ((functab = ospf_opaque_functab_lookup(lsa)) == NULL) {
			zlog_warn("ospf_opaque_lsa_reoriginate_schedule:"
				  ...
				  " opaque_type(%u)", lsa_type, opaque_type);
			goto out;
		}
		if ((oipt =
		     register_opaque_info_per_type(functab, lsa)) == NULL) {
			zlog_warn("ospf_opaque_lsa_reoriginate_schedule:"
...
void ospf_opaque_adjust_lsreq(struct ospf_neighbor *nbr, struct list *lsas)
{
	struct ospf *top;
	struct ospf_area *area;
	struct ospf_interface *oi;
	...
	for (ALL_LIST_ELEMENTS(lsas, node1, nnode1, lsa)) {
		...
		case OSPF_OPAQUE_AREA_LSA:
			area = nbr->oi->area;
			for (ALL_LIST_ELEMENTS
			     (area->oiflist, node2, nnode2, oi))
				ospf_opaque_exclude_lsa_from_lsreq(oi->nbrs,
								   nbr, lsa);
			break;
		case OSPF_OPAQUE_AS_LSA:
				   ...
				   struct ospf_lsa *lsa)
{
	...
	for (rn = route_top(nbrs); rn; rn = route_next(rn)) {
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug
			    ("LSA[%s]: Exclude this entry from LSReq to send.",
			     dump_lsa_key(lsa));
...
ospf_opaque_ls_ack_received(struct ospf_neighbor *nbr, struct ospf_lsa *lsa)
{
	...
	default:
		zlog_warn
		    ("ospf_opaque_ls_ack_received: Unexpected LSA-type(%u)",
		     lsa->data->type);
		return;
	}
...
static void ospf_opaque_type9_lsa_rxmt_nbr_check(struct ospf_interface *oi)
{
	...
	if (n == 0) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug
			    ("Self-originated type-9 Opaque-LSAs: OI(%s): Flush completed",
			     IF_NAME(oi));

		UNSET_FLAG(oi->area->ospf->opaque,
...
static void ospf_opaque_type10_lsa_rxmt_nbr_check(struct ospf_area *area)
{
	...
	if (n == 0) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug
			    ("Self-originated type-10 Opaque-LSAs: AREA(%s): Flush completed",
			     inet_ntoa(area->area_id));
...
static void ospf_opaque_type11_lsa_rxmt_nbr_check(struct ospf *top)
{
	...
	if (n == 0) {
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug
			    ("Self-originated type-11 Opaque-LSAs: Flush completed");
