...
static void ospf_finish_final(struct ospf *ospf)
{
	...
	for (i = ZEBRA_ROUTE_SYSTEM; i <= ZEBRA_ROUTE_MAX; i++)
		if (EXTERNAL_INFO(i) != NULL)
			for (rn = route_top(EXTERNAL_INFO(i)); rn;
			     rn = route_next(rn)) {
				if (rn->info == NULL)
					continue;
...
void ospf_if_update(struct ospf *ospf, struct interface *ifp)
{
	...
	for (rn = route_top(ospf->networks); rn; rn = route_next(rn))
		if (rn->info != NULL) {
			...
			area =
			    ospf_area_get(ospf, network->area_id,
					  network->format);
...
static void ospf_area_type_set(struct ospf_area *area, int type)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("Area[%s]: Configured as %s",
			   inet_ntoa(area->area_id), LOOKUP(ospf_area_type_msg,
							    type));
...
static int
ospf_area_nssa_translator_role_unset(struct ospf *ospf, struct in_addr area_id)
{
	struct ospf_area *area;
