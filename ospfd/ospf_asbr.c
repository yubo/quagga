...
/* Add an External info for AS-external-LSA. */
struct external_info *ospf_external_info_add(u_char type, struct prefix_ipv4 p,
					     ...
					     struct in_addr nexthop)
{
	...
	if (rn)
		if (rn->info) {
			route_unlock_node(rn);
			zlog_warn
			    ("Redistribute[%s]: %s/%d already exists, discard.",
...
struct external_info *ospf_external_info_lookup(u_char type,
						struct prefix_ipv4 *p)
{
	struct route_node *rn;
	rn = route_node_lookup(EXTERNAL_INFO(type), (struct prefix *)p);
			...
			if ((ei = rn->info))
				if (ospf_external_info_find_lsa(ospf, &ei->p)) {
					if (is_prefix_default(&ei->p) &&
						...
						continue;
					ospf_external_lsa_flush(ospf, type,
								&ei->p,
								ei->ifindex
								/*, ei->nexthop */
					    );
