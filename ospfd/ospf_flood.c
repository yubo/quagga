				 ...
				 struct ospf_lsa *new, struct ospf_area *area)
{
	...
	case OSPF_OPAQUE_LINK_LSA:
		...
		for (ALL_LIST_ELEMENTS_RO(ospf->oiflist, node, oi))
			...
			if (IPV4_ADDR_SAME
			    (&oi->address->u.prefix4, &new->data->id)) {
				if (oi->area != area
	   ...
	   struct ospf_lsa *current, struct ospf_lsa *new)
{
	...
	if (current != NULL) {	/* -- endo. */
		if (IS_LSA_SELF(current)
		    ...
		    if (tv_cmp
			(tv_sub(recent_relative_time(), current->tv_recv),
			     ...
			     struct ospf_neighbor *inbr, struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("ospf_flood_through_interface(): "
			   ...
			   IF_NAME(oi),
			   inbr ? inet_ntoa(inbr->router_id) : "NULL",
			   dump_lsa_key(lsa));
	...
	for (rn = route_top(oi->nbrs); rn; rn = route_next(rn)) {
		...
		if (IS_OPAQUE_LSA(lsa->data->type)) {
			...
			if (IS_OPAQUE_LSA_ORIGINATION_BLOCKED(oi->ospf->opaque)
				...
				if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
					zlog_debug
					    ("Skip this neighbor: Initial flushing done.");
				continue;
			}
		}
#endif /* HAVE_OPAQUE_LSA */
		...
		if (inbr) {
			 ...
			 */
			if (IPV4_ADDR_SAME
			    (&lsa->data->adv_router, &onbr->router_id)) {
				if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
					zlog_debug
...
void
ospf_ls_retransmit_delete_nbr_area(struct ospf_area *area, struct ospf_lsa *lsa)
{
	struct listnode *node, *nnode;
	struct ospf_interface *oi;
