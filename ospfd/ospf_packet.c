...
#define OSPF_WRITE_IPHL_SHIFT 2
	...
	if (IS_DEBUG_OSPF_PACKET(type - 1, SEND)) {
		...
		zlog_debug("%s sent to [%s] via [%s].",
			   LOOKUP(ospf_packet_type_str, type),
		...
		if (OSPF_IF_PARAM(oi, v_hello) != ntohs(hello->hello_interval)) {
			zlog_warn
			    ("Packet %s [Hello:RECV]: HelloInterval mismatch "
			     "(expected %u, but received %u).",
			     inet_ntoa(ospfh->router_id), OSPF_IF_PARAM(oi,
									v_hello),
	...
	case NSM_TwoWay:
		zlog_warn
		    ("Packet[DD]: Neighbor %s state is %s, packet discarded.",
		     inet_ntoa(nbr->router_id), LOOKUP(ospf_nsm_state_msg,
						       nbr->state));
		break;
	case NSM_Init:
	    ...
	    struct stream *s, struct ospf_interface *oi, u_int16_t size)
{
	...
	if (nbr->state != NSM_Exchange &&
	    nbr->state != NSM_Loading && nbr->state != NSM_Full) {
	...
	for (ALL_LIST_ELEMENTS(lsas, node, nnode, lsa)) {
		...
		if (lsa->data->type == OSPF_NETWORK_LSA) {
			...
			for (ALL_LIST_ELEMENTS
			     (oi->ospf->oiflist, oinode, oinnode, out_if)) {
				if (out_if == NULL)
					break;

				if ((IPV4_ADDR_SAME
				     (&out_if->address->u.prefix4,
				      &lsa->data->id))
				    &&
				    (!(IPV4_ADDR_SAME
				       (&oi->ospf->router_id,
...
static struct ospf_interface *ospf_associate_packet_vl(struct ospf *ospf,
...
static int ospf_check_auth(struct ospf_interface *oi, struct ospf_header *ospfh)
{
			...
			if (IS_DEBUG_OSPF_PACKET(ospfh->type - 1, RECV))
				zlog_warn
				    ("interface %s: auth-type mismatch, local %s, rcvd Null",
				     IF_NAME(oi), LOOKUP(ospf_auth_type_str,
							 iface_auth_type));
			return 0;
		}
	...
	case OSPF_AUTH_SIMPLE:	/* RFC2328 D.5.2 */
		...
		if (memcmp
		    (OSPF_IF_PARAM(oi, auth_simple), ospfh->u.auth_data,
...
static unsigned
    ...
    (struct router_lsa_link *link,
     u_int16_t linkbytes, const u_int16_t num_links) {
		...
		const u_char headeronly)
{
		...
		if (headeronly) {
			ret =
			    (lsalen - OSPF_LSA_HEADER_SIZE -
			     OSPF_ROUTER_LSA_MIN_SIZE) % 4 ? MSG_NG : MSG_OK;
			break;
		}
		rlsa = (struct router_lsa *)lsah;
		ret = ospf_router_lsa_links_examin((struct router_lsa_link *)rlsa->link, lsalen - OSPF_LSA_HEADER_SIZE - 4,	/* skip: basic header, "flags", 0, "# links" */
	...
	case OSPF_AS_NSSA_LSA:
		...
		ret =
		    (lsalen - OSPF_LSA_HEADER_SIZE -
		     OSPF_AS_EXTERNAL_LSA_MIN_SIZE) % 12 ? MSG_NG : MSG_OK;
		break;
...
static unsigned ospf_lsaseq_examin(struct lsa_header *lsah,	/* start of buffered data */
				   ...
				   const u_int32_t declared_num_lsas)
{
	...
	while (length) {
		...
		if (headeronly) {
			...
			lsah =
			    (struct lsa_header *)((caddr_t) lsah +
...
ospf_packet_examin(struct ospf_header *oh, const unsigned bytesonwire)
{
	...
	if (oh->type >= OSPF_MSG_HELLO &&
		...
		if (IS_DEBUG_OSPF_PACKET(0, RECV))
			zlog_debug("%s: undersized (%u B) %s packet", __func__,
				   bytesdeclared, LOOKUP(ospf_packet_type_str,
							 oh->type));
		return MSG_NG;
	}
	switch (oh->type) {
	case OSPF_MSG_HELLO:
		...
		ret =
		    (bytesdeclared - OSPF_HEADER_SIZE -
		     OSPF_HELLO_MIN_SIZE) % 4 ? MSG_NG : MSG_OK;
		break;
	case OSPF_MSG_DB_DESC:
		/* RFC2328 A.3.3, packet header + OSPF_DB_DESC_MIN_SIZE bytes followed
		   by N>=0 header-only LSAs. */
		ret = ospf_lsaseq_examin((struct lsa_header *)((caddr_t) oh + OSPF_HEADER_SIZE + OSPF_DB_DESC_MIN_SIZE), bytesdeclared - OSPF_HEADER_SIZE - OSPF_DB_DESC_MIN_SIZE, 1,	/* header-only LSAs */
					 0);
		break;
	...
	case OSPF_MSG_LS_UPD:
		...
		lsupd =
		    (struct ospf_ls_update *)((caddr_t) oh + OSPF_HEADER_SIZE);
		ret = ospf_lsaseq_examin((struct lsa_header *)((caddr_t) lsupd + OSPF_LS_UPD_MIN_SIZE), bytesdeclared - OSPF_HEADER_SIZE - OSPF_LS_UPD_MIN_SIZE, 0,	/* full LSAs */
					 ntohl(lsupd->num_lsas)	/* 32 bits */
		    );
		break;
	case OSPF_MSG_LS_ACK:
		/* RFC2328 A.3.6, packet header followed by N>=0 header-only LSAs. */
		ret = ospf_lsaseq_examin((struct lsa_header *)((caddr_t) oh + OSPF_HEADER_SIZE + OSPF_LS_ACK_MIN_SIZE), bytesdeclared - OSPF_HEADER_SIZE - OSPF_LS_ACK_MIN_SIZE, 1,	/* header-only LSAs */
					 0);
		break;
	default:
		if (IS_DEBUG_OSPF_PACKET(0, RECV))
	...
	if (ret != MSG_OK && IS_DEBUG_OSPF_PACKET(0, RECV))
		zlog_debug("%s: malformed %s packet", __func__,
			   LOOKUP(ospf_packet_type_str, oh->type));
	return ret;
}
...
int ospf_read(struct thread *thread)
{
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug("ignoring packet from router %s sent to %s, "
				   ...
				   inet_ntop(AF_INET, &ospfh->router_id, buf[0],
					     sizeof(buf[0])), inet_ntop(AF_INET,
									&iph->ip_dst,
									buf[1],
									sizeof
									(buf
	...
	if ((oi == NULL) || (OSPF_IS_AREA_ID_BACKBONE(ospfh->area_id)
		...
		if ((oi =
		     ospf_associate_packet_vl(ospf, ifp, iph, ospfh)) == NULL) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug
	 ...
	 */
		...
		zlog_warn
		    ("Ignoring packet from %s to %s received on interface that is "
		     "down [%s]; interface flags are %s", inet_ntop(AF_INET,
								    &iph->ip_src,
								    buf[0],
								    sizeof(buf
									   [0])),
...
static void
ospf_fill_header(struct ospf_interface *oi, struct stream *s, u_int16_t length)
{
	struct ospf_header *ospfh;
		...
		for (rn = route_top(table); rn; rn = route_next(rn))
			if ((lsa = rn->info) != NULL) {
				...
				if (!CHECK_FLAG(lsa->flags, OSPF_LSA_DISCARD)) {
					...
					lsah =
					    (struct lsa_header *)(STREAM_DATA(s)
								  +
								  stream_get_endp
								  (s));
...
static int ospf_make_ls_req(struct ospf_neighbor *nbr, struct stream *s)
{
	...
	for (i = OSPF_MIN_LSA; i < OSPF_MAX_LSA; i++) {
		...
		for (rn = route_top(table); rn; rn = route_next(rn))
			if ((lsa = (rn->info)) != NULL)
				if (ospf_make_ls_req_func
				    (s, &length, delta, nbr, lsa) == 0) {
		 ...
		 struct stream *s)
{
	...
	while ((node = listhead(update)) != NULL) {
		...
		/* Keep pointer to LS age. */
		lsah =
		    (struct lsa_header *)(STREAM_DATA(s) + stream_get_endp(s));
		...
		/* each hop must increment an lsa_age by transmit_delay 
		   of OSPF interface */
		ls_age =
		    ls_age_increment(lsa, OSPF_IF_PARAM(oi, transmit_delay));
		lsah->ls_age = htons(ls_age);
...
void
ospf_ls_upd_send_lsa(struct ospf_neighbor *nbr, struct ospf_lsa *lsa, int flag)
{
	struct list *update;
						  ...
						  struct ospf_interface *oi)
{
	...
	if ((OSPF_LS_UPD_MIN_SIZE + ntohs(lsa->data->length))
		 ...
		 */
		size =
		    ntohs(lsa->data->length) + (oi->ifp->mtu -
						ospf_packet_max(oi))
		    + OSPF_LS_UPD_MIN_SIZE;
	} else
		size = oi->ifp->mtu;
		...
		if (IS_DEBUG_OSPF_EVENT)
			zlog_debug
			    ("ospf_ls_upd_send_queue: update lists not cleared,"
			     ...
			     again);
		oi->t_ls_upd_event =
		    thread_add_event(master, ospf_ls_upd_send_queue_event, oi,
				     0);
	}
