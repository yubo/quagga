	...
	for (rn = route_top(rt); rn; rn = route_next(rn))
		if ((or = rn->info) != NULL) {
			if (or->type == OSPF_DESTINATION_NETWORK)
				...
				ospf_zebra_delete_discard((struct prefix_ipv4 *)
							  &rn->p);
		}
}
		      ...
		      struct ospf_route *newor)
{
	...
	if (or->type == newor->type && or->cost == newor->cost) {
		if (or->type == OSPF_DESTINATION_NETWORK) {
			...
			/* Check each path. */
			for (n1 = listhead(or->paths), n2 =
			     listhead(newor->paths); n1 && n2;
			     n1 = listnextnode(n1), n2 = listnextnode(n2)) {
				op = listgetdata(n1);
				newop = listgetdata(n2);

				if (!IPV4_ADDR_SAME
				    (&op->nexthop, &newop->nexthop))
					return 0;
		...
		if ((or = rn->info) != NULL) {
			if (or->type == OSPF_DESTINATION_NETWORK) {
				if (!ospf_route_match_same(ospf->old_table,
							   (struct prefix_ipv4
							    *)&rn->p, or))
					ospf_zebra_add((struct prefix_ipv4 *)
						       &rn->p, or);
			} else if (or->type == OSPF_DESTINATION_DISCARD)
				if (!ospf_route_match_same(ospf->old_table,
							   (struct prefix_ipv4
							    *)&rn->p, or))
					ospf_zebra_add_discard((struct
								prefix_ipv4 *)
							       &rn->p);
		}
}
		       ...
		       struct ospf_area *area)
{
	...
	if (rn->info) {
		...
		if (v->distance > cur_or->cost ||
		    IPV4_ADDR_CMP(&cur_or->u.std.origin->id,
				  &lsa->header.id) > 0)
			return;
		    ...
		    int parent_is_root, int lsa_pos)
{
	...
	if (rn->info) {
		...
		if (cost == cur_or->cost) {
			...
			if (IPV4_ADDR_CMP
			    (&cur_or->u.std.origin->id, &lsa->header.id) < 0)
			...
			if (or->type == OSPF_DESTINATION_NETWORK) {
				zlog_debug("N %s/%d\t%s\t%s\t%d",
					   inet_ntop(AF_INET, &rn->p.u.prefix4,
						     buf1, BUFSIZ),
					   rn->p.prefixlen, inet_ntop(AF_INET,
								      &or->
								      u.std.area_id,
								      buf2,
								      BUFSIZ),
...
/* Compare two routes.
 ...
 ret >  0 -- r2 is better. */
int
ospf_route_cmp(struct ospf *ospf, struct ospf_route *r1, struct ospf_route *r2)
{
	int ret = 0;
	...
	case OSPF_PATH_TYPE1_EXTERNAL:
		if (!CHECK_FLAG(ospf->config, OSPF_RFC1583_COMPATIBLE)) {
			ret =
			    ospf_asbr_route_cmp(ospf, r1->u.ext.asbr,
						r2->u.ext.asbr);
			if (ret != 0)
				return ret;
		}
...
void
ospf_route_copy_nexthops_from_vertex(struct ospf_route *to, struct vertex *v)
{
	...
	for (ALL_LIST_ELEMENTS_RO(v->parents, node, vp)) {
		...
		if (nexthop->oi != NULL) {
			if (!ospf_path_exist
			    (to->paths, nexthop->router, nexthop->oi)) {
				path = ospf_path_new();
...
void ospf_delete_discard_route(struct route_table *rt, struct prefix_ipv4 *p)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("ospf_delete_discard_route(): "
			   "deleting %s/%d", inet_ntoa(p->prefix),
			   p->prefixlen);
