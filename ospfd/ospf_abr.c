...
static int
ospf_abr_nexthops_belong_to_area(struct ospf_route *or, struct ospf_area *area)
{
	struct listnode *node, *nnode;
...
ospf_abr_process_network_rt(struct ospf *ospf, struct route_table *rt)
{
	...
	for (rn = route_top(rt); rn; rn = route_next(rn)) {
		...
		if (!
		    (area =
		     ospf_area_lookup_by_area_id(ospf, or->u.std.area_id))) {
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug
...
ospf_abr_process_router_rt(struct ospf *ospf, struct route_table *rt)
{
	...
	for (rn = route_top(rt); rn; rn = route_next(rn)) {
		...
		for (ALL_LIST_ELEMENTS(l, node, nnode, or)) {
			if (!ospf_area_lookup_by_area_id
			    (ospf, or->u.std.area_id)) {
				if (IS_DEBUG_OSPF_EVENT)
					zlog_debug
...
static void ospf_abr_unapprove_translates(struct ospf *ospf)
...
static void ospf_abr_nssa_task(struct ospf *ospf)
