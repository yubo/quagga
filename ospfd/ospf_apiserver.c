...
int ospf_apiserver_init(void)
{
	...
	rc = ospf_register_opaque_functab(0 /* all LSAs */ ,
					  0 /* all opaque types */ ,
					  ospf_apiserver_new_if, ospf_apiserver_del_if, ospf_apiserver_ism_change, ospf_apiserver_nsm_change, NULL, NULL, NULL, NULL,	/* ospf_apiserver_show_info */
...
static int ospf_apiserver_new_lsa_hook(struct ospf_lsa *lsa)
{
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("API: Put LSA(%p)[%s] into reserve, total=%ld", lsa,
...
{
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("API: Get LSA(%p)[%s] from reserve, total=%ld", lsa,
			   dump_lsa_key(lsa), lsa->lsdb->total);
	return 0;
}
...
void
ospf_apiserver_event(enum event event, int fd, struct ospf_apiserver *apiserv)
{
	switch (event) {
	...
	case OSPF_APISERVER_SYNC_WRITE:
		if (!apiserv->t_sync_write) {
			apiserv->t_sync_write =
			    thread_add_write(master, ospf_apiserver_sync_write,
					     apiserv, fd);
		}
		break;
	case OSPF_APISERVER_ASYNC_WRITE:
		if (!apiserv->t_async_write) {
			apiserv->t_async_write =
			    thread_add_write(master, ospf_apiserver_async_write,
					     apiserv, fd);
		}
		break;
	}
...
void ospf_apiserver_free(struct ospf_apiserver *apiserv)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("API: Delete apiserv(%p), total#(%d)", apiserv,
...
int ospf_apiserver_accept(struct thread *thread)
{
	...
	ret =
	    getpeername(new_sync_sock, (struct sockaddr *)&peer_sync, &peerlen);
	if (ret < 0) {
				    ...
				    u_char lsa_type, u_char opaque_type)
{
	...
	rc = ospf_register_opaque_functab(lsa_type, opaque_type, NULL,	/* ospf_apiserver_new_if */
					  NULL,	/* ospf_apiserver_del_if */
					  NULL,	/* ospf_apiserver_ism_change */
					  NULL,	/* ospf_apiserver_nsm_change */
					  NULL, NULL, NULL, ospf_apiserver_show_info, originator_func, ospf_apiserver_lsa_refresher, NULL,	/* ospf_apiserver_lsa_update */
			...
			if (IS_DEBUG_OSPF_EVENT)
				zlog_debug
				    ("API: Del LSA-type(%d)/Opaque-type(%d)"
				     " from apiserv(%p), total#(%d)", lsa_type,
				     opaque_type, apiserv,
				     listcount(apiserv->opaque_types));
...
int
ospf_apiserver_handle_sync_lsdb(struct ospf_apiserver *apiserv, struct msg *msg)
{
	struct listnode *node, *nnode;
	u_int32_t seqnum;
	int rc = 0;
					...
					struct msg *msg)
{
	...
	if (!apiserver_is_opaque_type_registered
	    (apiserv, lsa_type, opaque_type)) {
		zlog_warn
...
struct ospf_lsa *ospf_apiserver_lsa_refresher(struct ospf_lsa *lsa)
{
	...
	if (!new) {
		...
		new =
		    ospf_apiserver_opaque_lsa_new(lsa->area, lsa->oi,
						  lsa->data);
		if (!new) {
			zlog_warn
				...
				u_char lsa_type, u_char opaque_type)
{
	...
	case OSPF_OPAQUE_AS_LSA:
		LSDB_LOOP(OPAQUE_LINK_LSDB(ospf), rn, lsa)
		    apiserver_flush_opaque_type_callback(lsa, (void *)&param,
							 0);
		break;
	default:
		break;
...
void ospf_apiserver_clients_notify_nsm_change(struct ospf_neighbor *nbr)
{
	...
	msg =
	    new_msg_nsm_change(0, ifaddr, nbraddr, nbr->router_id, nbr->state);
	if (!msg) {
		zlog_warn
