...
static struct ospf_route *ospf_ase_calculate_asbr_route(struct ospf *ospf, struct route_table
							*rt_network, struct route_table
							*rt_router, struct as_external_lsa
							*al)
{
	struct prefix_ipv4 asbr;
	...
	if (asbr_route == NULL) {
		if (IS_DEBUG_OSPF(lsa, LSA))
			zlog_debug
			    ("ospf_ase_calculate(): Route to ASBR %s not found",
	...
	if (!(asbr_route->u.std.flags & ROUTER_LSA_EXTERNAL)) {
		if (IS_DEBUG_OSPF(lsa, LSA))
			zlog_debug
			    ("ospf_ase_calculate(): Originating router is not an ASBR");
		return NULL;
	}
...
static struct ospf_route *ospf_ase_calculate_new_route(struct ospf_lsa *lsa, struct ospf_route
						       *asbr_route,
						       u_int32_t metric)
{
			...
			struct route_table *old_external_route)
{
	...
	for (rn = route_top(old_external_route); rn; rn = route_next(rn))
		if ((or = rn->info)) {
			if (!
			    (new_rn =
			     route_node_lookup(new_external_route, &rn->p)))
	...
	for (rn = route_top(new_external_route); rn; rn = route_next(rn))
		if ((or = rn->info) != NULL)
			if (!ospf_ase_route_match_same
			    (old_external_route, &rn->p, or))
