...
static struct vertex *ospf_vertex_new(struct ospf_lsa *lsa)
{
	...
	if (IS_DEBUG_OSPF_EVENT)
		zlog_debug("%s: Created %s vertex %s", __func__,
			   new->type ==
			   OSPF_VERTEX_ROUTER ? "Router" : "Network",
			...
			if (vp) {
				zlog_debug
				    ("parent %s backlink %d nexthop %s  interface %s",
				     ...
				     vp->backlink, inet_ntop(AF_INET,
							     &vp->
							     nexthop->router,
							     buf1, BUFSIZ),
				     vp->nexthop->oi ? IF_NAME(vp->
							       nexthop->oi) :
				     "NULL");
			}
		}
...
static int ospf_lsa_has_link(struct lsa_header *w, struct lsa_header *v)
{
	...
	if (w->type == OSPF_ROUTER_LSA) {
		...
		for (i = 0; i < ntohs(rl->links)
		     && length >= sizeof(struct router_lsa); i++, length -= 12) {
 ...
 */
static struct router_lsa_link *ospf_get_next_link(struct vertex *v,
						  struct vertex *w,
		    ...
		    struct vertex_nexthop *newhop, unsigned int distance)
{
	...
	if (IS_DEBUG_OSPF_EVENT) {
		...
		zlog_debug("%s: Adding %s as parent of %s",
			   __func__,
			   inet_ntop(AF_INET, &v->lsa->id, buf[0],
	...
	if (v == area->spf) {
		...
		if (!oi) {
			zlog_debug
			    ("%s: OI not found in LSA: lsa_pos:%d link_id:%s link_data:%s",
			     __func__, lsa_pos, inet_ntop(AF_INET, &l->link_id,
							  buf1, BUFSIZ),
		...
		if (IS_DEBUG_OSPF_EVENT) {
			zlog_debug("%s: considering link:%s "
				   ...
				   inet_ntop(AF_INET, &l->link_id, buf1,
					     BUFSIZ), inet_ntop(AF_INET,
								&l->link_data,
								buf2, BUFSIZ));
		}
	...
	while (p < lim) {
		...
		if (v->lsa->type == OSPF_ROUTER_LSA) {
			...
			case LSA_LINK_TYPE_VIRTUALLINK:
				...
				w_lsa =
				    ospf_lsa_lookup(area, OSPF_ROUTER_LSA,
						    l->link_id, l->link_id);
				if (w_lsa) {
					if (IS_DEBUG_OSPF_EVENT)
						zlog_debug
			...
			w_lsa =
			    ospf_lsa_lookup_by_id(area, OSPF_ROUTER_LSA, *r);
			if (w_lsa) {
				if (IS_DEBUG_OSPF_EVENT)
		...
		if (w_lsa->stat == LSA_SPF_NOT_EXPLORED) {
			...
			if (ospf_nexthop_calculation
			    (area, v, w, l, distance, lsa_pos))
...
#if 0
static void ospf_rtrs_print(struct route_table *rtrs)
{
	struct route_node *rn;
	struct list *or_list;
	struct listnode *ln;
...
void ospf_spf_calculate_schedule(struct ospf *ospf, ospf_spf_reason_t reason)
{
	...
	ospf->t_spf_calc =
	    thread_add_timer_msec(master, ospf_spf_calculate_timer, ospf,
				  delay);
}
