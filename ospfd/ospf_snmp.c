...
static struct ospf_area *ospf_area_lookup_next(struct ospf *ospf,
...
static u_char *ospfStubAreaEntry(struct variable *v, oid * name,
					 ...
					 int router_id_next)
{
	...
	for (; i < OSPF_MAX_LSA; i++) {
		*type = i;
		...
		lsa =
		    ospf_lsdb_lookup_by_id_next(area->lsdb, *type, *ls_id,
	...
	if (exact) {
		...
		*type = *offset;
		...
		do {
			...
			lsa =
			    lsdb_lookup_next(area, type, type_next, ls_id,
		...
		while ((area =
			ospf_area_lookup_next(ospf, area_id, 0)) != NULL);
	}
	return NULL;
}
			     ...
			     WriteMethod ** write_method)
{
	...
	lsa =
	    ospfLsdbLookup(v, name, length, &area_id, &type, &ls_id, &router_id,
			   exact);
	if (!lsa)
		return NULL;
						   ...
						   int exact)
{
	...
	if (exact) {
		...
		if (len == IN_ADDR_SIZE)
			...
			area =
			    ospf_area_lookup_next(ospf, area_id,
						  len == 0 ? 1 : 0);
		...
		do {
			...
			if (range) {
				/* Fill in length. */
				*length =
				    v->namelen + IN_ADDR_SIZE + IN_ADDR_SIZE;
...
static u_char *ospfAreaRangeEntry(struct variable *v, oid * name,
				  size_t * length, int exact, size_t * var_len,
				  WriteMethod ** write_method)
{
	struct ospf_area_range *range;
	...
	range =
	    ospfAreaRangeLookup(v, name, length, &area_id, &range_net, exact);
	if (!range)
		return NULL;
...
static u_char *ospfIfMetricEntry(struct variable *v, oid * name,
				 size_t * length, int exact, size_t * var_len,
				 WriteMethod ** write_method)
{
...
static struct ospf_vl_data *ospfVirtIfLookup(struct variable *v, oid * name,
					     size_t * length,
					     struct in_addr *area_id,
					     struct in_addr *neighbor,
					     int exact)
				...
				WriteMethod ** write_method)
{
	...
	lsa =
	    ospfExtLsdbLookup(v, name, length, &type, &ls_id, &router_id,
			      exact);
	if (!lsa)
		return NULL;
