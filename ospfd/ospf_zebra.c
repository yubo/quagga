...
ospf_interface_add(int command, struct zclient *zclient, zebra_size_t length)
{
	...
	if (IS_DEBUG_OSPF(zebra, ZEBRA_INTERFACE))
		zlog_debug
		    ("Zebra: interface add %s index %d flags %llx metric %d mtu %d",
...
static int
ospf_interface_delete(int command, struct zclient *zclient, zebra_size_t length)
{
	struct interface *ifp;
	struct stream *s;
		...
		if (IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE))
			zlog_debug
			    ("Redistribute[%s]: Refresh  Type[%d], Metric[%d]",
			     ospf_redist_string(type), metric_type(ospf, type),
			     metric_value(ospf, type));
...
static int
ospf_zebra_read_ipv4(int command, struct zclient *zclient, zebra_size_t length)
{
	struct stream *s;
	struct zapi_ipv4 api;
			...
			if (ei) {
				if (is_prefix_default(&p))
					...
					if (!current)
						...
						zlog_warn
						    ("ospf_zebra_read_ipv4() : %s already exists",
						     inet_ntoa(p.prefix));
				}
			}
		}
...
ospf_distance_unset(struct vty *vty, struct ospf *ospf,
		    const char *distance_str, const char *ip_str, char
