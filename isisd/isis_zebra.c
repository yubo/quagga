			  ...
			  struct isis_route_info *route_info)
{
	...
	size =
	    sizeof(struct isis_nexthop6 *) * listcount(route_info->nexthops6);
...
static int
isis_zebra_read_ipv4(int command, struct zclient *zclient, zebra_size_t length)
{
	struct stream *stream;
	struct zapi_ipv4 api;
...
#ifdef HAVE_IPV6
static int
isis_zebra_read_ipv6(int command, struct zclient *zclient, zebra_size_t length)
{
	return 0;
}
#endif
