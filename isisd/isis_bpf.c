...
int isis_recv_pdu_p2p(struct isis_circuit *circuit, u_char * ssnpa)
{
	...
	if (bytesread < 0) {
		zlog_warn("isis_recv_pdu_p2p(): read () failed: %s",
			  safe_strerror(errno));
		return ISIS_WARNING;
	}
...
int isis_send_pdu_bcast(struct isis_circuit *circuit, int level)
{
	...
	if (buflen > sizeof(sock_buff)) {
		zlog_warn
		    ("isis_send_pdu_bcast: sock_buff size %lu is less than "
		     "output pdu size %d on circuit %s", sizeof(sock_buff),
		     buflen, circuit->interface->name);
		return ISIS_WARNING;
	}
