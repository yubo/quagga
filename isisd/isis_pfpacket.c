...
static int open_packet_socket(struct isis_circuit *circuit)
{
	...
	if (if_is_broadcast(circuit->interface)) {
		...
		retval =
		    isis_multicast_join(circuit->fd, 0,
					circuit->interface->ifindex);
	}
...
int isis_recv_pdu_bcast(struct isis_circuit *circuit, u_char * ssnpa)
{
	...
	if (bytesread < 0) {
		zlog_warn
		    ("isis_recv_packet_bcast(): ifname %s, fd %d, bytesread %d, "
		     "recvfrom(): %s", circuit->interface->name, circuit->fd,
		     bytesread, safe_strerror(errno));
		/* get rid of the packet */
		bytesread =
		    recvfrom(circuit->fd, discard_buff, sizeof(discard_buff),
	...
	bytesread =
	    recvfrom(circuit->fd, sock_buff, sizeof(sock_buff), MSG_DONTWAIT,
