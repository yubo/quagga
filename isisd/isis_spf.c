...
static const char *vid2string(struct isis_vertex *vertex, u_char * buff)
{
	...
	case VTYPE_IP6REACH_EXTERNAL:
#endif /* HAVE_IPV6 */
		prefix2str((struct prefix *)&vertex->N.prefix, (char *)buff,
			   BUFSIZ);
		break;
	default:
		return "UNKNOWN";
	...
	if (vertex) {
		...
		if (vertex->d_N == cost) {
			...
			for (ALL_LIST_ELEMENTS
			     (vertex->parents, pnode, pnextnode, pvertex))
...
process_N(struct isis_spftree *spftree, enum vertextype vtype, void *id,
	  uint32_t dist, uint16_t depth, int family, struct isis_vertex *parent)
{
	struct isis_vertex *vertex;
#ifdef EXTREME_DEBUG
	u_char buff[255];
...
lspfragloop:
	...
	if (!ISIS_MASK_LSP_OL_BIT(lsp->lsp_header->lsp_bits)) {
		if (lsp->tlv_data.is_neighs) {
			for (ALL_LIST_ELEMENTS_RO
			     (lsp->tlv_data.is_neighs, node, is_neigh)) {
				/* C.2.6 a) */
				/* Two way connectivity */
				if (!memcmp
				    (is_neigh->neigh_id, root_sysid,
				     ISIS_SYS_ID_LEN))
					continue;
				if (!memcmp
				    (is_neigh->neigh_id, null_sysid,
				     ISIS_SYS_ID_LEN))
					continue;
				dist = cost + is_neigh->metrics.metric_default;
				vtype =
				    LSP_PSEUDO_ID(is_neigh->neigh_id) ?
				    VTYPE_PSEUDO_IS : VTYPE_NONPSEUDO_IS;
		...
		if (lsp->tlv_data.te_is_neighs) {
			for (ALL_LIST_ELEMENTS_RO
			     (lsp->tlv_data.te_is_neighs, node, te_is_neigh)) {
				if (!memcmp
				    (te_is_neigh->neigh_id, root_sysid,
				     ISIS_SYS_ID_LEN))
					continue;
				if (!memcmp
				    (te_is_neigh->neigh_id, null_sysid,
				     ISIS_SYS_ID_LEN))
					continue;
				dist = cost + GET_TE_METRIC(te_is_neigh);
				vtype =
				    LSP_PSEUDO_ID(te_is_neigh->neigh_id) ?
				    VTYPE_PSEUDO_TE_IS : VTYPE_NONPSEUDO_TE_IS;
				process_N(spftree, vtype,
	...
	if (family == AF_INET && lsp->tlv_data.ipv4_int_reachs) {
		...
		for (ALL_LIST_ELEMENTS_RO
		     (lsp->tlv_data.ipv4_int_reachs, node, ipreach)) {
	...
	if (family == AF_INET && lsp->tlv_data.ipv4_ext_reachs) {
		prefix.family = AF_INET;
		for (ALL_LIST_ELEMENTS_RO
		     (lsp->tlv_data.ipv4_ext_reachs, node, ipreach)) {
			dist = cost + ipreach->metrics.metric_default;
	...
	if (family == AF_INET6 && lsp->tlv_data.ipv6_reachs) {
		...
		for (ALL_LIST_ELEMENTS_RO
		     (lsp->tlv_data.ipv6_reachs, node, ip6reach)) {
			assert(ip6reach->prefix_len <= IPV6_MAX_BITLEN);

			dist = cost + ntohl(ip6reach->metric);
			vtype =
			    (ip6reach->control_info & CTRL_INFO_DISTRIBUTION) ?
...
pseudofragloop:
	...
	if (lsp->tlv_data.is_neighs)
		for (ALL_LIST_ELEMENTS_RO
			...
			/* Two way connectivity */
			if (!memcmp
			    (is_neigh->neigh_id, root_sysid, ISIS_SYS_ID_LEN))
				continue;
			dist = cost + is_neigh->metrics.metric_default;
			vtype =
			    LSP_PSEUDO_ID(is_neigh->neigh_id) ? VTYPE_PSEUDO_IS
			    : VTYPE_NONPSEUDO_IS;
		...
		if (circuit->circ_type == CIRCUIT_T_BROADCAST) {
			...
			for (ALL_LIST_ELEMENTS_RO(adj_list, anode, adj)) {
				...
				case ISIS_SYSTYPE_ES:
					isis_spf_add_local(spftree, VTYPE_ES,
							   adj->sysid, adj,
							   circuit->te_metric
							   [level - 1], family,
							   parent);
			...
			lsp =
			    lsp_search(lsp_id, spftree->area->lspdb[level - 1]);
			if (lsp == NULL || lsp->lsp_header->rem_lifetime == 0) {
				zlog_warn
				    ("ISIS-Spf: No lsp (%p) found from root "
				     "to L%d DR %s on %s (ID %d)", lsp, level,
				     rawlspid_print(lsp_id),
				     circuit->interface->name,
				     circuit->circuit_id);
				continue;
...
isis_run_spf(struct isis_area *area, int level, int family, u_char * sysid)
{
	...
	/*              b) */
	retval =
	    isis_spf_preload_tent(spftree, level, family, sysid, root_vertex);
	if (retval != ISIS_OK) {
		...
		case VTYPE_NONPSEUDO_TE_IS:
			...
			if (lsp && lsp->lsp_header->rem_lifetime != 0) {
				if (LSP_PSEUDO_ID(lsp_id)) {
					isis_spf_process_pseudo_lsp(spftree,
								    lsp,
								    vertex->d_N,
								    vertex->depth,
								    family,
...
isis_print_paths(struct vty *vty, struct list *paths, u_char * root_sysid)
{
	...
	for (ALL_LIST_ELEMENTS_RO(paths, node, vertex)) {
		...
		if (listcount(vertex->children) > 0) {
			...
			for (ALL_LIST_ELEMENTS_RO
			     (vertex->children, cnode, cvertex)) {
				vty_out(vty, "%s", VTY_NEWLINE);
				vty_out(vty, "%-72s", "");
				vty_out(vty, "%s(%d) ",
					vid2string(cvertex, buff),
					cvertex->type);
			}
		}
#endif
