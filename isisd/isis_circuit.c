...
isis_circuit_configure(struct isis_circuit *circuit, struct isis_area *area)
{
	...
	if (area->is_type != IS_LEVEL_1_AND_2
	    && area->is_type != circuit->is_type)
		zlog_warn
		    ("circut %s is_type %d mismatch with area %s is_type %d",
...
void
isis_circuit_add_addr(struct isis_circuit *circuit, struct connected *connected)
{
	struct listnode *node;
	...
	if (connected->address->family == AF_INET) {
		...
		if (IPV4_NET0(addr) ||
		    IPV4_NET127(addr) ||
		    IN_CLASSD(addr) || IPV4_LINKLOCAL(addr))
			return;

		for (ALL_LIST_ELEMENTS_RO(circuit->ip_addrs, node, ipv4))
			if (prefix_same
			    ((struct prefix *)ipv4, connected->address))
				return;
	...
	if (connected->address->family == AF_INET6) {
		...
		for (ALL_LIST_ELEMENTS_RO(circuit->ipv6_link, node, ipv6))
			if (prefix_same
			    ((struct prefix *)ipv6, connected->address))
				return;
...
void
isis_circuit_del_addr(struct isis_circuit *circuit, struct connected *connected)
{
	...
	if (connected->address->family == AF_INET) {
		...
		for (ALL_LIST_ELEMENTS_RO(circuit->ip_addrs, node, ip))
			if (prefix_same
			    ((struct prefix *)ip, (struct prefix *)ipv4))
				break;
	...
	if (connected->address->family == AF_INET6) {
		...
		if (IN6_IS_ADDR_LINKLOCAL(&ipv6->prefix)) {
			...
			for (ALL_LIST_ELEMENTS_RO
			     (circuit->ipv6_non_link, node, ip6)) {
				if (prefix_same
				    ((struct prefix *)ip6,
				     (struct prefix *)ipv6))
					break;
...
int isis_circuit_up(struct isis_circuit *circuit)
{
	...
	if (circuit->circ_type == CIRCUIT_T_BROADCAST) {
		...
		if (circuit->interface->sdl.sdl_alen != ETHER_ADDR_LEN)
			...
			memcpy(circuit->u.bc.snpa,
			       LLADDR(&circuit->interface->sdl), ETH_ALEN);
#endif
#else
	...
	if (circuit->is_type & IS_LEVEL_1)
		THREAD_TIMER_ON(master, circuit->t_send_psnp[0], send_l1_psnp,
				circuit, isis_jitter(circuit->psnp_interval[0],
						     PSNP_JITTER));

	if (circuit->is_type & IS_LEVEL_2)
		THREAD_TIMER_ON(master, circuit->t_send_psnp[1], send_l2_psnp,
				circuit, isis_jitter(circuit->psnp_interval[1],
						     PSNP_JITTER));
      ...
      "Set default metric for circuit\n" "Default metric value\n")
{
	...
	if (circuit->area && circuit->area->oldmetric == 1 &&
		...
		vty_out(vty, "Invalid metric %d - should be <0-63> "
			"when narrow metric type enabled%s", met, VTY_NEWLINE);
		return CMD_ERR_AMBIGUOUS;
	}
	...
	if (circuit->area && circuit->area->newmetric == 1 &&
		...
		vty_out(vty, "Invalid metric %d - should be <0-16777215> "
			"when wide metric type enabled%s", met, VTY_NEWLINE);
		return CMD_ERR_AMBIGUOUS;
	}
