...
static struct isis_adjacency *adj_alloc(u_char * id)
{
...
struct isis_adjacency *isis_adj_lookup_snpa(u_char * ssnpa, struct list *adjdb)
{
	struct listnode *node;
	struct isis_adjacency *adj;
	...
	if (circuit->area->log_adj_changes) {
		...
		zlog_info
		    ("%%ADJCHANGE: Adjacency to %s (%s) changed from %s to %s, %s",
		     ...
		     adj_state2string(old_state), adj_state2string(new_state),
		     reason ? reason : "unspecified");
	}
	...
	if (circuit->circ_type == CIRCUIT_T_BROADCAST) {
		for (level = IS_LEVEL_1; level <= IS_LEVEL_2; level++) {
			...
			if (circuit->u.bc.lan_neighs[level - 1]) {
				list_delete_all_node(circuit->u.
						     bc.lan_neighs[level - 1]);
				isis_adj_build_neigh_list(circuit->u.
							  bc.adjdb[level - 1],
							  circuit->u.
							  bc.lan_neighs[level -
									1]);
...
isis_adj_print_vty(struct isis_adjacency *adj, struct vty *vty, char detail)
{
	...
	if (detail == ISIS_UI_LEVEL_BRIEF) {
		...
		if (adj->last_upd)
			vty_out(vty, "%-9lu",
				adj->last_upd + adj->hold_time - now);
		else
			vty_out(vty, "-        ");
	...
	if (detail == ISIS_UI_LEVEL_DETAIL) {
		...
		if (adj->circuit
			...
			vty_out(vty, ", %s, DIS flaps: %u, Last: %s ago",
				isis_disflag2string(adj->dis_record
						    [ISIS_LEVELS + level -
						     1].dis),
