...
int isis_area_destroy(struct vty *vty, const char *area_tag)
{
	...
	if (area->circuit_list) {
		for (ALL_LIST_ELEMENTS
		     (area->circuit_list, node, nnode, circuit)) {
			circuit->ip_router = 0;
#ifdef HAVE_IPV6
	...
	if (isis->sysid_set == 0) {
		...
		for (ALL_LIST_ELEMENTS_RO(area->area_addrs, node, addrp)) {
			if ((addrp->addr_len + ISIS_SYS_ID_LEN +
			     ISIS_NSEL_LEN) != (addr->addr_len))
				continue;
			if (!memcmp
			    (addrp->area_addr, addr->area_addr,
			     addr->addr_len)) {
				XFREE(MTYPE_ISIS_AREA_ADDR, addr);
...
int clear_isis_neighbor_common(struct vty *vty, const char *id)
{
	...
	for (ALL_LIST_ELEMENTS_RO(isis->area_list, anode, area)) {
		for (ALL_LIST_ELEMENTS
		     (area->circuit_list, cnode, cnextnode, circuit)) {
			if (circuit->circ_type == CIRCUIT_T_BROADCAST) {
				for (i = 0; i < 2; i++) {
					adjdb = circuit->u.bc.adjdb[i];
					if (adjdb && adjdb->count) {
						for (ALL_LIST_ELEMENTS
						     (adjdb, node, nnode, adj))
							if (!id
							    ||
							    !memcmp(adj->sysid,
								    sysid,
...
DEFUN(debug_isis_events,
      ...
      "debug isis events", DEBUG_STR "IS-IS information\n" "IS-IS Events\n")
{
...
DEFUN(show_isis_summary,
      ...
      "show isis summary", SHOW_STR "IS-IS information\n" "IS-IS summary\n")
{
	...
	for (ALL_LIST_ELEMENTS_RO(isis->area_list, node, area)) {
		...
		if (listcount(area->area_addrs) > 0) {
			...
			for (ALL_LIST_ELEMENTS_RO
			     (area->area_addrs, node2, area_addr)) {
				vty_out(vty, "  Net: %s%s",
		...
		for (level = ISIS_LEVEL1; level <= ISIS_LEVELS; level++) {
			...
			if (spftree->pending)
				vty_out(vty, "    IPv4 SPF: (pending)%s",
					VTY_NEWLINE);
			else
			...
			if (spftree->pending)
				vty_out(vty, "    IPv6 SPF: (pending)%s",
					VTY_NEWLINE);
			else
...
DEFUN(router_isis,
      router_isis_cmd,
      "router isis WORD", ROUTER_STR "ISO IS-IS\n" "ISO Routing area tag")
{
...
DEFUN(no_domain_passwd,
      ...
      "no domain-password",
      NO_STR "Set the authentication password for a routing domain\n")
{
	struct isis_area *area;
...
DEFUN(no_spf_interval,
      no_spf_interval_cmd,
      "no spf-interval", NO_STR "Minimum interval between SPF calculations\n")
{
	struct isis_area *area;
...
DEFUN(no_max_lsp_lifetime_l1,
      ...
      NO_STR "LSP lifetime for Level 1 only in seconds\n")
{
	struct isis_area *area;
	uint16_t interval;
	int level;
...
DEFUN(no_max_lsp_lifetime_l2,
      ...
      NO_STR "LSP lifetime for Level 2 only in seconds\n")
{
	struct isis_area *area;
	uint16_t interval;
	int level;
...
DEFUN(no_lsp_refresh_interval,
      ...
      "no lsp-refresh-interval", NO_STR "LSP refresh interval in seconds\n")
{
	struct isis_area *area;
	uint16_t interval;
	int level;
...
DEFUN(no_lsp_refresh_interval_l1,
      ...
      NO_STR "LSP refresh interval for Level 1 only in seconds\n")
{
	struct isis_area *area;
	uint16_t interval;
	int level;
...
DEFUN(no_lsp_refresh_interval_l2,
      ...
      NO_STR "LSP refresh interval for Level 2 only in seconds\n")
{
	struct isis_area *area;
	uint16_t interval;
	int level;
      ...
      SHOW_STR "ISIS network information\n" "Show generated topologies\n")
{
	...
	for (ALL_LIST_ELEMENTS_RO(isis->area_list, node, area)) {
		...
		vty_out(vty, "Topology for isis area: %s%s", area->area_tag,
			VTY_NEWLINE);
		vty_out(vty, "From node     To node     Distance%s",
			VTY_NEWLINE);

		for (ALL_LIST_ELEMENTS_RO(area->topology, node2, arc))
			vty_out(vty, "%9ld %11ld %12ld%s", arc->from_node,
				arc->to_node, arc->distance, VTY_NEWLINE);
	}
	return CMD_SUCCESS;
}
	...
	if (isis != NULL) {
		...
		for (ALL_LIST_ELEMENTS_RO(isis->area_list, node, area)) {
...
#ifdef TOPOLOGY_GENERATE
			if (memcmp
			    (area->topology_baseis, DEFAULT_TOPOLOGY_BASEIS,
			     ISIS_SYS_ID_LEN)) {
			...
			if (strlen(area->top_params)) {
				vty_out(vty, " %s%s", area->top_params,
					VTY_NEWLINE);
				write++;
			}
#endif /* TOPOLOGY_GENERATE */
