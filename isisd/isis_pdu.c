			 ...
			 int level, struct isis_passwd *passwd)
{
	...
	retval =
	    parse_tlvs(area->area_tag, STREAM_PNT(stream) + ISIS_LSP_HDR_LEN,
...
tlvs_to_adj_area_addrs(struct tlvs *tlvs, struct isis_adjacency *adj)
{
	...
	if (tlvs->area_addrs) {
		for (ALL_LIST_ELEMENTS_RO(tlvs->area_addrs, node, area_addr)) {
			malloced =
			    XMALLOC(MTYPE_ISIS_TMP, sizeof(struct area_addr));
...
tlvs_to_adj_ipv6_addrs(struct tlvs *tlvs, struct isis_adjacency *adj)
{
	...
	if (tlvs->ipv6_addrs) {
		for (ALL_LIST_ELEMENTS_RO(tlvs->ipv6_addrs, node, ipv6_addr)) {
			malloced =
			    XMALLOC(MTYPE_ISIS_TMP, sizeof(struct in6_addr));
...
static int process_p2p_hello(struct isis_circuit *circuit)
{
	...
	if (!adj || adj->level != hdr->circuit_t) {
		if (!adj) {
			adj =
			    isis_new_adj(hdr->source_id, NULL, hdr->circuit_t,
					 circuit);
			if (adj == NULL)
				return ISIS_ERROR;
		} else {
		...
		if (circuit->area->is_type == IS_LEVEL_1_AND_2) {
			...
			case IS_LEVEL_1:
				if (adj->adj_state != ISIS_ADJ_UP) {
				    ...
				    if ((adj->adj_usage == ISIS_ADJ_LEVEL1AND2)
...
process_lan_hello(int level, struct isis_circuit *circuit, u_char * ssnpa)
{
	...
	if (hdr.circuit_t != IS_LEVEL_1 &&
	    ...
	    hdr.circuit_t != IS_LEVEL_1_AND_2 && (level & hdr.circuit_t) == 0) {
	...
	if ((adj == NULL) || (memcmp(adj->snpa, ssnpa, ETH_ALEN)) ||
		...
		if (!adj) {
			 ...
			 */
			adj =
			    isis_new_adj(hdr.source_id, ssnpa, level, circuit);
			if (adj == NULL) {
				retval = ISIS_ERROR;
				goto out;
			}
	...
	if (adj->dis_record[level - 1].dis == ISIS_IS_DIS)
		...
		case 1:
			if (memcmp
			    (circuit->u.bc.l1_desig_is, hdr.lan_id,
			     ISIS_SYS_ID_LEN + 1)) {
		...
		case 2:
			if (memcmp
			    (circuit->u.bc.l2_desig_is, hdr.lan_id,
			     ISIS_SYS_ID_LEN + 1)) {
				thread_add_event(master,
						 isis_event_dis_status_change,
						 circuit, 0);
 ...
 */
static int process_lsp(int level, struct isis_circuit *circuit, u_char * ssnpa)
{
	struct isis_link_state_hdr *hdr;
	...
	if (isis->debugs & DEBUG_UPDATE_PACKETS) {
		zlog_debug
		    ("ISIS-Upd (%s): Rcvd L%d LSP %s, seq 0x%08x, cksum 0x%04x, "
		     "lifetime %us, len %u, on %s", circuit->area->area_tag,
		     level, rawlspid_print(hdr->lsp_id), ntohl(hdr->seq_num),
	...
	if (circuit->circ_type == CIRCUIT_T_BROADCAST) {
		adj =
		    isis_adj_lookup_snpa(ssnpa, circuit->u.bc.adjdb[level - 1]);
		if (!adj) {
			zlog_debug
			    ("(%s): DS ======= LSP %s, seq 0x%08x, cksum 0x%04x, "
			     "lifetime %us on %s", circuit->area->area_tag,
		...
		if (!circuit->u.p2p.neighbor) {
			...
			if (((level == IS_LEVEL_1) &&
			     (circuit->u.p2p.neighbor->adj_usage ==
			      ISIS_ADJ_LEVEL2)) || ((level == IS_LEVEL_2)
						    && (circuit->u.
							p2p.neighbor->
							adj_usage ==
							ISIS_ADJ_LEVEL1)))
	    ...
	    u_char * ssnpa)
{
	...
	if (circuit->circ_type == CIRCUIT_T_BROADCAST) {
		if (snp_type == ISIS_SNP_CSNP_FLAG) {
			adj =
			    isis_adj_lookup(chdr->source_id,
					    circuit->u.bc.adjdb[level - 1]);
		} else {
			/* a psnp on a broadcast, how lovely of Juniper :) */
			adj =
			    isis_adj_lookup(phdr->source_id,
					    circuit->u.bc.adjdb[level - 1]);
		}
		if (!adj)
			return ISIS_OK;	/* Silently discard */
	...
	if (tlvs.lsp_entries) {
		for (ALL_LIST_ELEMENTS_RO(tlvs.lsp_entries, node, entry)) {
			...
			if (lsp) {
				...
				cmp =
				    lsp_compare(circuit->area->area_tag, lsp,
						entry->seq_num, entry->checksum,
						entry->rem_lifetime);
				...
				if (entry->rem_lifetime && entry->checksum
				    ...
				    && memcmp(entry->lsp_id, isis->sysid,
					      ISIS_SYS_ID_LEN)) {
					lsp =
					    lsp_new(entry->lsp_id,
						    ntohs(entry->rem_lifetime),
						    0, 0, entry->checksum,
						    level);
					lsp->area = circuit->area;
					lsp_insert(lsp,
						   circuit->area->lspdb[level -
									1]);
	...
	if (snp_type == ISIS_SNP_CSNP_FLAG) {
		...
		if (tlvs.lsp_entries) {
			for (ALL_LIST_ELEMENTS
			     (tlvs.lsp_entries, node, nnode, entry)) {
				for (ALL_LIST_ELEMENTS
				     (lsp_list, node2, nnode2, lsp)) {
					if (lsp_id_cmp
					    (lsp->lsp_header->lsp_id,
					     entry->lsp_id) == 0) {
...
static int process_csnp(int level, struct isis_circuit *circuit, u_char * ssnpa)
{
	if (isis->debugs & DEBUG_SNP_PACKETS) {
		zlog_debug
...
static int process_psnp(int level, struct isis_circuit *circuit, u_char * ssnpa)
{
	if (isis->debugs & DEBUG_SNP_PACKETS) {
		zlog_debug
 ...
 */
static int process_is_hello(struct isis_circuit *circuit)
{
	struct isis_adjacency *adj;
	int retval = ISIS_OK;
	u_char neigh_len;
	...
	if (isis->debugs & DEBUG_ADJ_PACKETS) {
		zlog_debug
		    ("ISIS-Adj (%s): Rcvd ISH on %s, cirType %s, cirID %u",
		     circuit->area->area_tag, circuit->interface->name,
		     circuit_t2string(circuit->is_type), circuit->circuit_id);
		if (isis->debugs & DEBUG_PACKET_DUMP)
			zlog_dump_data(STREAM_DATA(circuit->rcv_stream),
	...
	sysid =
	    STREAM_PNT(circuit->rcv_stream) + neigh_len - 1 - ISIS_SYS_ID_LEN;
	adj = circuit->u.p2p.neighbor;
	if (!adj) {
		/* 8.2.2 */
...
int send_hello(struct isis_circuit *circuit, int level)
{
	...
	if (circuit->passwd.type == ISIS_PASSWD_TYPE_HMAC_MD5) {
		hmac_md5(STREAM_DATA(circuit->snd_stream),
			 ...
			 circuit->passwd.len, (caddr_t) & hmac_md5_hash);
	...
	if (isis->debugs & DEBUG_ADJ_PACKETS) {
		if (circuit->circ_type == CIRCUIT_T_BROADCAST) {
			zlog_debug
			    ("ISIS-Adj (%s): Sent L%d LAN IIH on %s, length %ld",
			     circuit->area->area_tag, level,
			     circuit->interface->name,
			...
			zlog_debug
			    ("ISIS-Adj (%s): Sent P2P IIH on %s, length %ld",
			     circuit->area->area_tag, circuit->interface->name,
			     /* FIXME: use %z when we stop supporting old compilers. */
			     length);
		}
	   ...
	   struct isis_circuit *circuit)
{
	...
	if (CHECK_FLAG(passwd->snp_auth, SNP_AUTH_SEND)) {
		...
		case ISIS_PASSWD_TYPE_CLEARTXT:
			if (tlv_add_authinfo
			    (ISIS_PASSWD_TYPE_CLEARTXT, passwd->len,
		...
		case ISIS_PASSWD_TYPE_HMAC_MD5:
			...
			memset(&hmac_md5_hash, 0, ISIS_AUTH_MD5_SIZE);
			if (tlv_add_authinfo
			    (ISIS_PASSWD_TYPE_HMAC_MD5, ISIS_AUTH_MD5_SIZE,
			     hmac_md5_hash, circuit->snd_stream))
				return ISIS_WARNING;
			break;
...
max_lsps_per_snp(int snp_type, int level, struct isis_circuit *circuit)
{
	...
	lsp_count =
	    get_max_lsp_count(stream_get_size(circuit->snd_stream) -
...
int send_csnp(struct isis_circuit *circuit, int level)
{
	...
	while (loop) {
		...
		if (isis->debugs & DEBUG_SNP_PACKETS) {
			zlog_debug
			    ("ISIS-Snp (%s): Sent L%d CSNP on %s, length %ld",
			     circuit->area->area_tag, level,
			     circuit->interface->name,
			     stream_get_endp(circuit->snd_stream));
...
static int send_psnp(int level, struct isis_circuit *circuit)
{
	...
	while (1) {
		...
		if (isis->debugs & DEBUG_SNP_PACKETS) {
			zlog_debug
			    ("ISIS-Snp (%s): Sent L%d PSNP on %s, length %ld",
			     circuit->area->area_tag, level,
			     circuit->interface->name,
			     stream_get_endp(circuit->snd_stream));
			if (isis->debugs & DEBUG_PACKET_DUMP)
