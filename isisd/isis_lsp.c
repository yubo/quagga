...
static void lsp_destroy(struct isis_lsp *lsp)
{
	...
	for (ALL_LIST_ELEMENTS_RO(lsp->area->circuit_list, cnode, circuit)) {
		...
		for (ALL_LIST_ELEMENTS
		     (circuit->lsp_queue, lnode, lnnode, lsp_in_list))
			if (lsp_in_list == lsp)
...
static u_int8_t lsp_bits_generate(int level, int overload_bit)
{
	u_int8_t lsp_bits = 0;
	if (level == IS_LEVEL_1)
		lsp_bits = IS_LEVEL_1;
			 ...
			 u_int16_t checksum, int level)
{
	...
	(level == IS_LEVEL_1) ? fill_fixed_hdr(lsp->isis_header,
					       L1_LINK_STATE) :
...
void lsp_print_detail(struct isis_lsp *lsp, struct vty *vty, char dynhost)
{
	...
	if (lsp->tlv_data.area_addrs)
		for (ALL_LIST_ELEMENTS_RO
		     (lsp->tlv_data.area_addrs, lnode, area_addr)) {
	...
	if (lsp->tlv_data.auth_info.type != ISIS_PASSWD_TYPE_UNUSED) {
		if (lsp->tlv_data.auth_info.type == ISIS_PASSWD_TYPE_HMAC_MD5)
			...
			vty_out(vty, "  Auth type   : clear text%s",
				VTY_NEWLINE);
	}
	...
	if (lsp->tlv_data.ipv4_addrs)
		for (ALL_LIST_ELEMENTS_RO
		     (lsp->tlv_data.ipv4_addrs, lnode, ipv4_addr)) {
			memcpy(ipv4_address, inet_ntoa(*ipv4_addr),
			       sizeof(ipv4_address));
			vty_out(vty, "  IPv4 Address: %s%s", ipv4_address,
				VTY_NEWLINE);
		}
	...
	if (lsp->tlv_data.is_neighs)
		for (ALL_LIST_ELEMENTS_RO
		     (lsp->tlv_data.is_neighs, lnode, is_neigh)) {
	...
	if (lsp->tlv_data.ipv6_reachs)
		for (ALL_LIST_ELEMENTS_RO
		     (lsp->tlv_data.ipv6_reachs, lnode, ipv6_reach)) {
			...
			if ((ipv6_reach->control_info &&
				...
				vty_out(vty,
					...
					ipv6_reach->prefix_len, VTY_NEWLINE);
			else
				vty_out(vty,
	...
	if (lsp->tlv_data.te_is_neighs)
		for (ALL_LIST_ELEMENTS_RO
		     (lsp->tlv_data.te_is_neighs, lnode, te_is_neigh)) {
...
static void lsp_build(struct isis_lsp *lsp, struct isis_area *area)
{
	...
	if (area->ip_circuits > 0
		...
		if (area->ipv6_circuits > 0) {
			lsp->tlv_data.nlpids->count++;
			lsp->tlv_data.nlpids->nlpids[lsp->tlv_data.
						     nlpids->count - 1] =
			    NLPID_IPV6;
		}
#endif /* HAVE_IPV6 */
	...
	if (area->topology && level == IS_LEVEL_1) {
		...
		memcpy(&is_neigh->neigh_id, area->topology_baseis,
		       ISIS_SYS_ID_LEN);
	...
	for (ALL_LIST_ELEMENTS_RO(area->circuit_list, node, circuit)) {
		...
		if (circuit->ip_router && circuit->ip_addrs &&
			...
			if (area->oldmetric) {
				...
				for (ALL_LIST_ELEMENTS_RO
				     (circuit->ip_addrs, ipnode, ipv4)) {
					ipreach =
					    XMALLOC(MTYPE_ISIS_TLV,
						    sizeof(struct
							   ipv4_reachability));
					ipreach->metrics =
					    circuit->metrics[level - 1];
					masklen2ip(ipv4->prefixlen,
						   &ipreach->mask);
					ipreach->prefix.s_addr =
					    ((ipreach->mask.
					      s_addr) & (ipv4->prefix.s_addr));
				...
				for (ALL_LIST_ELEMENTS_RO
					...
					if (area->oldmetric)
						te_ipreach->te_metric =
						    htonl(circuit->metrics
							  [level -
							   1].metric_default);
					else
						te_ipreach->te_metric =
						    htonl(circuit->te_metric
							  [level - 1]);
		...
		if (circuit->ipv6_router && circuit->ipv6_non_link &&
			...
			for (ALL_LIST_ELEMENTS_RO
			     (circuit->ipv6_non_link, ipnode, ipv6)) {
				ip6reach =
		...
		case CIRCUIT_T_P2P:
			...
			if (nei && (level & nei->circuit_t)) {
				...
				if (area->newmetric) {
					...
					SET_TE_METRIC(te_is_neigh, metric);
					listnode_add(tlv_data.te_is_neighs,
						     te_is_neigh);
				}
			}
			break;
	...
	while (tlv_data.ipv4_int_reachs && listcount(tlv_data.ipv4_int_reachs)) {
		...
		if (tlv_data.ipv4_int_reachs
			...
			lsp =
			    lsp_next_frag(LSP_FRAGMENT(lsp->lsp_header->lsp_id)
					  + 1, lsp0, area, level);
	}
	...
	while (tlv_data.te_is_neighs && listcount(tlv_data.te_is_neighs)) {
		...
		lsp_tlv_fit(lsp, &tlv_data.te_is_neighs,
			    &lsp->tlv_data.te_is_neighs, IS_NEIGHBOURS_LEN,
 ...
 */
static void
lsp_build_pseudo(struct isis_lsp *lsp, struct isis_circuit *circuit, int level)
{
...
int lsp_generate_pseudo(struct isis_circuit *circuit, int level)
{
	...
	/* RFC3787  section 4 SHOULD not set overload bit in pseudo LSPs */
	lsp =
	    lsp_new(lsp_id, rem_lifetime, 1, circuit->area->is_type, 0, level);
	...
	for (level = 0; level < ISIS_LEVELS; level++) {
		if (area->lspdb[level] && dict_count(area->lspdb[level]) > 0) {
			...
			if (listcount(lsp_list) > 0) {
				for (ALL_LIST_ELEMENTS_RO
				     (area->circuit_list, cnode, circuit)) {
					int diff =
					    time(NULL) -
 ...
 */
void
lsp_purge_non_exist(struct isis_link_state_hdr *lsp_hdr, struct isis_area *area)
{
	struct isis_lsp *lsp;
	...
	fill_fixed_hdr(lsp->isis_header,
		       (lsp->level ==
			IS_LEVEL_1) ? L1_LINK_STATE : L2_LINK_STATE);
	lsp->lsp_header =
	    (struct isis_link_state_hdr *)(STREAM_DATA(lsp->pdu) +
					   ISIS_FIXED_HDR_LEN);
...
void generate_topology_lsps(struct isis_area *area)
{
	...
	for (i = 1; i < (max + 1); i++) {
		...
		lsp =
		    lsp_new(lspid, rem_lifetime, 1,
			    IS_LEVEL_1 | area->overload_bit, 0, 1);
		if (!lsp)
			return;
		lsp->area = area;
		lsp->from_topology = 1;
		...
		refresh_time = lsp_refresh_time(lsp, rem_lifetime);
		THREAD_TIMER_ON(master, lsp->t_lsp_top_ref, top_lsp_refresh,
				lsp, refresh_time);
		lsp_set_all_srmflags(lsp);
			...
			int lsp_top_num)
{
	...
	if (lsp->tlv_data.nlpids == NULL)
		lsp->tlv_data.nlpids =
		    XMALLOC(MTYPE_ISIS_TLV, sizeof(struct nlpids));
	...
	if (area->dynhostname) {
		...
		sprintf(buff, "%s%d",
			area->
			topology_basedynh ? area->topology_basedynh : "feedme",
			lsp_top_num);
	...
	for (ALL_LIST_ELEMENTS_RO(area->topology, node, arc)) {
		...
		if (area->oldmetric) {
			is_neigh =
			    XCALLOC(MTYPE_ISIS_TLV, sizeof(struct is_neigh));

			memcpy(&is_neigh->neigh_id, area->topology_baseis,
			       ISIS_SYS_ID_LEN);
			is_neigh->neigh_id[ISIS_SYS_ID_LEN - 1] =
			    (to_lsp & 0xFF);
			is_neigh->neigh_id[ISIS_SYS_ID_LEN - 2] =
			    ((to_lsp >> 8) & 0xFF);
		...
		if (area->newmetric) {
			...
			memcpy(&te_is_neigh->neigh_id, area->topology_baseis,
			       ISIS_SYS_ID_LEN);
			te_is_neigh->neigh_id[ISIS_SYS_ID_LEN - 1] =
			    (to_lsp & 0xFF);
			te_is_neigh->neigh_id[ISIS_SYS_ID_LEN - 2] =
			    ((to_lsp >> 8) & 0xFF);
