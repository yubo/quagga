	...
	for (ALL_LIST_ELEMENTS(area->ospf6->area_list, node, nnode, oa)) {
		...
		if (oa != area) {
			for (ro = ospf6_route_head(oa->range_table); ro;
				...
				if (CHECK_FLAG
				    (ro->flag, OSPF6_ROUTE_ACTIVE_SUMMARY))
					ospf6_abr_originate_summary_to_area(ro,
									    area);
			}
		}
	}
...
void ospf6_abr_disable_area(struct ospf6_area *area)
{
	...
	for (ro = ospf6_route_head(area->summary_prefix); ro;
	     ro = ospf6_route_next(ro)) {
		old =
		    ospf6_lsdb_lookup(ro->path.origin.type, ro->path.origin.id,
				    ...
				    struct ospf6_area *area)
{
	...
	if (route->type != OSPF6_DEST_TYPE_NETWORK &&
		...
		if (is_debug)
			zlog_debug
			    ("Route type is none of network, range nor ASBR, withdraw");
		if (summary)
			ospf6_route_remove(summary, summary_table);
		if (old)
			ospf6_lsa_purge(old);
...
void ospf6_abr_originate_summary(struct ospf6_route *route)
{
	...
	if (route->type == OSPF6_DEST_TYPE_NETWORK) {
		...
		range =
		    ospf6_route_lookup_bestmatch(&route->prefix,
						 oa->range_table);
		if (range)
...
void ospf6_abr_examin_brouter(u_int32_t router_id)
{
	...
	for (ALL_LIST_ELEMENTS(ospf6->area_list, node, nnode, oa)) {
		type = htons(OSPF6_LSTYPE_INTER_ROUTER);
		for (lsa =
		     ospf6_lsdb_type_router_head(type, router_id, oa->lsdb);
		     lsa;
