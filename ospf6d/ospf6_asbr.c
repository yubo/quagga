...
static void ospf6_asbr_redistribute_unset(int type)
{
	...
	for (route = ospf6_route_head(ospf6->external_table); route;
		...
		ospf6_asbr_redistribute_remove(info->type,
			    ...
			    u_int nexthop_num, struct in6_addr *nexthop)
{
	...
	if (match) {
		...
		if (nexthop_num && nexthop)
			memcpy(&match->nexthop[0].address, nexthop,
			       sizeof(struct in6_addr));
	...
	if (nexthop_num && nexthop)
		memcpy(&route->nexthop[0].address, nexthop,
		       sizeof(struct in6_addr));
...
static void *ospf6_routemap_rule_match_address_prefixlist_compile(const char
								  *arg)
{
...
DEFUN(ospf6_routemap_no_match_interface,
      ...
      "no match interface",
      MATCH_STR NO_STR "Match first hop interface of route\n")
{
	int ret = route_map_delete_match((struct route_map_index *)vty->index,
					 "interface",
					 (argc == 0) ? NULL : argv[0]);
...
ALIAS(ospf6_routemap_no_match_interface,
      ...
      "no match interface WORD",
      MATCH_STR
      NO_STR "Match first hop interface of route\n" "Interface name\n")
...
static int ospf6_as_external_lsa_show(struct vty *vty, struct ospf6_lsa *lsa)
{
	...
	snprintf(buf, sizeof(buf), "%c%c%c",
		 (CHECK_FLAG(external->bits_metric, OSPF6_ASBR_BIT_E) ? 'E' :
		  '-'), (CHECK_FLAG(external->bits_metric,
				    OSPF6_ASBR_BIT_F) ? 'F' : '-'),
		 (CHECK_FLAG(external->bits_metric, OSPF6_ASBR_BIT_T) ? 'T' :
		  '-'));

	vty_out(vty, "     Bits: %s%s", buf, VNL);
	vty_out(vty, "     Metric: %5lu%s",
		(u_long) OSPF6_ASBR_METRIC(external), VNL);
