...
DEFUN(ospf6_router_id,
      ospf6_router_id_cmd,
      "router-id A.B.C.D", "Configure OSPF Router-ID\n" V4NOTATION_STR)
{
	int ret;
	u_int32_t router_id;
	struct ospf6 *o;
...
static void ospf6_show(struct vty *vty, struct ospf6 *o)
{
	...
	vty_out(vty, " Initial SPF scheduling delay %d millisec(s)%s"
		...
		o->spf_delay, VNL,
		o->spf_holdtime, VNL,
		o->spf_max_holdtime, VNL, o->spf_hold_multiplier, VNL);
	...
	if (o->ts_spf.tv_sec || o->ts_spf.tv_usec) {
		...
		vty_out(vty, " Last SPF duration %ld sec %ld usec%s",
			o->ts_spf_duration.tv_sec, o->ts_spf_duration.tv_usec,
			VNL);
	} else
...
DEFUN(show_ipv6_ospf6,
      show_ipv6_ospf6_cmd, "show ipv6 ospf6", SHOW_STR IP6_STR OSPF6_STR)
{
...
DEFUN(show_ipv6_ospf6_route,
      ...
      "show ipv6 ospf6 route", SHOW_STR IP6_STR OSPF6_STR ROUTE_STR)
{
