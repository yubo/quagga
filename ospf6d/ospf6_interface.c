...
static u_int32_t ospf6_interface_get_cost(struct ospf6_interface *oi)
{
	...
	bw = oi->interface->bandwidth ? oi->
	    interface->bandwidth : OSPF6_INTERFACE_BANDWIDTH;
...
static struct in6_addr *ospf6_interface_get_linklocal_address(struct interface
							      *ifp)
{
	struct listnode *n;
	struct connected *c;
...
static int ospf6_interface_show(struct vty *vty, struct interface *ifp)
{
	...
	vty_out(vty, "%s is %s, type %s%s",
		ifp->name, updown[if_is_operative(ifp)], type, VNL);
	...
	if (oi->area) {
		...
		vty_out(vty, "  Area ID %s, Cost %hu%s", strbuf, oi->cost, VNL);
	} else
...
DEFUN(show_ipv6_ospf6_interface,
      ...
      SHOW_STR IP6_STR OSPF6_STR INTERFACE_STR IFNAME_STR)
{
	struct interface *ifp;
	struct listnode *i;
...
ALIAS(show_ipv6_ospf6_interface,
      ...
      "show ipv6 ospf6 interface", SHOW_STR IP6_STR OSPF6_STR INTERFACE_STR)
...
ALIAS(show_ipv6_ospf6_interface_ifname_prefix,
    ...
    ALIAS(show_ipv6_ospf6_interface_ifname_prefix,
      ...
      OSPF6_ROUTE_MATCH_STR "Display details of the prefixes\n")
...
DEFUN(ipv6_ospf6_hellointerval,
      ...
      IP6_STR OSPF6_STR "Interval time of Hello packets\n" SECONDS_STR)
{
	struct ospf6_interface *oi;
	struct interface *ifp;
...
DEFUN(ipv6_ospf6_transmitdelay,
      ...
      IP6_STR OSPF6_STR "Transmit delay of this interface\n" SECONDS_STR)
{
	struct ospf6_interface *oi;
	struct interface *ifp;
...
DEFUN(no_ipv6_ospf6_passive,
      ...
      NO_STR
      IP6_STR
      OSPF6_STR "passive interface: No Adjacency will be formed on this I/F\n")
{
	struct ospf6_interface *oi;
	struct interface *ifp;
...
DEFUN(ipv6_ospf6_mtu_ignore,
      ...
      IP6_STR OSPF6_STR "Ignore MTU mismatch on this interface\n")
{
	struct ospf6_interface *oi;
	struct interface *ifp;
...
DEFUN(no_ipv6_ospf6_mtu_ignore,
      ...
      NO_STR IP6_STR OSPF6_STR "Ignore MTU mismatch on this interface\n")
{
	struct ospf6_interface *oi;
	struct interface *ifp;
...
static int config_write_ospf6_interface(struct vty *vty)
{
	...
	for (ALL_LIST_ELEMENTS_RO(iflist, i, ifp)) {
		...
		if (oi->type == OSPF_IFTYPE_POINTOPOINT)
			vty_out(vty, " ipv6 ospf6 network point-to-point%s",
				VNL);
...
DEFUN(debug_ospf6_interface,
      ...
      "debug ospf6 interface", DEBUG_STR OSPF6_STR "Debug OSPFv3 Interface\n")
{
