...
static int
ospf6_zebra_read_ipv6(int command, struct zclient *zclient, zebra_size_t length)
{
	struct stream *s;
	...
	if (IS_OSPF6_DEBUG_ZEBRA(RECV)) {
		...
		zlog_debug
		    ("Zebra Receive route %s: %s %s nexthop %s ifindex %ld",
	...
	if (command == ZEBRA_IPV6_ROUTE_ADD)
		ospf6_asbr_redistribute_add(api.type, ifindex,
					    (struct prefix *)&p,
	...
	for (i = 0; i < nhcount; i++) {
		if (IS_OSPF6_DEBUG_ZEBRA(SEND)) {
			...
			inet_ntop(AF_INET6, &request->nexthop[i].address,
				  buf, sizeof(buf));
			if (!if_indextoname
			    (request->nexthop[i].ifindex, ifname))
				strlcpy(ifname, "unknown", sizeof(ifname));
			zlog_debug("  nexthop: %s%%%.*s(%d)", buf, IFNAMSIZ,
				   ifname, request->nexthop[i].ifindex);
		}
