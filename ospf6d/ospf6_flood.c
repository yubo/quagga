...
void
ospf6_lsa_originate_interface(struct ospf6_lsa *lsa, struct ospf6_interface *oi)
{
	lsa->lsdb = oi->lsdb;
	ospf6_lsa_originate(lsa);
}
		      ...
		      struct ospf6_lsa *lsa, struct ospf6_interface *oi)
{
	...
	for (ALL_LIST_ELEMENTS(oi->neighbor_list, node, nnode, on)) {
		...
		if (on->state != OSPF6_NEIGHBOR_FULL) {
			...
			req =
			    ospf6_lsdb_lookup(lsa->header->type,
					      lsa->header->id,
			       ...
			       struct ospf6_neighbor *from)
{
	...
	if (ismore_recent < 0) {
		...
		if (oi->thread_send_lsack == NULL)
			oi->thread_send_lsack =
			    thread_add_timer(master, ospf6_lsack_send_interface,
					     oi, 3);
		return;
	}
	...
	if (old == NULL || ismore_recent < 0) {
		...
		if (old) {
			...
			if (res.tv_sec < OSPF_MIN_LS_ARRIVAL) {
				if (is_debug)
					zlog_debug
					    ("LSA can't be updated within MinLSArrival, discard");
				ospf6_lsa_delete(new);
