		   ...
		   caddr_t lsdesc)
{
	...
	for (lsa = ospf6_lsdb_type_router_head(type, adv_router, oi->lsdb); lsa;
		...
		link_lsa =
		    (struct ospf6_link_lsa *)OSPF6_LSA_HEADER_END(lsa->header);
		if (IS_OSPF6_DEBUG_SPF(PROCESS)) {
	...
	if (route && route->path.cost < v->cost) {
		...
		for (i = 0; ospf6_nexthop_is_set(&v->nexthop[i]) &&
			...
			for (j = 0; j < OSPF6_MULTI_PATH_LIMIT; j++) {
				...
				ospf6_nexthop_copy(&route->nexthop[j],
						   &v->nexthop[i]);
				break;
			}
		}
		      ...
		      struct ospf6_area *oa)
{
	...
	while (candidate_list->size) {
		...
		for (lsdesc = OSPF6_LSA_HEADER_END(v->lsa->header) + 4;
			...
			if (VERTEX_IS_TYPE(ROUTER, v)) {
				w->cost =
				    v->cost + ROUTER_LSDESC_GET_METRIC(lsdesc);
				w->hops =
				    v->hops +
			...
			if (w->hops == 0)
				...
				for (i = 0; ospf6_nexthop_is_set(&v->nexthop[i])
				     && i < OSPF6_MULTI_PATH_LIMIT; i++)
					ospf6_nexthop_copy(&w->nexthop[i],
							   &v->nexthop[i]);
			}
...
static int ospf6_spf_calculation_thread(struct thread *t)
{
	...
	zlog_info("SPF processing: # Areas: %d, SPF runtime: %ld sec %ld usec, "
		  "Reason: %s\n", areas_processed, runtime.tv_sec,
		  runtime.tv_usec, rbuf);
