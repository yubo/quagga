...
array_size(ospf6_message_type_str);
...
ospf6_dbdesc_recv_master(struct ospf6_header *oh, struct ospf6_neighbor *on)
{
	...
	case OSPF6_NEIGHBOR_EXCHANGE:
		if (!memcmp
		    (dbdesc, &on->dbdesc_last, sizeof(struct ospf6_dbdesc))) {
    ...
    )
{
	...
	while (length) {
		...
		current =
		    (struct ospf6_prefix *)((caddr_t) current +
					    requested_pfx_bytes);
		real_num_pfxs++;
	}
...
static unsigned
ospf6_lsa_examin(struct ospf6_lsa_header *lsah, const u_int16_t lsalen,
		 const u_char headeronly)
{
	...
	case OSPF6_LSTYPE_LINK:
		...
		if (headeronly)
			break;
		link_lsa =
		    (struct ospf6_link_lsa *)((caddr_t) lsah +
					      OSPF6_LSA_HEADER_SIZE);
				    ...
				    const u_int32_t declared_num_lsas)
{
	...
	while (length) {
		...
		if (headeronly) {
			...
			lsah =
			    (struct ospf6_lsa_header *)((caddr_t) lsah +
							OSPF6_LSA_HEADER_SIZE);
			...
			if (MSG_OK != ospf6_lsa_examin(lsah, lsalen, 0)) {
				if (IS_OSPF6_DEBUG_MESSAGE
				    (OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
					zlog_debug("%s: anomaly in %s LSA #%u",
						   __func__,
						   ospf6_lstype_name
						   (lsah->type), counted_lsas);
				return MSG_NG;
			}
			lsah =
			    (struct ospf6_lsa_header *)((caddr_t) lsah +
...
ospf6_packet_examin(struct ospf6_header *oh, const unsigned bytesonwire)
{
	...
	if (oh->type < OSPF6_MESSAGE_TYPE_ALL &&
		...
		if (IS_OSPF6_DEBUG_MESSAGE(OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
			zlog_debug("%s: undersized (%u B) %s packet", __func__,
				   bytesonwire, LOOKUP(ospf6_message_type_str,
						       oh->type));
		return MSG_NG;
	}
	...
	case OSPF6_MESSAGE_TYPE_HELLO:
		...
		if (0 ==
		    (bytesonwire - OSPF6_HEADER_SIZE -
		     OSPF6_HELLO_MIN_SIZE) % 4)
			return MSG_OK;
		if (IS_OSPF6_DEBUG_MESSAGE(OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
			zlog_debug("%s: alignment error in %s packet",
				   __func__, LOOKUP(ospf6_message_type_str,
						    oh->type));
		return MSG_NG;
	...
	case OSPF6_MESSAGE_TYPE_LSREQ:
		...
		if (0 ==
		    (bytesonwire - OSPF6_HEADER_SIZE -
		     OSPF6_LS_REQ_MIN_SIZE) % OSPF6_LSREQ_LSDESC_FIX_SIZE)
			return MSG_OK;
		if (IS_OSPF6_DEBUG_MESSAGE(OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
			zlog_debug("%s: alignment error in %s packet",
				   __func__, LOOKUP(ospf6_message_type_str,
	...
	case OSPF6_MESSAGE_TYPE_LSUPDATE:
		...
		lsupd =
		    (struct ospf6_lsupdate *)((caddr_t) oh + OSPF6_HEADER_SIZE);
	...
	case OSPF6_MESSAGE_TYPE_LSACK:
		...
		test = ospf6_lsaseq_examin
		    ((struct ospf6_lsa_header *)((caddr_t) oh +
						 OSPF6_HEADER_SIZE +
						 OSPF6_LS_ACK_MIN_SIZE),
		     bytesonwire - OSPF6_HEADER_SIZE - OSPF6_LS_ACK_MIN_SIZE, 1,
		     0);
		break;
	default:
		if (IS_OSPF6_DEBUG_MESSAGE(OSPF6_MESSAGE_TYPE_UNKNOWN, RECV))
	...
	if (test != MSG_OK
		...
		zlog_debug("%s: anomaly in %s packet", __func__,
			   LOOKUP(ospf6_message_type_str, oh->type));
	return test;
}
...
int ospf6_lsupdate_send_neighbor(struct thread *thread)
{
	...
	if (on->lsupdate_list->count != 0)
		on->thread_send_lsupdate =
		    thread_add_event(master, ospf6_lsupdate_send_neighbor, on,
				     0);
...
int ospf6_lsupdate_send_interface(struct thread *thread)
{
	...
	if (oi->lsupdate_list->count > 0) {
		oi->thread_send_lsupdate =
		    thread_add_event(master, ospf6_lsupdate_send_interface, oi,
				     0);
	}
	...
	for (lsa = ospf6_lsdb_head(on->lsack_list); lsa;
		...
		if (p - sendbuf + sizeof(struct ospf6_lsa_header) >
			...
			on->thread_send_lsack =
			    thread_add_event(master, ospf6_lsack_send_neighbor,
					     on, 0);
	...
	for (lsa = ospf6_lsdb_head(oi->lsack_list); lsa;
		...
		if (p - sendbuf + sizeof(struct ospf6_lsa_header) >
			...
			oi->thread_send_lsack =
			    thread_add_event(master, ospf6_lsack_send_interface,
					     oi, 0);
...
int config_write_ospf6_debug_message(struct vty *vty)
{
	...
	for (i = 1; i < 6; i++) {
		if (IS_OSPF6_DEBUG_MESSAGE(i, SEND) &&
			...
			vty_out(vty, "debug ospf6 message %s%s", type_str[i],
				VNL);
