	...
	case OSPF6_SCOPE_TYPE_AREA:
		{
			...
			case OSPF6_TABLE_TYPE_SUMMARY_PREFIXES:
				snprintf(name, sizeof(name),
					 "area %s summary prefix table",
					 oa->name);
				break;
			case OSPF6_TABLE_TYPE_SUMMARY_ROUTERS:
				snprintf(name, sizeof(name),
					 "area %s summary router table",
					 oa->name);
				break;
			default:
				snprintf(name, sizeof(name),
...
struct ospf6_route *ospf6_route_lookup_identical(struct ospf6_route *route, struct ospf6_route_table
						 *table)
{
...
struct ospf6_route *ospf6_route_lookup_bestmatch(struct prefix *prefix, struct ospf6_route_table
						 *table)
{
	struct route_node *node;
	struct ospf6_route *route;
		...
		if (IS_OSPF6_DEBUG_ROUTE(MEMORY))
			zlog_debug
			    ("%s %p: route add %p: another path: prev %p, next %p",
			     ospf6_route_table_name(table), table, route, prev,
			     next);
		else if (IS_OSPF6_DEBUG_ROUTE(TABLE))
					   ...
					   struct ospf6_route_table *table)
{
	...
	while (node && node->p.prefixlen < prefix->prefixlen &&
		...
		node =
		    node->link[prefix_bit
			       (&prefix->u.prefix, node->p.prefixlen)];
...
void ospf6_route_show(struct vty *vty, struct ospf6_route *route)
{
	...
	if (!if_indextoname(route->nexthop[0].ifindex, ifname))
		snprintf(ifname, sizeof(ifname), "%d",
...
static void
ospf6_route_show_table_summary(struct vty *vty, struct ospf6_route_table *table)
{
