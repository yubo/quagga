...
static int bgp_info_mpath_cmp(void *val1, void *val2)
{
	...
	if (!compare)
		compare =
		    sockunion_cmp(bi1->peer->su_remote, bi2->peer->su_remote);
		      ...
		      struct bgp_maxpaths_cfg *mpath_cfg)
{
	...
	while (mp_node || cur_mpath) {
				...
				if (debug)
					zlog_debug
					    ("%s remove mpath nexthop %s peer %s",
					     pfx_buf, inet_ntop(AF_INET,
								&cur_mpath->attr->
								nexthop,
								nh_buf[0],
								sizeof(nh_buf
								       [0])),
					     sockunion2str(cur_mpath->
							   peer->su_remote,
							   nh_buf[1],
							   sizeof(nh_buf[1])));
			}
		...
		if (cur_mpath && (!mp_node ||
			...
			mpath_changed = 1;
			if (debug)
				zlog_debug("%s remove mpath nexthop %s peer %s",
					   pfx_buf, inet_ntop(AF_INET,
							      &cur_mpath->
							      attr->nexthop,
							      nh_buf[0],
	...
	if (!CHECK_FLAG(new_best->flags, BGP_INFO_MULTIPATH_CHG) &&
		...
		if (CHECK_FLAG(new_best->flags, BGP_INFO_ATTR_CHANGED))
			...
			for (mpinfo = bgp_info_mpath_first(new_best); mpinfo;
				...
				if (CHECK_FLAG
				    (mpinfo->flags, BGP_INFO_ATTR_CHANGED)) {
					attr_chg = 1;
					break;
				}
			}
	...
	for (mpinfo = bgp_info_mpath_first(new_best); mpinfo;
		...
		if (mpinfo->attr->community) {
			if (community) {
				...
				community =
				    community_dup(mpinfo->attr->community);
		}
		...
		if (ae && ae->ecommunity) {
			if (ecomm) {
				ecommerge =
				    ecommunity_merge(ecomm, ae->ecommunity);
