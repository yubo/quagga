...
int attrhash_cmp(const void *p1, const void *p2)
{
	...
	if (attr1->flag == attr2->flag
		...
		if (ae1 && ae2
		    ...
		    && ae1->aggregator_addr.s_addr ==
		    ae2->aggregator_addr.s_addr && ae1->weight == ae2->weight
#ifdef HAVE_IPV6
...
{
	hash_iterate(attrhash, (void (*)(struct hash_backet *, void *))
		     attr_show_all_iterator, vty);
}
	...
	if (attr->extra) {
		...
		if (attre->ecommunity) {
			if (!attre->ecommunity->refcnt)
				attre->ecommunity =
				    ecommunity_intern(attre->ecommunity);
			else
...
static void bgp_attr_flags_diagnose(struct bgp_attr_parser_args *args, u_int8_t desired_flags	/* how RFC says it must be */
    )
{
	u_char seen = 0, i;
	...
	for (i = 0; i <= 2; i++)	/* O,T,P, but not E */
		if (CHECK_FLAG(desired_flags, attr_flag_str[i].key) !=
			...
			zlog(args->peer->log, LOG_ERR,
			     ...
			     CHECK_FLAG(desired_flags,
					attr_flag_str[i].key) ? "" : " not",
			     attr_flag_str[i].str);
			seen = 1;
		}
	if (!seen) {
...
bgp_attr_aspath_check(struct peer *const peer, struct attr *const attr)
{
	...
	if ((peer->sort == BGP_PEER_CONFED
		...
		zlog(peer->log, LOG_ERR, "Malformed AS path from %s",
		     peer->host);
	...
	if (bgp != NULL && bgp_flag_check(bgp, BGP_FLAG_ENFORCE_FIRST_AS)) {
		if (peer->sort == BGP_PEER_EBGP
			...
			zlog(peer->log, LOG_ERR,
			     "%s incorrect first AS (must be %u)", peer->host,
			     peer->as);
			bgp_notify_send(peer, BGP_NOTIFY_UPDATE_ERR,
					BGP_NOTIFY_UPDATE_MAL_AS_PATH);
			return BGP_ATTR_PARSE_ERROR;
	...
	*as4_path = aspath_parse(peer->ibuf, length, 1);
	...
	if (!*as4_path) {
		zlog(peer->log, LOG_ERR,
		     ...
		     peer->host, length);
		return bgp_attr_malformed(args,
					  BGP_NOTIFY_UPDATE_MAL_AS_PATH, 0);
	}
			...
			struct in_addr *as4_aggregator_addr)
{
	...
	if (length != 8) {
		zlog(peer->log, LOG_ERR, "New Aggregator length is not 8 [%d]",
		     length);
		return bgp_attr_malformed(args,
					  BGP_NOTIFY_UPDATE_ATTR_LENG_ERR, 0);
	}
			...
			if (BGP_DEBUG(as4, AS4))
				zlog_debug
				    ("[AS4] %s BGP not AS4 capable peer send"
				     ...
				     peer->host);
			(attre = bgp_attr_extra_get(attr))->aggregator_as =
			    as4_aggregator;
			...
			if (BGP_DEBUG(update, UPDATE_IN))
				zlog_debug
				    ("%s got two nexthop %s %s but second one is not a link-local nexthop",
				     peer->host, inet_ntop(AF_INET6,
							   &attre->mp_nexthop_global,
							   buf1,
							   INET6_ADDRSTRLEN),
				     inet_ntop(AF_INET6,
					       &attre->mp_nexthop_local, buf2,
					       INET6_ADDRSTRLEN));
...
#define BGP_MP_UNREACH_MIN_SIZE 3
	...
	if (safi != SAFI_MPLS_LABELED_VPN) {
		ret =
		    bgp_nlri_sanity_check(peer, afi, stream_pnt(s),
					  withdraw_len);
		if (ret < 0)
			return BGP_ATTR_PARSE_ERROR_NOTIFYPLS;
	}
...
static bgp_attr_parse_ret_t bgp_attr_unknown(struct bgp_attr_parser_args *args)
{
	...
	if (BGP_DEBUG(events, EVENTS))
		zlog(peer->log, LOG_DEBUG,
		     "Unknown attribute type %d length %d is received", type,
		     length);
		...
		case SAFI_MULTICAST:
			{
				...
				if (attre->mp_nexthop_len == 32)
					stream_put(s, &attre->mp_nexthop_local,
						   16);
			}
		default:
			break;
		     ...
		     struct peer *from, struct prefix_rd *prd, u_char * tag)
{
	...
	if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_SEND_EXT_COMMUNITY)
		...
		if (peer->sort == BGP_PEER_IBGP
			...
			stream_put(s, attre->ecommunity->val,
				   attre->ecommunity->size * 8);
		} else {
			u_int8_t *pnt;
			int tbit;
			int ecom_tr_size = 0;
			...
			for (i = 0; i < attre->ecommunity->size; i++) {
				...
				if (CHECK_FLAG
				    (tbit, ECOMMUNITY_FLAG_NON_TRANSITIVE))
					continue;
	...
	/* Unknown transit attribute. */
	if (attr->extra && attr->extra->transit)
		stream_put(s, attr->extra->transit->val,
			   attr->extra->transit->length);
...
/* Make attribute packet. */
void
bgp_dump_routes_attr(struct stream *s, struct attr *attr, struct prefix *prefix)
{
	unsigned long cp;
	unsigned long len;
	size_t aspath_lenp;
