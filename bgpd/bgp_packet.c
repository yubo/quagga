...
static void bgp_connect_check(struct peer *peer)
{
	...
	slen = sizeof(status);
	ret =
	    getsockopt(peer->fd, SOL_SOCKET, SO_ERROR, (void *)&status, &slen);
...
static struct stream *bgp_update_packet(struct peer *peer, afi_t afi,
					safi_t safi)
{
	struct stream *s;
	struct stream *snlri;
	...
	while (adv) {
		...
		if (afi == AFI_IP && safi == SAFI_UNICAST)
			...
			if (stream_empty(snlri))
				mpattrlen_pos =
				    bgp_packet_mpattr_start(snlri, afi, safi,
							    adv->baa->attr);
					  ...
					  safi_t safi)
{
	...
	while ((adv =
		...
		if (afi == AFI_IP && safi == SAFI_UNICAST)
			...
			if (first_time) {
				...
				mplen_pos =
				    bgp_packet_mpunreach_start(s, afi, safi);
			}
		...
		if (BGP_DEBUG(update, UPDATE_OUT)) {
			...
			zlog(peer->log, LOG_DEBUG,
			     "%s send UPDATE %s/%d -- unreachable", peer->host,
			     inet_ntop(rn->p.family, &(rn->p.u.prefix), buf,
				       INET6_BUFSIZ), rn->p.prefixlen);
		}
	...
	if (!stream_empty(s)) {
		if (afi == AFI_IP && safi == SAFI_UNICAST) {
			unfeasible_len
			    =
			    stream_get_endp(s) - BGP_HEADER_SIZE -
			    BGP_UNFEASIBLE_LEN;
			...
			afi_t afi, safi_t safi, struct peer *from)
{
	...
	if (BGP_DEBUG(update, UPDATE_OUT)) {
		...
		zlog(peer->log, LOG_DEBUG, "%s send UPDATE %s/%d %s",
		     peer->host, inet_ntop(p.family, &(p.u.prefix), buf,
					   INET6_BUFSIZ), p.prefixlen, attrstr);
	}
	...
	stream_putw(s, 0);
	total_attr_len =
	    bgp_packet_attribute(NULL, peer, s, attr, &p, afi, safi, from, NULL,
				 NULL);
...
static struct stream *bgp_write_packet(struct peer *peer)
{
	...
	for (afi = AFI_IP; afi < AFI_MAX; afi++)
		for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++) {
			adv =
			    BGP_ADV_FIFO_HEAD(&peer->sync[afi][safi]->withdraw);
			if (adv) {
	...
	for (afi = AFI_IP; afi < AFI_MAX; afi++)
		for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++) {
			...
			if (adv) {
				if (adv->binfo
				    && adv->binfo->uptime < peer->synctime) {
					if (CHECK_FLAG
					    (adv->binfo->peer->cap,
					    ...
					    !(CHECK_FLAG
					      ...
					      && CHECK_FLAG(adv->binfo->
							    peer->cap,
							    PEER_CAP_RESTART_BIT_ADV))
					    && !CHECK_FLAG(adv->binfo->flags,
							   BGP_INFO_STALE)
					    && safi != SAFI_MPLS_VPN) {
						if (CHECK_FLAG
						    (adv->binfo->
...
static int bgp_write_proceed(struct peer *peer)
{
	...
	for (afi = AFI_IP; afi < AFI_MAX; afi++)
		for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
			if ((adv =
			     BGP_ADV_FIFO_HEAD(&peer->
					       sync[afi][safi]->update)) !=
			    NULL)
			  ...
			  u_char * data, size_t datalen)
{
	...
	if (sub_code != BGP_NOTIFY_CEASE_CONFIG_CHANGE) {
		if (sub_code == BGP_NOTIFY_CEASE_ADMIN_RESET) {
			...
			zlog_info("Notification sent to neighbor %s: shutdown",
				  peer->host);
		} else {
	...
	if (BGP_DEBUG(normal, NORMAL)) {
		if (!orf_refresh)
			zlog_debug("%s sending REFRESH_REQ for afi/safi: %d/%d",
				   peer->host, afi, safi);
		zlog_debug("%s send message type %d, length (incl. header) %d",
			   peer->host, CHECK_FLAG(peer->cap,
						  PEER_CAP_REFRESH_NEW_RCV) ?
			   BGP_MSG_ROUTE_REFRESH_NEW :
			   BGP_MSG_ROUTE_REFRESH_OLD, length);
	}
		...
		if (BGP_DEBUG(normal, NORMAL))
			zlog_debug
			    ("%s sending CAPABILITY has %s MP_EXT CAP for afi/safi: %d/%d",
			     peer->host,
			     action ==
			     CAPABILITY_ACTION_SET ? "Advertising" : "Removing",
			     afi, safi);
	}
...
static int bgp_open_receive(struct peer *peer, bgp_size_t size)
{
	...
	if (CHECK_FLAG(peer->sflags, PEER_STATUS_ACCEPT_PEER)) {
		...
		realpeer =
		    peer_lookup_with_open(&peer->su, remote_as, &remote_id,
					  &as);
		...
		if (BGP_DEBUG(events, EVENTS))
			zlog_debug
			    ("%s [Event] Transfer accept BGP peer to real (state %s)",
			     peer->host, LOOKUP(bgp_status_msg,
						realpeer->status));
	...
	if (optlen != 0) {
		if ((ret =
		     bgp_open_option_parse(peer, optlen, &mp_capability)) < 0) {
			bgp_notify_send(peer, BGP_NOTIFY_OPEN_ERR,
					BGP_NOTIFY_OPEN_UNACEP_HOLDTIME);
			return ret;
		}
...
static int bgp_update_receive(struct peer *peer, bgp_size_t size)
{
	...
	if (withdraw_len > 0) {
		ret =
		    bgp_nlri_sanity_check(peer, AFI_IP, stream_pnt(s),
					  withdraw_len);
		if (ret < 0)
			return -1;

		if (BGP_DEBUG(packet, PACKET_RECV))
			zlog_debug("%s [Update:RECV] Unfeasible NLRI received",
				   peer->host);
	...
	if (peer->afc[AFI_IP][SAFI_UNICAST]) {
		...
		if (!attribute_len && !withdraw_len) {
			...
			if (peer->nsf[AFI_IP][SAFI_UNICAST])
				bgp_clear_stale_route(peer, AFI_IP,
						      SAFI_UNICAST);
	...
	if (peer->afc[AFI_IP6][SAFI_UNICAST]) {
		...
		if (!withdraw_len
			...
			if (peer->nsf[AFI_IP6][SAFI_UNICAST])
				bgp_clear_stale_route(peer, AFI_IP6,
						      SAFI_UNICAST);
...
bgp_capability_msg_parse(struct peer *peer, u_char * pnt, bgp_size_t length)
{
	...
	while (pnt < end) {
		...
		if (hdr->code == CAPABILITY_CODE_MP) {
			...
			if (CHECK_FLAG
			    (peer->flags, PEER_FLAG_OVERRIDE_CAPABILITY))
				continue;
			...
			if (BGP_DEBUG(normal, NORMAL))
				zlog_debug
				    ("%s CAPABILITY has %s MP_EXT CAP for afi/safi: %u/%u",
				     peer->host,
				     action ==
				     CAPABILITY_ACTION_SET ? "Advertising" :
				     "Removing", ntohs(mpc.afi), mpc.safi);
...
int bgp_read(struct thread *thread)
{
	...
	if (CHECK_FLAG(peer->sflags, PEER_STATUS_ACCEPT_PEER)) {
		if (BGP_DEBUG(events, EVENTS))
			zlog_debug("%s [Event] Accepting BGP peer delete",
				   peer->host);
		peer_delete(peer);
	}
	return 0;
