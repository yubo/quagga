		   ...
		   route_map_object_t type, void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		return ((as_list_apply(as_list, bgp_info->attr->aspath) ==
			 AS_FILTER_DENY) ? RMAP_NOMATCH : RMAP_MATCH);
	}
	return RMAP_NOMATCH;
}
		      ...
		      route_map_object_t type, void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		list =
		    community_list_lookup(bgp_clist, rcom->name,
		       ...
		       route_map_object_t type, void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		if (ecommunity_list_match
		    (bgp_info->attr->extra->ecommunity, list))
			return RMAP_MATCH;
	}
	return RMAP_NOMATCH;
}
	...
	if (type == RMAP_BGP) {
		...
		if (rins->peer_address) {
			if ((CHECK_FLAG(peer->rmap_type, PEER_RMAP_TYPE_IN) ||
			    ...
			    if (CHECK_FLAG(peer->rmap_type, PEER_RMAP_TYPE_OUT)
				&& peer->su_local
				&& sockunion_family(peer->su_local) ==
				AF_INET) {
				bgp_info->attr->nexthop.s_addr =
				    sockunion2ip(peer->su_local);
				bgp_info->attr->flag |=
				    ATTR_FLAG_BIT(BGP_ATTR_NEXT_HOP);
			}
		} else {
...
static void *route_set_ip_nexthop_compile(const char *arg)
{
	...
	if (strcmp(arg, "peer-address") == 0)
		...
		address =
		    XMALLOC(MTYPE_ROUTE_MAP_COMPILED, sizeof(struct in_addr));
...
static void *route_set_local_pref_compile(const char *arg)
{
	unsigned long tmp;
	u_int32_t *local_pref;
	char *endptr = NULL;
...
static void route_set_local_pref_free(void *rule)
{
	XFREE(MTYPE_ROUTE_MAP_COMPILED, rule);
}
...
/* set local preference compilation. */
static void *route_set_weight_compile(const char *arg)
{
	unsigned long tmp;
	u_int32_t *weight;
	char *endptr = NULL;
...
/* Free route map's local preference value. */
static void route_set_weight_free(void *rule)
{
	XFREE(MTYPE_ROUTE_MAP_COMPILED, rule);
}
	...
	if (type == RMAP_BGP) {
		...
		if (all_digit(metric)) {
			...
			if (strncmp(metric, "+", 1) == 0) {
				if (bgp_info->attr->med / 2 + metric_val / 2 >
				    BGP_MED_MAX / 2)
			   ...
			   route_map_object_t type, void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		binfo = object;
		list =
		    community_list_lookup(bgp_clist, rule,
					  COMMUNITY_LIST_MASTER);
		     ...
		     route_map_object_t type, void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		if (old_ecom) {
			new_ecom =
			    ecommunity_merge(ecommunity_dup(old_ecom), ecom);
...
/* For origin set. */
static route_map_result_t
route_set_origin(void *rule, struct prefix *prefix, route_map_object_t type,
		 void *object)
{
	u_char *origin;
	struct bgp_info *bgp_info;
			  ...
			  route_map_object_t type, void *object)
{
	...
	if (type == RMAP_BGP) {
		...
		if (IPV6_ADDR_SAME
		    (&bgp_info->attr->extra->mp_nexthop_global, addr))
			return RMAP_MATCH;
...
/* For origin set. */
static route_map_result_t
route_set_originator_id(void *rule, struct prefix *prefix,
			route_map_object_t type, void *object)
{
	struct in_addr *address;
	struct bgp_info *bgp_info;
	...
	for (ALL_LIST_ELEMENTS(bm->bgp, mnode, mnnode, bgp)) {
		for (afi = AFI_IP; afi < AFI_MAX; afi++)
			for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++)
				for (bn = bgp_table_top(bgp->route[afi][safi]);
		...
		for (i = 0; i < ZEBRA_ROUTE_MAX; i++) {
			if (bgp->rmap[ZEBRA_FAMILY_IPV4][i].name)
				bgp->rmap[ZEBRA_FAMILY_IPV4][i].map =
				    route_map_lookup_by_name(bgp->rmap
							     [ZEBRA_FAMILY_IPV4]
							     [i].name);
#ifdef HAVE_IPV6
...
DEFUN(no_match_peer,
      no_match_peer_cmd,
      "no match peer", NO_STR MATCH_STR "Match peer address\n")
{
	if (argc == 0)
...
DEFUN(no_match_ip_address,
      ...
      "no match ip address", NO_STR MATCH_STR IP_STR "Match address of route\n")
{
	if (argc == 0)
...
{
	if (argc == 0)
		return bgp_route_match_delete(vty, vty->index,
...
ALIAS(no_match_ip_next_hop_prefix_list,
    ...
    DEFUN(match_ip_route_source_prefix_list,
      ...
      "Match entries of prefix-lists\n" "IP prefix-list name\n")
{
	return bgp_route_match_add(vty, vty->index,
				   "ip route-source prefix-list", argv[0]);
}
...
DEFUN(no_match_metric,
      no_match_metric_cmd,
      "no match metric", NO_STR MATCH_STR "Match metric of route\n")
{
	if (argc == 0)
...
DEFUN(no_match_aspath,
      no_match_aspath_cmd,
      "no match as-path", NO_STR MATCH_STR "Match BGP AS path list\n")
{
      ...
      "BGP origin code\n" "remote EGP\n" "local IGP\n" "unknown heritage\n")
{
	...
	if (strncmp(argv[0], "incomplete", 2) == 0)
		return bgp_route_match_add(vty, vty->index, "origin",
					   "incomplete");
...
DEFUN(no_match_origin,
      no_match_origin_cmd,
      "no match origin", NO_STR MATCH_STR "BGP origin code\n")
{
...
{
	return bgp_route_set_add(vty, vty->index, "ip next-hop",
				 "peer-address");
}
...
DEFUN(no_set_ip_nexthop,
      ...
      "no set ip next-hop", NO_STR SET_STR "Next hop address\n")
{
	if (argc == 0)
...
DEFUN(no_set_weight,
      no_set_weight_cmd,
      "no set weight", NO_STR SET_STR "BGP weight for routing table\n")
{
	if (argc == 0)
      ...
      SET_STR "Transform BGP AS_PATH attribute\n" "Prepend to the as-path\n")
{
	...
	if (argc == 0)
		return bgp_route_set_delete(vty, vty->index, "as-path prepend",
					    NULL);
      ...
      SET_STR "Transform BGP AS_PATH attribute\n" "Exclude from the as-path\n")
{
	...
	if (argc == 0)
		return bgp_route_set_delete(vty, vty->index, "as-path exclude",
					    NULL);
      ...
      "Community number in aa:nn format or local-AS|no-advertise|no-export|internet or additive\n")
{
	...
	if (additive) {
		argstr =
		    XCALLOC(MTYPE_TMP, strlen(str) + strlen(" additive") + 1);
		strcpy(argstr, str);
...
DEFUN(no_set_community,
      ...
      "no set community", NO_STR SET_STR "BGP community attribute\n")
{
      ...
      "BGP origin code\n" "remote EGP\n" "local IGP\n" "unknown heritage\n")
{
	...
	if (strncmp(argv[0], "incomplete", 2) == 0)
		return bgp_route_set_add(vty, vty->index, "origin",
					 "incomplete");
...
DEFUN(no_set_origin,
      no_set_origin_cmd, "no set origin", NO_STR SET_STR "BGP origin code\n")
{
      ...
      NO_STR SET_STR "BGP aggregator attribute\n" "AS number of aggregator\n")
{
	...
	if (argv == 0)
		return bgp_route_set_delete(vty, vty->index, "aggregator as",
					    NULL);
...
ALIAS(no_set_aggregator_as,
    ...
    DEFUN(match_ipv6_address,
      ...
      MATCH_STR
      IPV6_STR "Match IPv6 address of route\n" "IPv6 access-list name\n")
{
...
DEFUN(no_match_ipv6_next_hop,
      ...
      "Match IPv6 next-hop address of route\n" "IPv6 address of next hop\n")
{
	return bgp_route_match_delete(vty, vty->index, "ipv6 next-hop",
				      argv[0]);
}
...
{
	return bgp_route_set_add(vty, vty->index, "ipv6 next-hop peer-address",
				 NULL);
}
...
DEFUN(set_ipv6_nexthop_global,
      ...
      "IPv6 global address\n" "IPv6 address of next hop\n")
{
	return bgp_route_set_add(vty, vty->index, "ipv6 next-hop global",
				 argv[0]);
}
...
{
	if (argc == 0)
		return bgp_route_set_delete(vty, vty->index,
...
ALIAS(no_set_ipv6_nexthop_global,
    ...
    DEFUN(set_ipv6_nexthop_local,
      ...
      "IPv6 local address\n" "IPv6 address of next hop\n")
{
	return bgp_route_set_add(vty, vty->index, "ipv6 next-hop local",
				 argv[0]);
}
...
DEFUN(no_set_originator_id,
      ...
      "no set originator-id", NO_STR SET_STR "BGP originator ID attribute\n")
{
	if (argc == 0)
