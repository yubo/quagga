...
static void bnc_nexthop_free(struct bgp_nexthop_cache *bnc)
{
...
static struct bgp_nexthop_cache *bnc_new(void)
{
	return XCALLOC(MTYPE_BGP_NEXTHOP_CACHE,
		       sizeof(struct bgp_nexthop_cache));
}

static void bnc_free(struct bgp_nexthop_cache *bnc)
{
	bnc_nexthop_free(bnc);
	XFREE(MTYPE_BGP_NEXTHOP_CACHE, bnc);
}

static int bgp_nexthop_same(struct nexthop *next1, struct nexthop *next2)
{
...
int bgp_nexthop_onlink(afi_t afi, struct attr *attr)
{
	...
	if (afi == AFI_IP) {
		rn = bgp_node_match_ipv4(bgp_connected_table[AFI_IP],
					 &attr->nexthop);
		if (rn) {
			bgp_unlock_node(rn);
			return 1;
	...
	if (rn->info) {
		...
		if (NULL ==
			...
			if (changed) {
				...
				if (bgp_nexthop_cache_table[AFI_IP6] ==
				    cache1_table[AFI_IP6])
...
#ifdef HAVE_IPV6
	if (afi == AFI_IP6)
		return bgp_nexthop_lookup_ipv6(peer, ri, changed,
					       metricchanged);
#endif /* HAVE_IPV6 */
	...
	if (rn->info) {
		...
		if (NULL == (bnc = zlookup_query(addr)))
			...
			if (changed) {
				...
				if (bgp_nexthop_cache_table[AFI_IP] ==
				    cache1_table[AFI_IP])
...
static void bgp_scan(afi_t afi, safi_t safi)
{
	struct bgp_node *rn;
	struct bgp *bgp;
	struct bgp_info *bi;
	...
	for (ALL_LIST_ELEMENTS(bgp->peer, node, nnode, peer)) {
		...
		if (peer->afc[afi][SAFI_MULTICAST])
			bgp_maximum_prefix_overflow(peer, afi, SAFI_MULTICAST,
						    1);
		if (peer->afc[afi][SAFI_MPLS_VPN])
			bgp_maximum_prefix_overflow(peer, afi, SAFI_MPLS_VPN,
						    1);
	}
	...
	for (rn = bgp_table_top(bgp->rib[afi][SAFI_UNICAST]); rn;
		...
		for (bi = rn->info; bi; bi = next) {
			...
			if (bi->type == ZEBRA_ROUTE_BGP
				...
				if (bi->peer->sort == BGP_PEER_EBGP
				    && bi->peer->ttl == 1
				    && !CHECK_FLAG(bi->peer->flags,
						   PEER_FLAG_DISABLE_CONNECTED_CHECK))
					valid =
					    bgp_nexthop_onlink(afi, bi->attr);
				else
					valid =
					    bgp_nexthop_lookup(afi, bi->peer,
							       bi, &changed,
							       &metricchanged);
				...
				if (valid != current) {
					if (CHECK_FLAG
					    (bi->flags, BGP_INFO_VALID)) {
						bgp_aggregate_decrement(bgp,
									&rn->p,
									bi, afi,
									SAFI_UNICAST);
						bgp_info_unset_flag(rn, bi,
								    BGP_INFO_VALID);
					} else {
						bgp_info_set_flag(rn, bi,
								  BGP_INFO_VALID);
...
static int bgp_scan_timer(struct thread *t)
{
	bgp_scan_thread =
...
void bgp_connected_add(struct connected *ifc)
{
	...
	if (addr->family == AF_INET) {
		...
		rn = bgp_node_get(bgp_connected_table[AFI_IP],
				  (struct prefix *)&p);
		if (rn->info) {
			bc = rn->info;
			bc->refcnt++;
		...
		rn = bgp_node_get(bgp_connected_table[AFI_IP6],
				  (struct prefix *)&p);
		if (rn->info) {
			bc = rn->info;
			bc->refcnt++;
...
void bgp_connected_delete(struct connected *ifc)
{
	...
	if (addr->family == AF_INET) {
		...
		rn = bgp_node_lookup(bgp_connected_table[AFI_IP6],
				     (struct prefix *)&p);
		if (!rn)
			return;
...
static struct bgp_nexthop_cache *zlookup_read(void)
{
	struct stream *s;
	uint16_t length;
	u_char marker;
	...
	if (nexthop_num) {
		...
		for (i = 0; i < nexthop_num; i++) {
			nexthop =
			    XCALLOC(MTYPE_NEXTHOP, sizeof(struct nexthop));
...
struct bgp_nexthop_cache *zlookup_query(struct in_addr addr)
{
	int ret;
	struct stream *s;
...
#ifdef HAVE_IPV6
static struct bgp_nexthop_cache *zlookup_read_ipv6(void)
{
	struct stream *s;
	uint16_t length;
...
struct bgp_nexthop_cache *zlookup_query_ipv6(struct in6_addr *addr)
{
	int ret;
	struct stream *s;
...
/* Scan all configured BGP route then check the route exists in IGP or
   not. */
static int bgp_import(struct thread *t)
{
	struct bgp *bgp;
	struct bgp_node *rn;
	...
	for (ALL_LIST_ELEMENTS(bm->bgp, node, nnode, bgp)) {
		for (afi = AFI_IP; afi < AFI_MAX; afi++)
			for (safi = SAFI_UNICAST; safi < SAFI_MPLS_VPN; safi++)
				for (rn = bgp_table_top(bgp->route[afi][safi]);
					...
					if ((bgp_static = rn->info) != NULL) {
						...
						if (bgp_static->valid != valid) {
							...
							if (bgp_static->igpmetric != metric || bgp_static->igpnexthop.s_addr != nexthop.s_addr || bgp_static->rmap.name)
								bgp_static_update
								    (bgp,
								     &rn->p,
								     bgp_static,
...
static int zlookup_connect(struct thread *t)
{
...
int bgp_multiaccess_check_v4(struct in_addr nexthop, char *peer)
{
	struct bgp_node *rn1;
	struct bgp_node *rn2;
	struct prefix p1;
      ...
      "Configure background scanner interval\n" "Scanner interval (seconds)\n")
{
	...
	if (bgp_scan_thread) {
		...
		bgp_scan_thread =
		    thread_add_timer(master, bgp_scan_timer, NULL,
				     bgp_scan_interval);
	}
...
static int show_ip_bgp_scan_tables(struct vty *vty, const char detail)
{
	struct bgp_node *rn;
	struct bgp_nexthop_cache *bnc;
	...
	vty_out(vty, "BGP scan interval is %d%s", bgp_scan_interval,
		VTY_NEWLINE);
...
#ifdef HAVE_IPV6
	{
		for (rn = bgp_table_top(bgp_connected_table[AFI_IP6]);
			...
			if (rn->info != NULL)
				vty_out(vty, " %s/%d%s",
					inet_ntop(AF_INET6, &rn->p.u.prefix6,
						  buf, INET6_ADDRSTRLEN),
					rn->p.prefixlen, VTY_NEWLINE);
	}
#endif /* HAVE_IPV6 */
...
DEFUN(show_ip_bgp_scan,
      ...
      "show ip bgp scan", SHOW_STR IP_STR BGP_STR "BGP scan status\n")
{
...
DEFUN(show_ip_bgp_scan_detail,
      ...
      SHOW_STR IP_STR BGP_STR "BGP scan status\n" "More detailed output\n")
{
	return show_ip_bgp_scan_tables(vty, 1);
}

int bgp_config_write_scan_time(struct vty *vty)
{
	if (bgp_scan_interval != BGP_SCAN_INTERVAL_DEFAULT)
		vty_out(vty, " bgp scan-time %d%s", bgp_scan_interval,
			VTY_NEWLINE);
	return CMD_SUCCESS;
}
...
void bgp_scan_init(void)
{
	...
	zlookup->sock = -1;
	zlookup->t_connect =
	    thread_add_event(master, zlookup_connect, zlookup, 0);
