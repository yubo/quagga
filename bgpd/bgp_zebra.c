...
static int
bgp_interface_delete(int command, struct zclient *zclient, zebra_size_t length)
{
	struct stream *s;
	struct interface *ifp;
		...
		if (BGP_DEBUG(zebra, ZEBRA)) {
			...
			zlog_debug
			    ("Zebra rcvd: IPv4 route add %s %s/%d nexthop %s metric %u",
			     zebra_route_string(api.type), inet_ntop(AF_INET,
								     &p.prefix,
								     buf[0],
								     sizeof(buf
									    [0])),
		...
		if (BGP_DEBUG(zebra, ZEBRA)) {
			...
			zlog_debug
			    ("Zebra rcvd: IPv6 route add %s %s/%d nexthop %s metric %u",
			     zebra_route_string(api.type), inet_ntop(AF_INET6,
								     &p.prefix,
								     buf[0],
								     sizeof(buf
									    [0])),
		...
		struct bgp_nexthop *nexthop, struct peer *peer)
{
	...
	if (local->sa.sa_family == AF_INET6) {
		if (IN6_IS_ADDR_LINKLOCAL(&local->sin6.sin6_addr)) {
			if (peer->ifname)
				ifp =
				    if_lookup_by_index(if_nametoindex
						       (peer->ifname));
		} else
		...
		if (BGP_DEBUG(zebra, ZEBRA)) {
			...
			zlog_debug
			    ("Zebra send: IPv4 route add %s/%d nexthop %s metric %u"
			     " count %d", inet_ntop(AF_INET, &p->u.prefix4,
						    buf[0], sizeof(buf[0])),
			     p->prefixlen, inet_ntop(AF_INET, api.nexthop[0],
						     buf[1], sizeof(buf[1])),
			     api.metric, api.nexthop_num);
			for (i = 1; i < api.nexthop_num; i++)
				zlog_debug
				    ("Zebra send: IPv4 route add [nexthop %d] %s",
				     i, inet_ntop(AF_INET, api.nexthop[i],
						  buf[1], sizeof(buf[1])));
		}
...
int bgp_redistribute_set(struct bgp *bgp, afi_t afi, int type)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("Zebra send: redistribute add %s",
...
int bgp_redistribute_unset(struct bgp *bgp, afi_t afi, int type)
{
	...
	if (bgp->redist[AFI_IP][type] == 0
	    && bgp->redist[AFI_IP6][type] == 0 && zclient->sock >= 0) {
