	...
	if (attr->extra) {
		...
		if (attr->extra->mp_nexthop_len == 16
			...
			snprintf(buf + strlen(buf), size - strlen(buf),
				 ", mp_nexthop %s", inet_ntop(AF_INET6,
							      &attr->
		 ...
		 const char *direct)
{
	...
	case BGP_NOTIFY_HEADER_ERR:
		subcode_str =
		    LOOKUP_DEF(bgp_notify_head_msg, bgp_notify->subcode,
			       "Unrecognized Error Subcode");
		break;
	case BGP_NOTIFY_OPEN_ERR:
		subcode_str =
		    LOOKUP_DEF(bgp_notify_open_msg, bgp_notify->subcode,
			       "Unrecognized Error Subcode");
		break;
	case BGP_NOTIFY_UPDATE_ERR:
		subcode_str =
		    LOOKUP_DEF(bgp_notify_update_msg, bgp_notify->subcode,
			       "Unrecognized Error Subcode");
		break;
	...
	case BGP_NOTIFY_CEASE:
		subcode_str =
		    LOOKUP_DEF(bgp_notify_cease_msg, bgp_notify->subcode,
			       "Unrecognized Error Subcode");
		break;
	case BGP_NOTIFY_CAPABILITY_ERR:
		subcode_str =
		    LOOKUP_DEF(bgp_notify_capability_msg, bgp_notify->subcode,
			       "Unrecognized Error Subcode");
		break;
	}
	...
	if (bgp_flag_check(peer->bgp, BGP_FLAG_LOG_NEIGHBOR_CHANGES))
		...
		plog_debug(peer->log,
			   ...
			   peer ? peer->host : "", direct, bgp_notify->code,
...
DEFUN(debug_bgp_as4,
      debug_bgp_as4_cmd, "debug bgp as4", DEBUG_STR BGP_STR "BGP AS4 actions\n")
{
...
DEFUN(no_debug_bgp_as4,
      ...
      "no debug bgp as4", NO_STR DEBUG_STR BGP_STR "BGP AS4 actions\n")
{
	if (vty->node == CONFIG_NODE)
		DEBUG_OFF(as4, AS4);
	else {
...
ALIAS(no_debug_bgp_as4,
      undebug_bgp_as4_cmd,
      "undebug bgp as4", UNDEBUG_STR BGP_STR "BGP AS4 actions\n")
...
ALIAS(no_debug_bgp_as4_segment,
    ...
    DEFUN(debug_bgp_fsm,
      debug_bgp_fsm_cmd,
      "debug bgp fsm", DEBUG_STR BGP_STR "BGP Finite State Machine\n")
{
	if (vty->node == CONFIG_NODE)
		DEBUG_ON(fsm, FSM);
	else {
...
DEFUN(no_debug_bgp_fsm,
      ...
      "no debug bgp fsm", NO_STR DEBUG_STR BGP_STR "Finite State Machine\n")
{
	if (vty->node == CONFIG_NODE)
		DEBUG_OFF(fsm, FSM);
	else {
...
ALIAS(no_debug_bgp_fsm,
      undebug_bgp_fsm_cmd,
      "undebug bgp fsm", UNDEBUG_STR BGP_STR "Finite State Machine\n")

    DEFUN(debug_bgp_events,
      debug_bgp_events_cmd,
      "debug bgp events", DEBUG_STR BGP_STR "BGP events\n")
{
	if (vty->node == CONFIG_NODE)
...
DEFUN(no_debug_bgp_events,
      ...
      "no debug bgp events", NO_STR DEBUG_STR BGP_STR "BGP events\n")
{
	if (vty->node == CONFIG_NODE)
...
ALIAS(no_debug_bgp_events,
      ...
      "undebug bgp events", UNDEBUG_STR BGP_STR "BGP events\n")

    DEFUN(debug_bgp_filter,
      debug_bgp_filter_cmd,
      "debug bgp filters", DEBUG_STR BGP_STR "BGP filters\n")
{
	if (vty->node == CONFIG_NODE)
...
DEFUN(no_debug_bgp_filter,
      ...
      "no debug bgp filters", NO_STR DEBUG_STR BGP_STR "BGP filters\n")
{
	if (vty->node == CONFIG_NODE)
...
ALIAS(no_debug_bgp_filter,
      ...
      "undebug bgp filters", UNDEBUG_STR BGP_STR "BGP filters\n")

    DEFUN(debug_bgp_keepalive,
      debug_bgp_keepalive_cmd,
      "debug bgp keepalives", DEBUG_STR BGP_STR "BGP keepalives\n")
{
	if (vty->node == CONFIG_NODE)
...
ALIAS(no_debug_bgp_keepalive,
      ...
      "undebug bgp keepalives", UNDEBUG_STR BGP_STR "BGP keepalives\n")

    DEFUN(debug_bgp_update,
      debug_bgp_update_cmd,
      "debug bgp updates", DEBUG_STR BGP_STR "BGP updates\n")
{
	if (vty->node == CONFIG_NODE) {
...
DEFUN(no_debug_bgp_update,
      ...
      "no debug bgp updates", NO_STR DEBUG_STR BGP_STR "BGP updates\n")
{
	if (vty->node == CONFIG_NODE) {
		DEBUG_OFF(update, UPDATE_IN);
		DEBUG_OFF(update, UPDATE_OUT);
...
ALIAS(no_debug_bgp_update,
      ...
      "undebug bgp updates", UNDEBUG_STR BGP_STR "BGP updates\n")
...
DEFUN(no_debug_bgp_normal,
      no_debug_bgp_normal_cmd, "no debug bgp", NO_STR DEBUG_STR BGP_STR)
{
	if (vty->node == CONFIG_NODE)
...
ALIAS(no_debug_bgp_normal,
      undebug_bgp_normal_cmd, "undebug bgp", UNDEBUG_STR BGP_STR)

    DEFUN(debug_bgp_zebra,
      debug_bgp_zebra_cmd,
      "debug bgp zebra", DEBUG_STR BGP_STR "BGP Zebra messages\n")
{
	if (vty->node == CONFIG_NODE)
...
DEFUN(no_debug_bgp_zebra,
      ...
      "no debug bgp zebra", NO_STR DEBUG_STR BGP_STR "BGP Zebra messages\n")
{
	if (vty->node == CONFIG_NODE)
...
ALIAS(no_debug_bgp_zebra,
      ...
      "undebug bgp zebra", UNDEBUG_STR BGP_STR "BGP Zebra messages\n")

    DEFUN(no_debug_bgp_all,
      no_debug_bgp_all_cmd,
      "no debug all bgp", NO_STR DEBUG_STR "Enable all debugging\n" BGP_STR)
{
	TERM_DEBUG_OFF(normal, NORMAL);
	TERM_DEBUG_OFF(events, EVENTS);
	TERM_DEBUG_OFF(keepalive, KEEPALIVE);
...
ALIAS(no_debug_bgp_all,
      undebug_bgp_all_cmd,
      "undebug all bgp", UNDEBUG_STR "Enable all debugging\n" BGP_STR)

    DEFUN(show_debugging_bgp,
      show_debugging_bgp_cmd, "show debugging bgp", SHOW_STR DEBUG_STR BGP_STR)
{
...
static int bgp_config_write_debug(struct vty *vty)
{
	...
	if (CONF_BGP_DEBUG(update, UPDATE_IN)
	    && CONF_BGP_DEBUG(update, UPDATE_OUT)) {
