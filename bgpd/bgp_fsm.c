...
static int bgp_connect_timer(struct thread *thread)
{
	...
	if (BGP_DEBUG(fsm, FSM))
		zlog(peer->log, LOG_DEBUG,
		     "%s [FSM] Timer (connect timer expire)", peer->host);
...
static int bgp_holdtime_timer(struct thread *thread)
{
	...
	if (BGP_DEBUG(fsm, FSM))
		zlog(peer->log, LOG_DEBUG,
		     "%s [FSM] Timer (holdtime timer expire)", peer->host);
...
static int bgp_keepalive_timer(struct thread *thread)
{
	...
	if (BGP_DEBUG(fsm, FSM))
		zlog(peer->log, LOG_DEBUG,
		     "%s [FSM] Timer (keepalive timer expire)", peer->host);
...
static int bgp_routeadv_timer(struct thread *thread)
{
	...
	if (BGP_DEBUG(fsm, FSM))
		zlog(peer->log, LOG_DEBUG,
		     "%s [FSM] Timer (routeadv timer expire)", peer->host);
...
int bgp_stop(struct peer *peer)
{
	...
	if (peer->status == Established) {
		...
		if (bgp_flag_check(peer->bgp, BGP_FLAG_LOG_NEIGHBOR_CHANGES))
			zlog_info("%%ADJCHANGE: neighbor %s Down %s",
				  peer->host,
	...
	if (BGP_DEBUG(normal, NORMAL)) {
		...
		if (!CHECK_FLAG(peer->sflags, PEER_STATUS_ACCEPT_PEER))
			zlog_debug("%s open active, local address %s",
				   peer->host, sockunion2str(peer->su_local,
							     buf1,
							     SU_ADDRSTRLEN));
		else
...
int bgp_start(struct peer *peer)
{
	...
	case connect_error:
		if (BGP_DEBUG(fsm, FSM))
			plog_debug(peer->log, "%s [FSM] Connect error",
				   peer->host);
		BGP_EVENT_ADD(peer, TCP_connection_open_failed);
		break;
