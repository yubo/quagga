...
static struct bgp_node *bgp_afi_node_get(struct bgp_table *table, afi_t afi,
...
static struct bgp_info *bgp_info_new(void)
{
	...
	if (attr->community) {
		...
		if (peer->sort == BGP_PEER_EBGP
			...
			if (community_include
			    (attr->community, COMMUNITY_NO_EXPORT_SUBCONFED))
				return 1;
	}
	return 0;
}
		    ...
		    struct prefix *p, struct attr *attr, afi_t afi, safi_t safi)
{
	...
	if (ROUTE_MAP_EXPORT_NAME(filter)) {
		...
		/* Apply BGP route map to the attribute. */
		ret =
		    route_map_apply(ROUTE_MAP_EXPORT(filter), p, RMAP_BGP,
				    &info);
		    ...
		    struct prefix *p, struct attr *attr, afi_t afi, safi_t safi)
{
	...
	if (ROUTE_MAP_IMPORT_NAME(filter)) {
		...
		/* Apply BGP route map to the attribute. */
		ret =
		    route_map_apply(ROUTE_MAP_IMPORT(filter), p, RMAP_BGP,
				    &info);
		   ...
		   struct attr *attr, afi_t afi, safi_t safi)
{
	...
	/* Default route check.  */
	if (CHECK_FLAG
	    (peer->af_sflags[afi][safi], PEER_STATUS_DEFAULT_ORIGINATE)) {
	...
	if (reflect) {
		...
		if (CHECK_FLAG
		    (from->af_flags[afi][safi], PEER_FLAG_REFLECTOR_CLIENT)) {
	...
	if (transparent
		...
		/* NEXT-HOP Unchanged. */
		   ...
		   || (peer->sort == BGP_PEER_EBGP
		       && bgp_multiaccess_check_v4(attr->nexthop,
						   peer->host) == 0)) {
		/* Set IPv4 nexthop. */
		if (p->family == AF_INET) {
			if (safi == SAFI_MPLS_VPN)
				memcpy(&attr->extra->mp_nexthop_global_in,
				       &peer->nexthop.v4, IPV4_MAX_BYTELEN);
			else
				memcpy(&attr->nexthop, &peer->nexthop.v4,
				       IPV4_MAX_BYTELEN);
		}
#ifdef HAVE_IPV6
		/* Set IPv6 nexthop. */
		if (p->family == AF_INET6) {
			...
			memcpy(&attr->extra->mp_nexthop_global,
			       &peer->nexthop.v6_global, IPV6_MAX_BYTELEN);
	...
	if (p->family == AF_INET6) {
		...
		/* Default nexthop_local treatment for non-RS-Clients */
			...
			if (peer->shared_network
			    && !IN6_IS_ADDR_UNSPECIFIED(&peer->
							nexthop.v6_local)) {
				memcpy(&attr->extra->mp_nexthop_local,
	...
	if (ROUTE_MAP_OUT_NAME(filter)
		...
		if (ri->extra && ri->extra->suppress)
			ret =
			    route_map_apply(UNSUPPRESS_MAP(filter), p, RMAP_BGP,
					    &info);
		else
			ret =
			    route_map_apply(ROUTE_MAP_OUT(filter), p, RMAP_BGP,
					    &info);
			    ...
			    safi_t safi)
{
	...
	if (CHECK_FLAG(rsclient->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_RM_ADV)
	    ...
	    (CHECK_FLAG(rsclient->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_RCV)
	     || CHECK_FLAG(rsclient->af_cap[afi][safi],
			   PEER_CAP_ORF_PREFIX_SM_OLD_RCV)))
		if (rsclient->orf_plist[afi][safi]) {
			if (prefix_list_apply(rsclient->orf_plist[afi][safi], p)
			    == PREFIX_DENY)
				return 0;
		}
	...
	if ((p->family == AF_INET && attr->nexthop.s_addr == 0)
		...
		if (p->family == AF_INET) {
			if (safi == SAFI_MPLS_VPN)
				memcpy(&attr->extra->mp_nexthop_global_in,
				       &rsclient->nexthop.v4, IPV4_MAX_BYTELEN);
			else
		...
		if (p->family == AF_INET6) {
			/* IPv6 global nexthop must be included. */
			memcpy(&attr->extra->mp_nexthop_global,
			       &rsclient->nexthop.v6_global, IPV6_MAX_BYTELEN);
			attr->extra->mp_nexthop_len = 16;
		}
#endif /* HAVE_IPV6 */
	}
	...
	if (p->family == AF_INET6) {
		...
		/* Default nexthop_local treatment for RS-Clients */
			...
			/* Set link-local address for shared network peer. */
				 ...
				 && IN6_IS_ADDR_LINKLOCAL(&rsclient->
							  nexthop.v6_local)) {
				memcpy(&attre->mp_nexthop_local,
				       &rsclient->nexthop.v6_local,
				       IPV6_MAX_BYTELEN);
				attre->mp_nexthop_len = 32;
			}
		   ...
		   struct bgp_info_pair *result)
{
	...
	if (bgp_flag_check(bgp, BGP_FLAG_DETERMINISTIC_MED))
		for (ri1 = rn->info; ri1; ri1 = ri1->next) {
			...
			old_select =
			    CHECK_FLAG(ri1->flags,
				       BGP_INFO_SELECTED) ? ri1 : NULL;
			if (ri1->next)
				for (ri2 = ri1->next; ri2; ri2 = ri2->next) {
					if (CHECK_FLAG
					    (ri2->flags, BGP_INFO_DMED_CHECK))
						continue;
			...
			bgp_info_mpath_update(rn, new_select, old_select,
					      &mp_list, mpath_cfg);
			      ...
			      struct bgp_node *rn, afi_t afi, safi_t safi)
{
	...
	case BGP_TABLE_MAIN:
		/* Announcement to peer->conf.  If the route is filtered,
		   withdraw it. */
		if (selected
		    && bgp_announce_check(selected, peer, p, &attr, afi, safi))
			bgp_adj_out_set(rn, peer, p, &attr, afi, safi,
					selected);
		else
	...
	case BGP_TABLE_RSCLIENT:
		/* Announcement to peer->conf.  If the route is filtered, 
		   withdraw it. */
		if (selected &&
		    bgp_announce_check_rsclient(selected, peer, p, &attr, afi,
						safi))
			bgp_adj_out_set(rn, peer, p, &attr, afi, safi,
					selected);
		else
	...
	if (CHECK_FLAG(rsclient->sflags, PEER_STATUS_GROUP)) {
		if (rsclient->group)
			for (ALL_LIST_ELEMENTS
			     (rsclient->group->peer, node, nnode, rsclient)) {
				/* Nothing to do. */
				...
				if (old_select)
					bgp_info_unset_flag(rn, old_select,
							    BGP_INFO_SELECTED);
				if (new_select) {
					bgp_info_set_flag(rn, new_select,
							  BGP_INFO_SELECTED);
					bgp_info_unset_flag(rn, new_select,
							    BGP_INFO_ATTR_CHANGED);
					UNSET_FLAG(new_select->flags,
						   BGP_INFO_MULTIPATH_CHG);
				}

				bgp_process_announce_selected(rsclient,
							      new_select, rn,
							      afi, safi);
			}
	} else {
		...
		bgp_process_announce_selected(rsclient, new_select, rn, afi,
					      safi);
	}
			    ...
			    safi_t safi, int always)
{
	...
	if (peer->pcount[afi][safi] > peer->pmax[afi][safi]) {
		if (CHECK_FLAG
		    (peer->af_sflags[afi][safi], PEER_STATUS_PREFIX_LIMIT)
		    && !always)
			return 0;
		...
		if (CHECK_FLAG
		    (peer->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_WARNING))
			return 0;
	...
	if (peer->pcount[afi][safi] >
		...
		if (CHECK_FLAG
		    (peer->af_sflags[afi][safi], PEER_STATUS_PREFIX_THRESHOLD)
		    && !always)
			return 0;
		    ...
		    int type, int sub_type, struct prefix_rd *prd, u_char * tag)
{
	...
	/* AS path loop check. */
	if (aspath_loop_check(attr->aspath, rsclient->as) >
	    rsclient->allowas_in[afi][safi]) {
	...
	/* Apply export policy. */
	if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT) &&
	    bgp_export_modifier(rsclient, peer, p, &new_attr, afi,
				safi) == RMAP_DENY) {
	...
	/* Apply import policy. */
	if (bgp_import_modifier(rsclient, peer, p, &new_attr, afi, safi) ==
	    RMAP_DENY) {
	...
	if (ri) {
		...
		if (BGP_DEBUG(update, UPDATE_IN))
			zlog(peer->log, LOG_DEBUG,
			     "%s rcvd %s/%d for RS-client %s", peer->host,
			     inet_ntop(p->family, &p->u.prefix, buf,
				       SU_ADDRSTRLEN), p->prefixlen,
			     rsclient->host);
		...
		struct prefix_rd *prd, u_char * tag, int soft_reconfig)
{
	...
	if (ri) {
		...
		if (!CHECK_FLAG(ri->flags, BGP_INFO_REMOVED)
			...
			if (CHECK_FLAG
			    (bgp->af_flags[afi][safi], BGP_CONFIG_DAMPENING)
			    && peer->sort == BGP_PEER_EBGP
			    && CHECK_FLAG(ri->flags, BGP_INFO_HISTORY)) {
				if (BGP_DEBUG(update, UPDATE_IN))
					zlog(peer->log, LOG_DEBUG,
					     "%s rcvd %s/%d", peer->host,
	   ...
	   struct prefix_rd *prd, u_char * tag, int soft_reconfig)
{
	...
	ret =
	    bgp_update_main(peer, p, attr, afi, safi, type, sub_type, prd, tag,
			    soft_reconfig);
	...
	for (ALL_LIST_ELEMENTS(bgp->rsclient, node, nnode, rsclient)) {
		if (CHECK_FLAG
		    (rsclient->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT))
	     ...
	     struct prefix_rd *prd, u_char * tag)
{
	...
	for (ALL_LIST_ELEMENTS(bgp->rsclient, node, nnode, rsclient)) {
		if (CHECK_FLAG
		    (rsclient->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT))
			bgp_withdraw_rsclient(rsclient, afi, safi, peer, p,
					      type, sub_type, prd, tag);
	}
	...
	if (BGP_DEBUG(update, UPDATE_IN))
		zlog(peer->log, LOG_DEBUG,
		     "%s rcvd UPDATE about %s/%d -- withdrawn", peer->host,
		     inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
		     p->prefixlen);
	...
	if (peer->default_rmap[afi][safi].name) {
		...
		for (rn = bgp_table_top(bgp->rib[afi][safi]); rn;
			...
			for (ri = rn->info; ri; ri = ri->next) {
				...
				ret =
				    route_map_apply(peer->default_rmap[afi]
						    [safi].map, &rn->p,
						    RMAP_BGP, &info);
		   ...
		   struct bgp_table *table, int rsclient)
{
	...
	if (!table)
		table =
		    (rsclient) ? peer->rib[afi][safi] : peer->
		    bgp->rib[afi][safi];
			...
			if (CHECK_FLAG(ri->flags, BGP_INFO_SELECTED)
				...
				if ((rsclient)
				    ? (bgp_announce_check_rsclient
				       (ri, peer, &rn->p, &attr, afi, safi))
				    : (bgp_announce_check
				       (ri, peer, &rn->p, &attr, afi, safi)))
					bgp_adj_out_set(rn, peer, &rn->p, &attr,
							afi, safi, ri);
				else
					bgp_adj_out_unset(rn, peer, &rn->p, afi,
							  safi);
			}
}
...
void bgp_announce_route(struct peer *peer, afi_t afi, safi_t safi)
{
	...
	/* First update is deferred until ORF or ROUTE-REFRESH is received */
	if (CHECK_FLAG
	    (peer->af_sflags[afi][safi], PEER_STATUS_ORF_WAIT_REFRESH))
		return;
				 ...
				 struct prefix_rd *prd)
{
	...
	for (rn = bgp_table_top(table); rn; rn = bgp_route_next(rn))
		for (ain = rn->adj_in; ain; ain = ain->next) {
			...
			bgp_update_rsclient(rsclient, afi, safi, ain->attr,
	...
	if (safi != SAFI_MPLS_VPN)
		...
		for (rn = bgp_table_top(rsclient->bgp->rib[afi][safi]); rn;
			...
			if ((table = rn->info) != NULL) {
				...
				bgp_soft_reconfig_table_rsclient(rsclient, afi,
								 safi, table,
								 &prd);
			}
}
	...
	for (rn = bgp_table_top(table); rn; rn = bgp_route_next(rn))
		for (ain = rn->adj_in; ain; ain = ain->next) {
			if (ain->peer == peer) {
				...
				ret =
				    bgp_update(peer, &rn->p, ain->attr, afi,
					       safi, ZEBRA_ROUTE_BGP,
					       BGP_ROUTE_NORMAL, prd, tag, 1);
...
static wq_item_status bgp_clear_route_node(struct work_queue *wq, void *data)
{
	...
	for (ri = rn->info; ri; ri = ri->next)
		if (ri->peer == peer
		    || cnq->purpose == BGP_CLEAR_ROUTE_MY_RSCLIENT) {
...
#undef CLEAR_QUEUE_NAME_LEN
	...
	if ((peer->clear_node_queue =
	     work_queue_new(bm->master, wname)) == NULL) {
		zlog_err("%s: Failed to allocate work queue", __func__);
		exit(1);
	}
		      ...
		      enum bgp_clear_route_type purpose)
{
	...
	if (!table)
		table =
		    (rsclient) ? rsclient->rib[afi][safi] : peer->
		    bgp->rib[afi][safi];
	...
	for (rn = bgp_table_top(table); rn; rn = bgp_route_next(rn)) {
		...
		for (ain = rn->adj_in; ain; ain = ain->next)
			if (ain->peer == peer
			    || purpose == BGP_CLEAR_ROUTE_MY_RSCLIENT) {
		...
		enum bgp_clear_route_type purpose)
{
	...
	case BGP_CLEAR_ROUTE_NORMAL:
		...
		for (ALL_LIST_ELEMENTS
		     (peer->bgp->rsclient, node, nnode, rsclient))
			if (CHECK_FLAG
			    (rsclient->af_flags[afi][safi],
...
bgp_nlri_parse(struct peer *peer, struct attr *attr, struct bgp_nlri *packet)
{
	...
	for (; pnt < lim; pnt += psize) {
		...
		if (attr)
			ret =
			    bgp_update(peer, &p, attr, packet->afi,
				       packet->safi, ZEBRA_ROUTE_BGP,
				       BGP_ROUTE_NORMAL, NULL, NULL, 0);
		else
			ret =
			    bgp_withdraw(peer, &p, attr, packet->afi,
					 packet->safi, ZEBRA_ROUTE_BGP,
					 BGP_ROUTE_NORMAL, NULL, NULL);
			   ...
			   afi_t afi, safi_t safi)
{
	...
	if (bgp_static->rmap.name) {
		...
		if (ret == RMAP_DENYMATCH) {
			...
			bgp_static_withdraw_rsclient(bgp, rsclient, p, afi,
						     safi);
			bgp_attr_extra_free(&attr);
	...
	if (bgp_import_modifier
	    (rsclient, bgp->peer_self, p, &new_attr, afi, safi)
	    == RMAP_DENY) {
		/* This BGP update is filtered.  Log the reason then update BGP entry.  */
		if (BGP_DEBUG(update, UPDATE_IN))
			zlog(rsclient->log, LOG_DEBUG,
		  ...
		  struct bgp_static *bgp_static, afi_t afi, safi_t safi)
{
	...
	for (ALL_LIST_ELEMENTS(bgp->rsclient, node, nnode, rsclient)) {
		if (CHECK_FLAG
		    (rsclient->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT))
			bgp_static_update_rsclient(rsclient, p, bgp_static, afi,
						   safi);
	}
}
...
void
bgp_static_withdraw(struct bgp *bgp, struct prefix *p, afi_t afi, safi_t safi)
{
	struct bgp_node *rn;
	struct bgp_info *ri;
		     ...
		     const char *tag_str)
{
	...
	if (rn->info) {
		...
		bgp_static_update_vpnv4(bgp, &p, AFI_IP, SAFI_MPLS_VPN, &prd,
					tag);
	}
		       ...
		       const char *rd_str, const char *tag_str)
{
	...
	if (rn) {
		bgp_static_withdraw_vpnv4(bgp, &p, AFI_IP, SAFI_MPLS_VPN, &prd,
					  tag);
...
bgp_aggregate_set(struct vty *vty, const char *prefix_str,
		  afi_t afi, safi_t safi, u_char summary_only, u_char as_set)
{
	int ret;
	struct prefix p;
	struct bgp_node *rn;
...
{
	return bgp_aggregate_set(vty, argv[0], AFI_IP, bgp_node_safi(vty), 0,
				 0);
}
	...
	for (ALL_LIST_ELEMENTS(bm->bgp, node, nnode, bgp)) {
		...
		if (bgp->redist[afi][type]) {
			...
			if (bgp->rmap[afi][type].map) {
				...
				ret =
				    route_map_apply(bgp->rmap[afi][type].map, p,
						    RMAP_BGP, &info);
...
void bgp_redistribute_delete(struct prefix *p, u_char type)
{
	...
	for (ALL_LIST_ELEMENTS(bm->bgp, node, nnode, bgp)) {
		...
		if (bgp->redist[afi][type]) {
			...
			if (ri) {
				bgp_aggregate_decrement(bgp, p, ri, afi,
							SAFI_UNICAST);
				bgp_info_delete(rn, ri);
		     ...
		     struct bgp_info *binfo, afi_t afi, safi_t safi)
{
	...
	if (attr) {
		...
		if (p->family == AF_INET) {
			...
			vty_out(vty, "    %s",
				inet_ntop(AF_INET6,
					  &attr->extra->mp_nexthop_global, buf,
					  INET6_ADDRSTRLEN));
		}
#endif /* HAVE_IPV6 */
		...
		if (attr->extra && attr->extra->mp_nexthop_len == 32) {
			vty_out(vty, "    (%s)%s",
				inet_ntop(AF_INET6,
					  &attr->extra->mp_nexthop_local, buf,
		...
		if (binfo->peer != bgp->peer_self) {
			if (binfo->peer->as == binfo->peer->local_as)
				...
				vty_out(vty, ", %s",
					(bgp_confederation_peers_check
					 (bgp,
					  binfo->peer->as) ? "confed-external" :
					 "external"));
		...
		if ((attr->flag & ATTR_FLAG_BIT(BGP_ATTR_ORIGINATOR_ID)) ||
			...
			if (attr->flag & ATTR_FLAG_BIT(BGP_ATTR_CLUSTER_LIST)) {
				...
				for (i = 0;
				     i < attr->extra->cluster->length / 4; i++)
					vty_out(vty, "%s ",
						inet_ntoa(attr->extra->
							  cluster->list[i]));
			}
			vty_out(vty, "%s", VTY_NEWLINE);
		}
	...
	for (rn = bgp_table_top(table); rn; rn = bgp_route_next(rn))
		if (rn->info != NULL) {
			...
			for (ri = rn->info; ri; ri = ri->next) {
				...
				if (type == bgp_show_type_dampend_paths
				    || type == bgp_show_type_damp_neighbor)
					damp_route_vty_out(vty, &rn->p, ri,
							   display,
							   SAFI_UNICAST);
					 ...
					 || type == bgp_show_type_flap_route_map
					 || type == bgp_show_type_flap_neighbor)
					flap_route_vty_out(vty, &rn->p, ri,
							   display,
							   SAFI_UNICAST);
				else
		...
		if (CHECK_FLAG(ri->flags, BGP_INFO_SELECTED)) {
			...
			if (ri->attr->community != NULL) {
				if (community_include
					...
					no_advertise = 1;
				if (community_include
				    (ri->attr->community, COMMUNITY_NO_EXPORT))
					no_export = 1;
				if (community_include
				    (ri->attr->community, COMMUNITY_LOCAL_AS))
					local_as = 1;
			}
		}
	}
	...
	if (safi == SAFI_MPLS_VPN) {
		for (rn = bgp_table_top(rib); rn; rn = bgp_route_next(rn)) {
			...
			if ((table = rn->info) != NULL) {
				...
				if ((rm =
				     bgp_node_match(table, &match)) != NULL) {
					if (prefix_check
					    && rm->p.prefixlen !=
					    match.prefixlen) {
						bgp_unlock_node(rm);
		...
		if ((rn = bgp_node_match(rib, &match)) != NULL) {
			if (!prefix_check || rn->p.prefixlen == match.prefixlen) {
				for (ri = rn->info; ri; ri = ri->next) {
					if (header) {
						route_vty_out_detail_header(vty,
									    bgp,
									    rn,
									    NULL,
									    afi,
					...
					display++;
					route_vty_out_detail(vty, bgp, &rn->p,
							     ri, afi, safi);
				}
			}
...
bgp_show_route(struct vty *vty, const char *view_name, const char *ip_str,
	       afi_t afi, safi_t safi, struct prefix_rd *prd, int prefix_check)
{
	struct bgp *bgp;
	...
	if (view_name) {
		...
		if (bgp == NULL) {
			vty_out(vty, "Can't find BGP view %s%s", view_name,
				VTY_NEWLINE);
			return CMD_WARNING;
		}
	} else {
...
ALIAS(show_ip_bgp_ipv4,
    ...
    DEFUN(show_ip_bgp_route,
      ...
      "show ip bgp A.B.C.D",
      SHOW_STR IP_STR BGP_STR "Network in the BGP routing table to display\n")
{
	return bgp_show_route(vty, NULL, argv[0], AFI_IP, SAFI_UNICAST, NULL,
			      0);
}
...
ALIAS(show_ip_bgp_ipv4_route,
    ...
    DEFUN(show_ip_bgp_vpnv4_all_route,
      ...
      "Network in the BGP routing table to display\n")
{
	return bgp_show_route(vty, NULL, argv[0], AFI_IP, SAFI_MPLS_VPN, NULL,
			      0);
}
      ...
      "Network in the BGP routing table to display\n")
{
	...
	return bgp_show_route(vty, NULL, argv[1], AFI_IP, SAFI_MPLS_VPN, &prd,
			      0);
}
...
DEFUN(show_ip_bgp_prefix,
      ...
      SHOW_STR
      IP_STR BGP_STR "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n")
{
	return bgp_show_route(vty, NULL, argv[0], AFI_IP, SAFI_UNICAST, NULL,
			      1);
}
...
DEFUN(show_ip_bgp_view_route,
      ...
      "View name\n" "Network in the BGP routing table to display\n")
{
	return bgp_show_route(vty, argv[0], argv[1], AFI_IP, SAFI_UNICAST, NULL,
			      0);
}
...
ALIAS(show_bgp,
      show_bgp_ipv6_cmd, "show bgp ipv6", SHOW_STR BGP_STR "Address family\n")
...
/* old command */
DEFUN(show_ipv6_bgp,
      show_ipv6_bgp_cmd, "show ipv6 bgp", SHOW_STR IP_STR BGP_STR)
{
	return bgp_show(vty, NULL, AFI_IP6, SAFI_UNICAST, bgp_show_type_normal,
			NULL);
}
...
{
	return bgp_show_route(vty, NULL, argv[0], AFI_IP6, SAFI_UNICAST, NULL,
			      0);
}
...
DEFUN(show_ipv6_bgp_prefix,
      ...
      SHOW_STR
      IP_STR BGP_STR "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n")
{
	return bgp_show_route(vty, NULL, argv[0], AFI_IP6, SAFI_UNICAST, NULL,
			      1);
}
...
DEFUN(show_bgp_view,
      show_bgp_view_cmd,
      "show bgp view WORD", SHOW_STR BGP_STR "BGP view\n" "View name\n")
{
	struct bgp *bgp;
...
ALIAS(show_bgp_view,
    ...
    DEFUN(show_bgp_view_route,
      ...
      "View name\n" "Network in the BGP routing table to display\n")
{
	return bgp_show_route(vty, argv[0], argv[1], AFI_IP6, SAFI_UNICAST,
			      NULL, 0);
}
...
/* old command */
    DEFUN(show_ipv6_mbgp,
      show_ipv6_mbgp_cmd, "show ipv6 mbgp", SHOW_STR IP_STR MBGP_STR)
{
	return bgp_show(vty, NULL, AFI_IP6, SAFI_MULTICAST,
			bgp_show_type_normal, NULL);
}
...
DEFUN(show_ipv6_mbgp_route,
      ...
      SHOW_STR IP_STR MBGP_STR "Network in the MBGP routing table to display\n")
{
	return bgp_show_route(vty, NULL, argv[0], AFI_IP6, SAFI_MULTICAST, NULL,
			      0);
}
...
DEFUN(show_ipv6_mbgp_prefix,
      ...
      SHOW_STR
      IP_STR MBGP_STR "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n")
{
	return bgp_show_route(vty, NULL, argv[0], AFI_IP6, SAFI_MULTICAST, NULL,
			      1);
}
...
{
	if (strncmp(argv[0], "m", 1) == 0)
		return bgp_show_prefix_list(vty, argv[1], AFI_IP,
					    SAFI_MULTICAST,
					    bgp_show_type_prefix_list);
...
{
	if (strncmp(argv[0], "m", 1) == 0)
		return bgp_show_filter_list(vty, argv[1], AFI_IP,
					    SAFI_MULTICAST,
					    bgp_show_type_filter_list);
...
ALIAS(show_bgp_route_map,
    ...
    DEFUN(show_ip_bgp_cidr_only,
      ...
      SHOW_STR IP_STR BGP_STR "Display only routes with non-natural netmasks\n")
{
...
{
	return bgp_show_community(vty, NULL, argc, argv, 0, AFI_IP,
				  SAFI_UNICAST);
}
...
ALIAS(show_bgp_view_afi_safi_community, show_bgp_view_afi_safi_community2_cmd,
#ifdef HAVE_IPV6
...
{
	return bgp_show_community(vty, NULL, argc, argv, 1, AFI_IP,
				  SAFI_UNICAST);
}
...
ALIAS(show_ip_bgp_ipv4_community_exact,
    ...
    DEFUN(show_bgp_community,
      ...
      "Do not export to next AS (well-known community)\n")
{
	return bgp_show_community(vty, NULL, argc, argv, 0, AFI_IP6,
				  SAFI_UNICAST);
}
...
/* old command */
    ...
    DEFUN(show_bgp_community_exact,
      ...
      "Exact match of the communities")
{
	return bgp_show_community(vty, NULL, argc, argv, 1, AFI_IP6,
				  SAFI_UNICAST);
}
...
{
	if (strncmp(argv[0], "m", 1) == 0)
		return bgp_show_community_list(vty, argv[1], 0, AFI_IP,
					       SAFI_MULTICAST);
...
DEFUN(show_ipv6_mbgp_community_list,
      ...
      "Display routes matching the community-list\n" "community-list name\n")
{
	return bgp_show_community_list(vty, argv[0], 0, AFI_IP6,
				       SAFI_MULTICAST);
}
...
{
	if (strncmp(argv[0], "m", 1) == 0)
		return bgp_show_prefix_longer(vty, argv[1], AFI_IP,
					      SAFI_MULTICAST,
					      bgp_show_type_prefix_longer);
...
static struct peer *peer_lookup_in_view(struct vty *vty, const char *view_name,
					const char *ip_str)
{
	int ret;
	struct bgp *bgp;
	...
	for (i = 0; i < BGP_STATS_MAX; i++) {
		...
		case BGP_STATS_SPACE:
			...
			vty_out(vty, "%12.2f%s",
				...
				(float)((uint64_t) 1UL <<
					ts.counts[BGP_STATS_MAXBITLEN]),
				VTY_NEWLINE);
			vty_out(vty, "%30s: ", "/8 equivalent ");
			vty_out(vty, "%12.2f%s",
				(float)ts.counts[BGP_STATS_SPACE] /
				(float)(1UL <<
					(ts.counts[BGP_STATS_MAXBITLEN] - 8)),
				VTY_NEWLINE);
			...
			vty_out(vty, "%12.2f",
				(float)ts.counts[BGP_STATS_SPACE] /
				(float)(1UL <<
					(ts.counts[BGP_STATS_MAXBITLEN] - 24)));
			break;
		default:
			vty_out(vty, "%-30s: ", table_stats_strs[i]);
      ...
      "Display the prefixlist filter\n")
{
	...
	if (strncmp(argv[0], "m", 1) == 0) {
		...
		count = prefix_bgp_show_prefix_list(NULL, AFI_IP, name);
		if (count) {
			vty_out(vty, "Address family: IPv4 Multicast%s",
				VTY_NEWLINE);
			prefix_bgp_show_prefix_list(vty, AFI_IP, name);
		}
	} else {
		...
		count = prefix_bgp_show_prefix_list(NULL, AFI_IP, name);
		if (count) {
			vty_out(vty, "Address family: IPv4 Unicast%s",
				VTY_NEWLINE);
			prefix_bgp_show_prefix_list(vty, AFI_IP, name);
		}
	}
      ...
      "Display routes learned from neighbor\n")
{
	...
	if (strncmp(argv[0], "m", 1) == 0)
		return bgp_show_neighbor_route(vty, peer, AFI_IP,
					       SAFI_MULTICAST,
					       bgp_show_type_neighbor);
      ...
      "View name\n" "Information about Route Server Client\n" NEIGHBOR_ADDR_STR)
{
	...
	return bgp_show_table(vty, table, &peer->remote_id,
			      bgp_show_type_normal, NULL);
}
...
ALIAS(show_ip_bgp_view_rsclient,
      ...
      SHOW_STR
      IP_STR
      BGP_STR "Information about Route Server Client\n" NEIGHBOR_ADDR_STR)
      ...
      NEIGHBOR_ADDR_STR "Network in the BGP routing table to display\n")
{
	...
	if (argc == 3) {
		bgp = bgp_lookup_by_name(argv[0]);
		if (bgp == NULL) {
			vty_out(vty, "Can't find BGP view %s%s", argv[0],
				VTY_NEWLINE);
			return CMD_WARNING;
		}
	} else {
	...
	return bgp_show_route_in_table(vty, bgp,
				       peer->rib[AFI_IP][SAFI_UNICAST],
				       (argc == 3) ? argv[2] : argv[1], AFI_IP,
				       SAFI_UNICAST, NULL, 0);
}
      ...
      NEIGHBOR_ADDR_STR "Network in the BGP routing table to display\n")
{
	...
	return bgp_show_route_in_table(vty, bgp,
				       peer->rib[AFI_IP6][SAFI_UNICAST],
				       (argc == 3) ? argv[2] : argv[1], AFI_IP6,
				       SAFI_UNICAST, NULL, 0);
}
...
bgp_distance_apply(struct prefix * p, struct bgp_info * rinfo, struct bgp * bgp)
{
	...
	if (rn) {
		...
		if (bdistance->access_list) {
			alist =
			    access_list_lookup(AFI_IP, bdistance->access_list);
			if (alist
...
ALIAS(bgp_damp_unset,
    ...
    DEFUN(show_ip_bgp_dampened_paths,
      ...
      SHOW_STR IP_STR BGP_STR "Display paths suppressed due to dampening\n")
{
	return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST,
		...
		if ((table = prn->info) != NULL)
			for (rn = bgp_table_top(table); rn;
				...
				if ((bgp_static = rn->info) != NULL) {
					...
					bgp_config_write_family_header(vty, afi,
								       safi,
								       write);
			 ...
			 afi_t afi, safi_t safi, int *write)
{
	...
	if (afi == AFI_IP && safi == SAFI_MPLS_VPN)
		return bgp_config_write_network_vpnv4(vty, bgp, afi, safi,
						      write);
...
int bgp_config_write_distance(struct vty *vty, struct bgp *bgp)
{
	...
	if (bgp->distance_ebgp
		...
		vty_out(vty, " distance bgp %d %d %d%s",
			bgp->distance_ebgp, bgp->distance_ibgp,
			bgp->distance_local, VTY_NEWLINE);

	for (rn = bgp_table_top(bgp_distance_table); rn;
	     rn = bgp_route_next(rn))
		if ((bdistance = rn->info) != NULL) {
			vty_out(vty, " distance %d %s/%d %s%s",
				bdistance->distance, inet_ntoa(rn->p.u.prefix4),
				rn->p.prefixlen,
				bdistance->
				access_list ? bdistance->access_list : "",
				VTY_NEWLINE);
		}
