...
struct ecommunity *ecommunity_dup(struct ecommunity *ecom)
{
	...
	if (new->size) {
		new->val =
		    XMALLOC(MTYPE_ECOMMUNITY_VAL, ecom->size * ECOMMUNITY_SIZE);
...
struct ecommunity *ecommunity_intern(struct ecommunity *ecom)
{
	...
	if (!find->str)
		find->str =
		    ecommunity_ecom2str(find, ECOMMUNITY_FORMAT_DISPLAY);
...
#define ECOMMUNITY_STR_DEFAULT_LEN  27
	...
	for (i = 0; i < ecom->size; i++) {
		...
		while (str_pnt + ECOMMUNITY_STR_DEFAULT_LEN >= str_size) {
			str_size *= 2;
			str_buf =
			    XREALLOC(MTYPE_ECOMMUNITY_STR, str_buf, str_size);
		}
		...
		case ECOMMUNITY_FORMAT_COMMUNITY_LIST:
			prefix =
			    (type == ECOMMUNITY_ROUTE_TARGET ? "rt " : "soo ");
			break;
...
ecommunity_match(const struct ecommunity *ecom1, const struct ecommunity *ecom2)
{
	...
	while (i < ecom1->size && j < ecom2->size) {
		if (memcmp(ecom1->val + i, ecom2->val + j, ECOMMUNITY_SIZE) ==
		    0)
			j++;
		i++;
	}
