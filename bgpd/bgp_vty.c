...
static struct peer *peer_and_group_lookup_vty(struct vty *vty,
					      const char *peer_str)
{
	int ret;
	struct bgp *bgp;
...
DEFUN(bgp_multiple_instance_func,
      ...
      "bgp multiple-instance", BGP_STR "Enable bgp multiple instance\n")
{
...
DEFUN(no_bgp_multiple_instance,
      ...
      "no bgp multiple-instance", NO_STR BGP_STR "BGP multiple instance\n")
{
	int ret;
...
DEFUN(no_bgp_config_type,
      ...
      "no bgp config-type", NO_STR BGP_STR "Display configuration type\n")
{
	bgp_option_unset(BGP_OPT_CONFIG_CISCO);
	return CMD_SUCCESS;
}
...
DEFUN(no_synchronization,
      ...
      "no synchronization", NO_STR "Perform IGP synchronization\n")
{
	return CMD_SUCCESS;
}
...
/* "router bgp" commands. */
DEFUN(router_bgp,
      router_bgp_cmd, "router bgp " CMD_AS_RANGE, ROUTER_STR BGP_STR AS_STR)
{
	int ret;
	as_t as;
	struct bgp *bgp;
	...
	case BGP_ERR_INSTANCE_MISMATCH:
		vty_out(vty, "BGP view name and AS number mismatch%s",
			VTY_NEWLINE);
		vty_out(vty, "BGP instance is already running; AS is %u%s", as,
			VTY_NEWLINE);
		return CMD_WARNING;
	}
...
/* "no router bgp" commands. */
    DEFUN(no_router_bgp,
      no_router_bgp_cmd,
      "no router bgp " CMD_AS_RANGE, NO_STR ROUTER_STR BGP_STR AS_STR)
{
	as_t as;
	struct bgp *bgp;
	const char *name = NULL;
      ...
      NO_STR "Forward packets over multiple paths\n" "Number of paths\n")
{
	...
	ret =
	    bgp_maximum_paths_unset(bgp, bgp_node_afi(vty), bgp_node_safi(vty),
				    BGP_PEER_EBGP);
	if (ret < 0) {
		vty_out(vty,
...
DEFUN(bgp_enforce_first_as,
      ...
      "bgp enforce-first-as", BGP_STR "Enforce the first AS for EBGP routes\n")
{
	struct bgp *bgp;
...
DEFUN(no_bgp_enforce_first_as,
      ...
      NO_STR BGP_STR "Enforce the first AS for EBGP routes\n")
{
	struct bgp *bgp;
...
DEFUN(no_neighbor,
      no_neighbor_cmd, NO_NEIGHBOR_CMD2, NO_STR NEIGHBOR_STR NEIGHBOR_ADDR_STR2)
{
	int ret;
	union sockunion su;
	struct peer_group *group;
...
ALIAS(no_neighbor,
      ...
      NO_STR NEIGHBOR_STR NEIGHBOR_ADDR_STR "Specify a BGP neighbor\n" AS_STR)
...
DEFUN(no_neighbor_password,
      ...
      NO_STR NEIGHBOR_STR NEIGHBOR_ADDR_STR2 "Set a password\n")
{
	struct peer *peer;
	int ret;
      ...
      NEIGHBOR_ADDR_STR "Member of the peer-group\n" "peer-group name\n")
{
	...
	ret = str2sockunion(argv[0], &su);
	if (ret < 0) {
		vty_out(vty, "%% Malformed address: %s%s", argv[0],
			VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(neighbor_passive,
      ...
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR2 "Don't send open messages to this neighbor\n")
{
...
DEFUN(neighbor_shutdown,
      ...
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR2 "Administratively shut down this neighbor\n")
{
...
DEFUN(neighbor_remove_private_as,
      ...
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR2 "Remove private AS number from outbound updates\n")
{
	return peer_af_flag_set_vty(vty, argv[0], bgp_node_afi(vty),
				    bgp_node_safi(vty),
...
DEFUN(neighbor_send_community,
      ...
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR2 "Send Community attribute to this neighbor\n")
{
	return peer_af_flag_set_vty(vty, argv[0], bgp_node_afi(vty),
				    bgp_node_safi(vty),
...
DEFUN(neighbor_route_reflector_client,
      ...
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR2 "Configure a neighbor as Route Reflector client\n")
{
	struct peer *peer;
...
static int
peer_rsclient_set_vty(struct vty *vty, const char *peer_str, int afi, int safi)
{
	int ret;
	struct bgp *bgp;
	struct peer *peer;
	...
	if (CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP)) {
		...
		for (ALL_LIST_ELEMENTS(group->peer, node, nnode, peer)) {
			...
			if (gfilter->map[RMAP_IMPORT].name) {
				...
				pfilter->map[RMAP_IMPORT].map =
				    gfilter->map[RMAP_IMPORT].map;
			} else {
			...
			/* Export policy. */
			if (gfilter->map[RMAP_EXPORT].name
			    && !pfilter->map[RMAP_EXPORT].name) {
				pfilter->map[RMAP_EXPORT].name =
				    strdup(gfilter->map[RMAP_EXPORT].name);
				pfilter->map[RMAP_EXPORT].map =
				    gfilter->map[RMAP_EXPORT].map;
			}
		}
	}
	return CMD_SUCCESS;
...
DEFUN(neighbor_route_server_client,
      ...
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR2 "Configure a neighbor as Route Server client\n")
{
...
/* disable-connected-check */
    DEFUN(neighbor_disable_connected_check,
      ...
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR2 "one-hop away EBGP peer using loopback address\n")
{
	return peer_flag_set_vty(vty, argv[0],
...
DEFUN(no_neighbor_disable_connected_check,
      ...
      NEIGHBOR_ADDR_STR2 "one-hop away EBGP peer using loopback address\n")
{
	return peer_flag_unset_vty(vty, argv[0],
				   PEER_FLAG_DISABLE_CONNECTED_CHECK);
}
...
ALIAS(neighbor_disable_connected_check,
      ...
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR2 "Enforce EBGP neighbors perform multihop\n")
...
DEFUN(neighbor_default_originate,
      ...
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR2 "Originate default route to this neighbor\n")
{
...
DEFUN(neighbor_port,
      ...
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR "Neighbor's BGP port\n" "TCP port number\n")
{
...
/* neighbor weight. */
static int
peer_weight_set_vty(struct vty *vty, const char *ip_str, const char *weight_str)
{
	struct peer *peer;
...
DEFUN(no_neighbor_weight,
      ...
      NO_STR
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR2 "Set default weight for routes from this neighbor\n")
{
...
/* Override capability negotiation. */
    DEFUN(neighbor_override_capability,
      ...
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR2 "Override capability negotiation result\n")
{
...
DEFUN(neighbor_strict_capability,
      ...
      NEIGHBOR_STR NEIGHBOR_ADDR_STR "Strict capability negotiation match\n")
{
...
DEFUN(no_neighbor_timers_connect,
      ...
      NO_STR
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR "BGP per neighbor timers\n" "BGP connect timer\n")
{
...
peer_aslist_unset_vty(struct vty *vty, const char *ip_str,
		      afi_t afi, safi_t safi, const char *direct_str)
{
	int ret;
	struct peer *peer;
	int direct = FILTER_IN;
			    ...
			    const char *restart_str)
{
	...
	ret =
	    peer_maximum_prefix_set(peer, afi, safi, max, threshold, warning,
				    restart);
...
/* "neighbor allowas-in" */
    DEFUN(neighbor_allowas_in,
      ...
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR2 "Accept as-path with my AS present in it\n")
{
	int ret;
	struct peer *peer;
...
ALIAS(neighbor_allowas_in,
    ...
    DEFUN(no_neighbor_allowas_in,
      ...
      NO_STR
      NEIGHBOR_STR
      NEIGHBOR_ADDR_STR2 "allow local ASN appears in aspath attribute\n")
{
	int ret;
	struct peer *peer;
	...
	ret =
	    peer_allowas_in_unset(peer, bgp_node_afi(vty), bgp_node_safi(vty));
		    ...
		    safi_t safi, int error)
{
	...
	case BGP_ERR_AF_UNCONFIGURED:
		vty_out(vty,
			...
			afi == AFI_IP6 ? "IPv6" : safi ==
			SAFI_MPLS_VPN ? "VPNv4" : "IPv4",
	  ...
	  enum clear_sort sort, enum bgp_clear_type stype, const char *arg)
{
	...
	if (sort == clear_peer) {
		...
		if (ret < 0) {
			vty_out(vty, "Malformed address: %s%s", arg,
				VTY_NEWLINE);
			return CMD_WARNING;
		}
		peer = peer_lookup(bgp, &su);
		if (!peer) {
			vty_out(vty, "%%BGP: Unknown neighbor - \"%s\"%s", arg,
				VTY_NEWLINE);
			return CMD_WARNING;
		}
	      ...
	      enum clear_sort sort, enum bgp_clear_type stype, const char *arg)
{
	...
	if (name) {
		...
		if (bgp == NULL) {
			vty_out(vty, "Can't find BGP view %s%s", name,
				VTY_NEWLINE);
			return CMD_WARNING;
		}
	} else {
...
DEFUN(clear_ip_bgp_all,
      ...
      "clear ip bgp *", CLEAR_STR IP_STR BGP_STR "Clear all peers\n")
{
	if (argc == 1)
		return bgp_clear_vty(vty, argv[0], 0, 0, clear_all,
				     BGP_CLEAR_SOFT_NONE, NULL);

	return bgp_clear_vty(vty, NULL, 0, 0, clear_all, BGP_CLEAR_SOFT_NONE,
			     NULL);
}

ALIAS(clear_ip_bgp_all,
      clear_bgp_all_cmd, "clear bgp *", CLEAR_STR BGP_STR "Clear all peers\n")
...
{
	return bgp_clear_vty(vty, NULL, 0, 0, clear_peer, BGP_CLEAR_SOFT_NONE,
			     argv[0]);
}
...
ALIAS(clear_ip_bgp_peer,
      ...
      CLEAR_STR
      BGP_STR "BGP neighbor address to clear\n" "BGP IPv6 neighbor to clear\n")
    ...
    DEFUN(clear_ip_bgp_peer_group,
      ...
      CLEAR_STR
      IP_STR
      BGP_STR "Clear all members of peer-group\n" "BGP peer-group name\n")
{
	return bgp_clear_vty(vty, NULL, 0, 0, clear_group, BGP_CLEAR_SOFT_NONE,
			     argv[0]);
}
...
{
	return bgp_clear_vty(vty, NULL, 0, 0, clear_external,
			     BGP_CLEAR_SOFT_NONE, NULL);
}
...
{
	return bgp_clear_vty(vty, NULL, 0, 0, clear_as, BGP_CLEAR_SOFT_NONE,
			     argv[0]);
}
...
{
	if (argc == 1)
		return bgp_clear_vty(vty, argv[0], AFI_IP, SAFI_UNICAST,
				     clear_all, BGP_CLEAR_SOFT_OUT, NULL);
...
ALIAS(clear_ip_bgp_all_soft_out,
      ...
      "clear ip bgp * out",
      CLEAR_STR
      IP_STR BGP_STR "Clear all peers\n" "Soft reconfig outbound update\n")
...
{
	if (strncmp(argv[0], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_all, BGP_CLEAR_SOFT_OUT, NULL);
      ...
      "Address Family modifier\n" "Soft reconfig outbound update\n")
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_clear_vty(vty, argv[0], AFI_IP, SAFI_MULTICAST,
				     clear_all, BGP_CLEAR_SOFT_OUT, NULL);
...
{
	if (argc == 1)
		return bgp_clear_vty(vty, argv[0], AFI_IP6, SAFI_UNICAST,
				     clear_all, BGP_CLEAR_SOFT_OUT, NULL);
...
ALIAS(clear_bgp_all_soft_out,
    ...
    ALIAS(clear_bgp_all_soft_out,
      ...
      "clear bgp * out",
      CLEAR_STR BGP_STR "Clear all peers\n" "Soft reconfig outbound update\n")
      ...
      "Soft reconfig\n" "Soft reconfig outbound update\n")
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_group, BGP_CLEAR_SOFT_OUT, argv[0]);
...
ALIAS(clear_ip_bgp_external_soft_out,
      ...
      CLEAR_STR
      IP_STR
      BGP_STR "Clear all external peers\n" "Soft reconfig outbound update\n")
      ...
      "Soft reconfig\n" "Soft reconfig outbound update\n")
{
	if (strncmp(argv[0], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_external, BGP_CLEAR_SOFT_OUT, NULL);
      ...
      "Soft reconfig\n" "Soft reconfig outbound update\n")
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_as, BGP_CLEAR_SOFT_OUT, argv[0]);
      ...
      "Push out prefix-list ORF and do inbound soft reconfig\n")
{
	if (argc == 1)
		return bgp_clear_vty(vty, argv[0], AFI_IP, SAFI_UNICAST,
				     clear_all, BGP_CLEAR_SOFT_IN_ORF_PREFIX,
				     NULL);
      ...
      "Soft reconfig\n" "Soft reconfig inbound update\n")
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_peer, BGP_CLEAR_SOFT_IN, argv[0]);
      ...
      "Push out the existing ORF prefix-list\n")
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_peer, BGP_CLEAR_SOFT_IN_ORF_PREFIX,
				     argv[0]);
      ...
      "Soft reconfig\n" "Soft reconfig inbound update\n")
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_group, BGP_CLEAR_SOFT_IN, argv[0]);
      ...
      "Push out prefix-list ORF and do inbound soft reconfig\n")
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_group, BGP_CLEAR_SOFT_IN_ORF_PREFIX,
				     argv[0]);
      ...
      "Soft reconfig\n" "Soft reconfig inbound update\n")
{
	if (strncmp(argv[0], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_external, BGP_CLEAR_SOFT_IN, NULL);
      ...
      "Soft reconfig\n" "Soft reconfig inbound update\n")
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_as, BGP_CLEAR_SOFT_IN, argv[0]);
      ...
      "Push out prefix-list ORF and do inbound soft reconfig\n")
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_as, BGP_CLEAR_SOFT_IN_ORF_PREFIX,
				     argv[0]);
      ...
      CLEAR_STR IP_STR BGP_STR "Clear all peers\n" "Soft reconfig\n")
{
	if (argc == 1)
		return bgp_clear_vty(vty, argv[0], AFI_IP, SAFI_UNICAST,
				     clear_all, BGP_CLEAR_SOFT_BOTH, NULL);
      ...
      CLEAR_STR BGP_STR "Clear all peers\n" "Soft reconfig\n")
{
	if (argc == 1)
		return bgp_clear_vty(vty, argv[0], AFI_IP6, SAFI_UNICAST,
				     clear_all, BGP_CLEAR_SOFT_BOTH, argv[0]);
...
ALIAS(clear_bgp_all_soft,
    ...
    DEFUN(clear_ip_bgp_peer_soft,
      ...
      CLEAR_STR
      IP_STR BGP_STR "BGP neighbor address to clear\n" "Soft reconfig\n")
{
	return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_UNICAST, clear_peer,
			     BGP_CLEAR_SOFT_BOTH, argv[0]);
}
      ...
      "Address Family Modifier\n" "Address Family Modifier\n" "Soft reconfig\n")
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_peer, BGP_CLEAR_SOFT_BOTH, argv[0]);
      ...
      "Address Family modifier\n" "Address Family modifier\n" "Soft reconfig\n")
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_group, BGP_CLEAR_SOFT_BOTH, argv[0]);
      ...
      "Address Family modifier\n" "Address Family modifier\n" "Soft reconfig\n")
{
	if (strncmp(argv[0], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_external, BGP_CLEAR_SOFT_BOTH, NULL);
...
ALIAS(clear_bgp_external_soft,
    ...
    DEFUN(clear_ip_bgp_as_soft,
      ...
      CLEAR_STR
      IP_STR BGP_STR "Clear peers with the AS number\n" "Soft reconfig\n")
{
	return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_UNICAST, clear_as,
			     BGP_CLEAR_SOFT_BOTH, argv[0]);
}
      ...
      "Address Family Modifier\n" "Address Family Modifier\n" "Soft reconfig\n")
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_clear_vty(vty, NULL, AFI_IP, SAFI_MULTICAST,
				     clear_as, BGP_CLEAR_SOFT_BOTH, argv[0]);
      ...
      CLEAR_STR BGP_STR "Clear all peers\n" "Soft reconfig for rsclient RIB\n")
{
	if (argc == 1)
		return bgp_clear_vty(vty, argv[0], AFI_IP6, SAFI_UNICAST,
				     clear_all, BGP_CLEAR_SOFT_RSCLIENT, NULL);
      ...
      "BGP IPv6 neighbor to clear\n" "Soft reconfig for rsclient RIB\n")
{
	if (argc == 2)
		return bgp_clear_vty(vty, argv[0], AFI_IP6, SAFI_UNICAST,
				     clear_peer, BGP_CLEAR_SOFT_RSCLIENT,
				     argv[1]);
...
ALIAS(clear_ip_bgp_peer_rsclient,
    ...
    DEFUN(show_bgp_views,
      show_bgp_views_cmd,
      "show bgp views", SHOW_STR BGP_STR "Show the defined BGP views\n")
{
	...
	if (!bgp_option_check(BGP_OPT_MULTIPLE_INSTANCE)) {
		vty_out(vty, "Multiple BGP views are not defined%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	for (ALL_LIST_ELEMENTS_RO(inst, node, bgp))
		vty_out(vty, "\t%s (AS%u)%s",
			bgp->name ? bgp->name : "(null)", bgp->as, VTY_NEWLINE);
      ...
      "show bgp memory", SHOW_STR BGP_STR "Global BGP memory statistics\n")
{
	...
	vty_out(vty, "%ld RIB nodes, using %s of memory%s", count,
		mtype_memstr(memstrbuf, sizeof(memstrbuf),
			     count * sizeof(struct bgp_node)), VTY_NEWLINE);

	count = mtype_stats_alloc(MTYPE_BGP_ROUTE);
	vty_out(vty, "%ld BGP routes, using %s of memory%s", count,
		mtype_memstr(memstrbuf, sizeof(memstrbuf),
			     count * sizeof(struct bgp_info)), VTY_NEWLINE);
	...
	vty_out(vty, "%ld BGP attributes, using %s of memory%s", count,
		mtype_memstr(memstrbuf, sizeof(memstrbuf),
			     count * sizeof(struct attr)), VTY_NEWLINE);
	...
	vty_out(vty, "%ld peers, using %s of memory%s", count,
		mtype_memstr(memstrbuf, sizeof(memstrbuf),
			     count * sizeof(struct peer)), VTY_NEWLINE);
	...
	if ((count = mtype_stats_alloc(MTYPE_HASH)))
		vty_out(vty, "%ld hash tables, using %s of memory%s", count,
			mtype_memstr(memstrbuf, sizeof(memstrbuf),
				     count * sizeof(struct hash)), VTY_NEWLINE);
	...
	for (ALL_LIST_ELEMENTS(bgp->peer, node, nnode, peer)) {
		if (peer->afc[afi][safi]) {
			if (!count) {
				...
				if (CHECK_FLAG
				    (bgp->af_flags[afi][safi],
				     BGP_CONFIG_DAMPENING))
					vty_out(vty, "Dampening enabled.%s",
						VTY_NEWLINE);
			...
			vty_out(vty, "%5u %7d %7d %8d %4d %4lu ",
				...
				peer->refresh_out + peer->dynamic_cap_out, 0, 0,
...
static int
bgp_show_summary_vty(struct vty *vty, const char *name, afi_t afi, safi_t safi)
{
	struct bgp *bgp;
	...
	if (name) {
		...
		if (!bgp) {
			vty_out(vty, "%% No such BGP instance exist%s",
				VTY_NEWLINE);
			return CMD_WARNING;
		}
...
DEFUN(show_ip_bgp_summary,
      ...
      "show ip bgp summary",
      SHOW_STR IP_STR BGP_STR "Summary of BGP neighbor status\n")
{
...
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_show_summary_vty(vty, argv[0], AFI_IP,
					    SAFI_MULTICAST);
	else
		return bgp_show_summary_vty(vty, argv[0], AFI_IP, SAFI_UNICAST);
}
...
DEFUN(show_bgp_summary,
      ...
      "show bgp summary", SHOW_STR BGP_STR "Summary of BGP neighbor status\n")
{
...
{
	if (strncmp(argv[1], "m", 1) == 0)
		return bgp_show_summary_vty(vty, argv[0], AFI_IP6,
					    SAFI_MULTICAST);
...
DEFUN(show_ipv6_bgp_summary,
      ...
      SHOW_STR IPV6_STR BGP_STR "Summary of BGP neighbor status\n")
{
	return bgp_show_summary_vty(vty, NULL, AFI_IP6, SAFI_UNICAST);
}
...
DEFUN(show_ipv6_mbgp_summary,
      ...
      SHOW_STR IPV6_STR MBGP_STR "Summary of BGP neighbor status\n")
{
	return bgp_show_summary_vty(vty, NULL, AFI_IP6, SAFI_MULTICAST);
}
#endif /* HAVE_IPV6 */
...
bgp_show_peer_afi(struct vty *vty, struct peer *p, afi_t afi, safi_t safi)
{
	...
	if (CHECK_FLAG(p->af_sflags[afi][safi], PEER_STATUS_ORF_PREFIX_SEND)
		...
		if (CHECK_FLAG
		    (p->af_sflags[afi][safi], PEER_STATUS_ORF_PREFIX_SEND))
			vty_out(vty, " sent;");
		if (orf_pfx_count)
	...
	if (CHECK_FLAG(p->af_flags[afi][safi], PEER_FLAG_SEND_COMMUNITY)
		...
		if (CHECK_FLAG(p->af_flags[afi][safi], PEER_FLAG_SEND_COMMUNITY)
			...
			vty_out(vty, "(both)%s", VTY_NEWLINE);
			 ...
			 (p->af_flags[afi][safi], PEER_FLAG_SEND_EXT_COMMUNITY))
	...
	if (CHECK_FLAG(p->af_flags[afi][safi], PEER_FLAG_DEFAULT_ORIGINATE)) {
		...
		if (CHECK_FLAG
		    (p->af_sflags[afi][safi], PEER_STATUS_DEFAULT_ORIGINATE))
	...
	if (CHECK_FLAG(p->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX)) {
		vty_out(vty, "  Maximum prefixes allowed %ld%s%s",
			p->pmax[afi][safi], CHECK_FLAG(p->af_flags[afi][safi],
...
static void bgp_show_peer(struct vty *vty, struct peer *p)
{
	...
	vty_out(vty, "%s link%s",
		p->as == p->local_as ? "internal" : "external", VTY_NEWLINE);
	...
	vty_out(vty, ", remote router ID %s%s",
		inet_ntop(AF_INET, &p->remote_id, buf1, BUFSIZ), VTY_NEWLINE);
	...
	if (p->su_local) {
		...
		vty_out(vty, "BGP connection: %s%s",
			...
			"non shared network", VTY_NEWLINE);
#endif /* HAVE_IPV6 */
	}
		      ...
		      enum show_type type, const char *ip_str)
{
	...
	if (ip_str) {
		ret = str2sockunion(ip_str, &su);
		if (ret < 0) {
			vty_out(vty, "%% Malformed address: %s%s", ip_str,
				VTY_NEWLINE);
			return CMD_WARNING;
		}
	}
...
DEFUN(show_ip_bgp_neighbors,
      ...
      SHOW_STR
      IP_STR
      BGP_STR "Detailed information on TCP and BGP neighbor connections\n")
{
...
/* Show BGP's AS paths internal data.  There are both `show ip bgp
   ...
   same.*/
    DEFUN(show_ip_bgp_paths,
      ...
      "show ip bgp paths", SHOW_STR IP_STR BGP_STR "Path information\n")
{
      ...
      SHOW_STR IP_STR BGP_STR "List all bgp community information\n")
{
	...
	hash_iterate(community_hash(), (void (*)(struct hash_backet *, void *))
		     community_show_all_iterator, vty);
			   ...
			   afi_t afi, safi_t safi)
{
	...
	if (CHECK_FLAG(rsclient->sflags, PEER_STATUS_GROUP)) {
		for (ALL_LIST_ELEMENTS
		     (rsclient->group->peer, node, nnode, peer)) {
			count++;
	...
	vty_out(vty, "%8s",
		peer_uptime(rsclient->uptime, timebuf, BGP_UPTIME_LEN));
	...
	if (CHECK_FLAG(rsclient->flags, PEER_FLAG_SHUTDOWN))
		...
		vty_out(vty, " %-11s",
			LOOKUP(bgp_status_msg, rsclient->status));
			  ...
			  afi_t afi, safi_t safi)
{
	...
	if (count)
		vty_out(vty, "%sTotal number of Route Server Clients %d%s",
...
DEFUN(show_ip_bgp_instance_rsclient_summary,
      ...
      "Summary of all Route Server Clients\n")
{
	return bgp_show_rsclient_summary_vty(vty, argv[0], AFI_IP,
					     SAFI_UNICAST);
}
...
{
	if (strncmp(argv[0], "m", 1) == 0)
		return bgp_show_rsclient_summary_vty(vty, NULL, AFI_IP,
						     SAFI_MULTICAST);
...
DEFUN(show_bgp_instance_rsclient_summary,
      ...
      "Summary of all Route Server Clients\n")
{
	return bgp_show_rsclient_summary_vty(vty, argv[0], AFI_IP6,
					     SAFI_UNICAST);
}
...
{
	return community_list_unset_vty(vty, argc, argv,
		...
		if (entry == list->head) {
			if (all_digit(list->name))
				vty_out(vty, "Community %s list %s%s",
					entry->style ==
					COMMUNITY_LIST_STANDARD ? "standard" :
				...
				vty_out(vty, "Named Community %s list %s%s",
					entry->style ==
					COMMUNITY_LIST_STANDARD ? "standard" :
					"expanded", list->name, VTY_NEWLINE);
		}
		if (entry->any)
			vty_out(vty, "    %s%s",
...
DEFUN(show_ip_community_list,
      ...
      "show ip community-list", SHOW_STR IP_STR "List community-list\n")
{
...
DEFUN(no_ip_extcommunity_list_expanded_all,
      ...
      NO_STR
      IP_STR
      EXTCOMMUNITY_LIST_STR "Extended Community list number (expanded)\n")
{
	return extcommunity_list_unset_vty(vty, argc, argv,
		...
		if (entry == list->head) {
			if (all_digit(list->name))
				vty_out(vty, "Extended community %s list %s%s",
					entry->style ==
					EXTCOMMUNITY_LIST_STANDARD ? "standard"
					: "(expanded) access", list->name,
					VTY_NEWLINE);
			else
				vty_out(vty,
      ...
      "Extcommunity-list number\n" "Extcommunity-list name\n")
{
	...
	list =
	    community_list_lookup(bgp_clist, argv[0], EXTCOMMUNITY_LIST_MASTER);
	if (!list) {
