...
static int bgp_reuse_index(int penalty)
{
	...
	i = (int)(((double)penalty / damp->reuse_limit -
		  ...
		  afi_t afi, safi_t safi, int attr_change)
{
	...
	if (bdi == NULL) {
		...
		bdi =
		    XCALLOC(MTYPE_BGP_DAMP_INFO, sizeof(struct bgp_damp_info));
		bdi->binfo = binfo;
		bdi->rn = rn;
		bdi->penalty =
...
static void bgp_damp_parameter_set(int hlife, int reuse, int sup, int maxsup)
{
	...
	damp->ceiling =
	    (int)(damp->reuse_limit *
		  (pow(2, (double)damp->max_suppress_time / damp->half_life)));
		...
		unsigned int reuse, unsigned int suppress, time_t max)
{
	...
	if (!damp->t_reuse)
		damp->t_reuse =
		    thread_add_timer(master, bgp_reuse_timer, NULL,
				     DELTA_REUSE);
