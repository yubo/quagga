...
#define BGP_PeerAdmin_start 2
		...
		else if (intval == BGP_PeerAdmin_start) ;	/* Do nothing. */
		else
			return SNMP_ERR_NOSUCHNAME;
		break;
...
#define BGP_PeerAdmin_start 2
	...
	case BGPPEERLOCALPORT:
		if (peer->su_local)
			return
			    SNMP_INTEGER(ntohs(peer->su_local->sin.sin_port));
		else
	...
	case BGPPEERREMOTEPORT:
		if (peer->su_remote)
			return
			    SNMP_INTEGER(ntohs(peer->su_remote->sin.sin_port));
		else
			return SNMP_INTEGER(0);
		break;
...
static u_char *bgp4PathAttrTable(struct variable *v, oid name[],
				 ...
				 WriteMethod ** write_method)
{
	...
	case BGP4PATHATTRAGGREGATORADDR:	/* 11 */
		if (binfo->attr->extra)
			return SNMP_IPADDRESS(binfo->attr->
					      extra->aggregator_addr);
		else
			return SNMP_INTEGER(0);
		break;
