...
int peer_rsclient_active(struct peer *peer)
{
	...
	for (i = AFI_IP; i < AFI_MAX; i++)
		for (j = SAFI_UNICAST; j < SAFI_MAX; j++)
			if (CHECK_FLAG
			    (peer->af_flags[i][j], PEER_FLAG_RSERVER_CLIENT)
			    && !peer->af_group[i][j])
				return 1;
	return 0;
}
...
static bgp_peer_sort_t peer_calc_sort(struct peer *peer)
{
	...
	if (CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP)) {
		if (peer->as)
			return (bgp->as ==
				peer->as ? BGP_PEER_IBGP : BGP_PEER_EBGP);
		else {
			struct peer *peer1;
...
struct peer *peer_lock(struct peer *peer)
{
 ...
 */
struct peer *peer_unlock(struct peer *peer)
{
	assert(peer && (peer->lock > 0));
...
/* Create new BGP peer.  */
static struct peer *peer_create(union sockunion *su, struct bgp *bgp,
				as_t local_as, as_t remote_as, afi_t afi,
				safi_t safi)
{
	int active;
	struct peer *peer;
			    ...
			    afi_t afi, safi_t safi)
{
	...
	if (conf->default_rmap[afi][safi].name) {
		...
		peer->default_rmap[afi][safi].name =
		    strdup(conf->default_rmap[afi][safi].name);
	...
	if (gfilter->map[RMAP_OUT].name) {
		...
		pfilter->map[RMAP_OUT].name =
		    strdup(gfilter->map[RMAP_OUT].name);
		...
		struct peer_group *group, afi_t afi, safi_t safi, as_t * as)
{
	...
	if (!peer) {
		...
		peer =
		    peer_create(su, bgp, bgp->as, group->conf->as, afi, safi);
		peer->group = group;
	...
	if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT)) {
		...
		/* Export policy. */
		if (!CHECK_FLAG
		    (group->conf->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT)
...
struct bgp *bgp_lookup(as_t as, const char *name)
{
	...
	for (ALL_LIST_ELEMENTS(bm->bgp, node, nnode, bgp))
		if (bgp->as == as && ((bgp->name == NULL && name == NULL)
				      || (bgp->name && name
					  && strcmp(bgp->name, name) == 0)))
			return bgp;
	return NULL;
}
...
struct peer *peer_lookup_with_open(union sockunion *su, as_t remote_as,
...
static int peer_flag_modify(struct peer *peer, u_int32_t flag, int set)
{
	...
	found =
	    peer_flag_action_set(peer_flag_action_list, size, &action, flag);
		    ...
		    int set)
{
	...
	memset(&action, 0, sizeof(struct peer_flag_action));
	size =
	    sizeof peer_af_flag_action_list / sizeof(struct peer_flag_action);

	found =
	    peer_flag_action_set(peer_af_flag_action_list, size, &action, flag);
	...
	if (CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP)) {
		...
		for (ALL_LIST_ELEMENTS(group->peer, node, nnode, peer)) {
			...
			if (!set
			    && !CHECK_FLAG(peer->af_flags[afi][safi], flag))
				continue;
...
int peer_password_set(struct peer *peer, const char *password)
{
	...
	if (!CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP)) {
		...
		return (bgp_md5_set(peer) >=
			0) ? BGP_SUCCESS : BGP_ERR_TCPSIG_FAILED;
	}
	...
	if (peer->af_group[afi][safi]) {
		...
		if (gfilter->dlist[direct].name) {
			...
			filter->dlist[direct].name =
			    strdup(gfilter->dlist[direct].name);
	...
	if (peer->af_group[afi][safi]) {
		...
		if (gfilter->plist[direct].name) {
			...
			filter->plist[direct].name =
			    strdup(gfilter->plist[direct].name);
	...
	if (peer->af_group[afi][safi]) {
		...
		if (gfilter->aslist[direct].name) {
			...
			filter->aslist[direct].name =
			    strdup(gfilter->aslist[direct].name);
	...
	if (peer->af_group[afi][safi]) {
		...
		if (gfilter->map[direct].name) {
			...
			filter->map[direct].name =
			    strdup(gfilter->map[direct].name);
		...
		enum bgp_clear_type stype)
{
	...
	if (stype == BGP_CLEAR_SOFT_RSCLIENT) {
		if (!CHECK_FLAG
		    (peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT))
			return 0;
	...
	if (stype == BGP_CLEAR_SOFT_IN_ORF_PREFIX) {
		if (CHECK_FLAG
		    (peer->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_ADV)
		    &&
		    (CHECK_FLAG
	...
	if (stype == BGP_CLEAR_SOFT_IN || stype == BGP_CLEAR_SOFT_BOTH
		...
		if (CHECK_FLAG
		    (peer->af_flags[afi][safi], PEER_FLAG_SOFT_RECONFIG))
			...
			afi_t afi, safi_t safi)
{
	...
	if (filter->dlist[in].name)
		if (!gfilter || !gfilter->dlist[in].name
		    || strcmp(filter->dlist[in].name,
			      gfilter->dlist[in].name) != 0)
			vty_out(vty, " neighbor %s distribute-list %s in%s",
	...
	if (filter->plist[in].name)
		if (!gfilter || !gfilter->plist[in].name
		    || strcmp(filter->plist[in].name,
			      gfilter->plist[in].name) != 0)
	...
	if (filter->aslist[in].name)
		if (!gfilter || !gfilter->aslist[in].name
		    || strcmp(filter->aslist[in].name,
			      gfilter->aslist[in].name) != 0)
		      ...
		      struct peer *peer, afi_t afi, safi_t safi)
{
   ...
   ************************************/
	if (afi == AFI_IP && safi == SAFI_UNICAST) {
		...
		if (!peer_group_active(peer)) {
			if (CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP))
				vty_out(vty, " neighbor %s peer-group%s", addr,
					VTY_NEWLINE);
			if (peer->as)
				vty_out(vty, " neighbor %s remote-as %u%s",
					addr, peer->as, VTY_NEWLINE);
		} else {
			if (!g_peer->as)
				vty_out(vty, " neighbor %s remote-as %u%s",
					addr, peer->as, VTY_NEWLINE);
			if (peer->af_group[AFI_IP][SAFI_UNICAST])
				vty_out(vty, " neighbor %s peer-group %s%s",
					addr, peer->group->name, VTY_NEWLINE);
		}

		/* local-as. */
		if (peer->change_local_as)
			if (!peer_group_active(peer))
				vty_out(vty, " neighbor %s local-as %u%s%s%s",
					addr, peer->change_local_as,
		...
		/* Description. */
		if (peer->desc)
			vty_out(vty, " neighbor %s description %s%s", addr,
				peer->desc, VTY_NEWLINE);

		/* Shutdown. */
		if (CHECK_FLAG(peer->flags, PEER_FLAG_SHUTDOWN))
			if (!peer_group_active(peer) ||
			    !CHECK_FLAG(g_peer->flags, PEER_FLAG_SHUTDOWN))
				vty_out(vty, " neighbor %s shutdown%s", addr,
					VTY_NEWLINE);
		...
		if (peer->password)
			if (!peer_group_active(peer)
				...
				vty_out(vty, " neighbor %s password %s%s", addr,
					peer->password, VTY_NEWLINE);
		...
		if (peer->ifname)
			vty_out(vty, " neighbor %s interface %s%s", addr,
				peer->ifname, VTY_NEWLINE);
		...
		if (peer->sort != BGP_PEER_IBGP && peer->ttl != 1 &&
			...
			if (!peer_group_active(peer) ||
			    g_peer->ttl != peer->ttl)
				vty_out(vty, " neighbor %s ebgp-multihop %d%s",
					addr, peer->ttl, VTY_NEWLINE);
		...
		if (peer->gtsm_hops != 0)
			if (!peer_group_active(peer)
				...
				vty_out(vty,
					...
					addr, peer->gtsm_hops, VTY_NEWLINE);
		...
		if (peer->update_if)
			if (!peer_group_active(peer) || !g_peer->update_if
				...
				vty_out(vty, " neighbor %s update-source %s%s",
					addr, peer->update_if, VTY_NEWLINE);
		if (peer->update_source)
			if (!peer_group_active(peer) || !g_peer->update_source
			    || sockunion_cmp(g_peer->update_source,
					     peer->update_source) != 0)
				vty_out(vty, " neighbor %s update-source %s%s",
					addr, sockunion2str(peer->update_source,
							    buf, SU_ADDRSTRLEN),
					VTY_NEWLINE);
		...
		if (CHECK_FLAG(peer->config, PEER_CONFIG_WEIGHT))
			if (!peer_group_active(peer) ||
				...
				vty_out(vty, " neighbor %s weight %d%s", addr,
					peer->weight, VTY_NEWLINE);
		...
		if (!peer->af_group[AFI_IP][SAFI_UNICAST]) {
			if (bgp_flag_check(bgp, BGP_FLAG_NO_DEFAULT_IPV4)) {
				if (peer->afc[AFI_IP][SAFI_UNICAST])
					vty_out(vty, " neighbor %s activate%s",
						addr, VTY_NEWLINE);
			} else {
				if (!peer->afc[AFI_IP][SAFI_UNICAST])
					vty_out(vty,
   ...
   ************************************/
	...
	if (!(afi == AFI_IP && safi == SAFI_UNICAST)) {
		if (peer->af_group[afi][safi])
			...
			vty_out(vty, " neighbor %s activate%s", addr,
				VTY_NEWLINE);
	}
	...
	if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_ORF_PREFIX_SM)
		...
		if (!peer->af_group[afi][safi]) {
			vty_out(vty, " neighbor %s capability orf prefix-list",
				addr);

			if (CHECK_FLAG
			    (peer->af_flags[afi][safi], PEER_FLAG_ORF_PREFIX_SM)
			    && CHECK_FLAG(peer->af_flags[afi][safi],
					  PEER_FLAG_ORF_PREFIX_RM))
				vty_out(vty, " both");
			else if (CHECK_FLAG
	...
	if (peer_af_flag_check(peer, afi, safi, PEER_FLAG_NEXTHOP_SELF)
		...
		vty_out(vty, " neighbor %s next-hop-self%s%s", addr,
			peer_af_flag_check(peer, afi, safi,
					   PEER_FLAG_NEXTHOP_SELF_ALL) ? " all"
			: "", VTY_NEWLINE);
	...
	if (!peer->af_group[afi][safi]) {
		if (bgp_option_check(BGP_OPT_CONFIG_CISCO)) {
			if (peer_af_flag_check
			    (peer, afi, safi, PEER_FLAG_SEND_COMMUNITY)
			    && peer_af_flag_check(peer, afi, safi,
						  PEER_FLAG_SEND_EXT_COMMUNITY))
				vty_out(vty,
					" neighbor %s send-community both%s",
					addr, VTY_NEWLINE);
			else if (peer_af_flag_check
				 (peer, afi, safi,
				  PEER_FLAG_SEND_EXT_COMMUNITY))
				vty_out(vty,
				 ...
				 (peer, afi, safi, PEER_FLAG_SEND_COMMUNITY))
				vty_out(vty, " neighbor %s send-community%s",
					addr, VTY_NEWLINE);
		} else {
			if (!peer_af_flag_check
			    (peer, afi, safi, PEER_FLAG_SEND_COMMUNITY)
	...
	if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX))
		if (!peer->af_group[afi][safi]
		    ...
		    != CHECK_FLAG(peer->af_flags[afi][safi],
				  PEER_FLAG_MAX_PREFIX_WARNING)) {
			vty_out(vty, " neighbor %s maximum-prefix %ld", addr,
				peer->pmax[afi][safi]);
	...
	if (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT)
	    && !peer->af_group[afi][safi])
		vty_out(vty, " neighbor %s route-server-client%s", addr,
			VTY_NEWLINE);

	/* Nexthop-local unchanged. */
	if (CHECK_FLAG
	    (peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_LOCAL_UNCHANGED)
	    && !peer->af_group[afi][safi])
		vty_out(vty, " neighbor %s nexthop-local unchanged%s", addr,
			VTY_NEWLINE);
	...
	if (peer_af_flag_check(peer, afi, safi, PEER_FLAG_ALLOWAS_IN))
		if (!peer_group_active(peer)
			...
			if (peer->allowas_in[afi][safi] == 3)
				vty_out(vty, " neighbor %s allowas-in%s", addr,
					VTY_NEWLINE);
			else
				vty_out(vty, " neighbor %s allowas-in %d%s",
	...
	if ((CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_AS_PATH_UNCHANGED)
	    ...
	    && !peer->af_group[afi][safi]) {
		if (CHECK_FLAG
		    (peer->af_flags[afi][safi], PEER_FLAG_AS_PATH_UNCHANGED)
		    && CHECK_FLAG(peer->af_flags[afi][safi],
				  PEER_FLAG_NEXTHOP_UNCHANGED)
		    && CHECK_FLAG(peer->af_flags[afi][safi],
				  PEER_FLAG_MED_UNCHANGED))
			vty_out(vty, " neighbor %s attribute-unchanged%s", addr,
				VTY_NEWLINE);
		else
			vty_out(vty, " neighbor %s attribute-unchanged%s%s%s%s",
				addr,
				(CHECK_FLAG
				 (peer->af_flags[afi][safi],
				  PEER_FLAG_AS_PATH_UNCHANGED)) ? " as-path" :
				"",
				(CHECK_FLAG
				 (peer->af_flags[afi][safi],
				  PEER_FLAG_NEXTHOP_UNCHANGED)) ? " next-hop" :
				"",
				(CHECK_FLAG
				 (peer->af_flags[afi][safi],
				  PEER_FLAG_MED_UNCHANGED)) ? " med" : "",
				VTY_NEWLINE);
	}
}
		...
		if (peer->afc[afi][safi]) {
			if (!CHECK_FLAG(peer->sflags, PEER_STATUS_ACCEPT_PEER)) {
				bgp_config_write_family_header(vty, afi, safi,
							       &write);
				bgp_config_write_peer(vty, bgp, peer, afi,
						      safi);
			}
		}
	}
...
int bgp_config_write(struct vty *vty)
{
	...
	for (ALL_LIST_ELEMENTS(bm->bgp, mnode, mnnode, bgp)) {
		...
		if (CHECK_FLAG(bgp->flags, BGP_FLAG_NO_FAST_EXT_FAILOVER))
			vty_out(vty, " no bgp fast-external-failover%s",
				VTY_NEWLINE);
		...
		if (bgp_flag_check(bgp, BGP_FLAG_NO_DEFAULT_IPV4))
			vty_out(vty, " no bgp default ipv4-unicast%s",
				VTY_NEWLINE);
		...
		if (bgp_flag_check(bgp, BGP_FLAG_NO_CLIENT_TO_CLIENT))
			vty_out(vty, " no bgp client-to-client reflection%s",
				VTY_NEWLINE);

		/* BGP cluster ID. */
		if (CHECK_FLAG(bgp->config, BGP_CONFIG_CLUSTER_ID))
			vty_out(vty, " bgp cluster-id %s%s",
				inet_ntoa(bgp->cluster_id), VTY_NEWLINE);

		/* Confederation identifier */
		if (CHECK_FLAG(bgp->config, BGP_CONFIG_CONFEDERATION))
			vty_out(vty, " bgp confederation identifier %i%s",
				bgp->confed_id, VTY_NEWLINE);
		...
		if (bgp_flag_check(bgp, BGP_FLAG_ASPATH_IGNORE))
			vty_out(vty, " bgp bestpath as-path ignore%s",
				VTY_NEWLINE);
		if (bgp_flag_check(bgp, BGP_FLAG_ASPATH_CONFED))
			vty_out(vty, " bgp bestpath as-path confed%s",
				VTY_NEWLINE);
		if (bgp_flag_check(bgp, BGP_FLAG_ASPATH_MULTIPATH_RELAX)) {
			vty_out(vty, " bgp bestpath as-path multipath-relax%s",
				VTY_NEWLINE);
		}
		if (bgp_flag_check(bgp, BGP_FLAG_COMPARE_ROUTER_ID))
			vty_out(vty, " bgp bestpath compare-routerid%s",
				VTY_NEWLINE);
		...
		bgp_config_write_network(vty, bgp, AFI_IP, SAFI_UNICAST,
					 &write);
		...
		if (bgp->default_keepalive != BGP_DEFAULT_KEEPALIVE
		    && bgp->default_holdtime != BGP_DEFAULT_HOLDTIME)
			vty_out(vty, " timers bgp %d %d%s",
				bgp->default_keepalive, bgp->default_holdtime,
				VTY_NEWLINE);

		/* peer-group */
		for (ALL_LIST_ELEMENTS(bgp->group, node, nnode, group)) {
			bgp_config_write_peer(vty, bgp, group->conf, AFI_IP,
					      SAFI_UNICAST);
		}
		...
		for (ALL_LIST_ELEMENTS(bgp->peer, node, nnode, peer)) {
			if (!CHECK_FLAG(peer->sflags, PEER_STATUS_ACCEPT_PEER))
				bgp_config_write_peer(vty, bgp, peer, AFI_IP,
						      SAFI_UNICAST);
		}

		/* maximum-paths */
		bgp_config_write_maxpaths(vty, bgp, AFI_IP, SAFI_UNICAST,
					  &write);
