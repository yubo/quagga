...
unsigned int aspath_count_hops(struct aspath *aspath)
{
	int count = 0;
	struct assegment *seg = aspath->segments;
...
#undef SEGMENT_STR_LEN
		...
		for (i = 0; i < seg->length; i++) {
			len +=
			    snprintf(str_buf + len, str_size - len, "%u",
				     seg->as[i]);

			if (i < (seg->length - 1))
				len +=
				    snprintf(str_buf + len, str_size - len,
					     "%c", seperator);
		}
...
size_t aspath_put(struct stream * s, struct aspath * as, int use32bit)
{
	...
	if (seg) {
		...
		while (seg
			...
			bytes +=
			    ASSEGMENT_SIZE(seg->length - written + asns_packed,
					   use32bit);
			seg = next;
		}
	}
...
struct aspath *aspath_aggregate(struct aspath *as1, struct aspath *as2)
{
	...
	while (seg1) {
		for (i = from; i < seg1->length; i++)
			asset =
			    aspath_aggregate_as_set_add(aspath, asset,
							seg1->as[i]);
...
{
	hash_iterate(ashash, (void (*)(struct hash_backet *, void *))
		     aspath_show_all_iterator, vty);
}
