...
bgp_capability_orf_entry(struct peer *peer, struct capability_header *hdr)
{
	...
	for (i = 0; i < entry.num; i++) {
		...
		case ORF_MODE_RECEIVE:
			break;
		default:
			bgp_capability_orf_not_support(peer, afi, safi, type,
						       mode);
			continue;
		}
		...
		case CAPABILITY_CODE_ORF:
			...
			case ORF_TYPE_PREFIX:
				break;
			default:
				bgp_capability_orf_not_support(peer, afi, safi,
							       type, mode);
				continue;
			}
			break;
...
static int bgp_auth_parse(struct peer *peer, size_t length)
{
	bgp_notify_send(peer,
			BGP_NOTIFY_OPEN_ERR, BGP_NOTIFY_OPEN_AUTH_FAILURE);
	return -1;
}
...
as_t peek_for_as4_capability(struct peer * peer, u_char length)
{
	...
	if (BGP_DEBUG(as4, AS4))
		zlog_info("%s [AS4] rcv OPEN w/ OPTION parameter len: %u,"
			  " peeking for as4", peer->host, length);
...
int bgp_open_option_parse(struct peer *peer, u_char length, int *mp_capability)
{
	...
	while (stream_get_getp(s) < end) {
		...
		if (BGP_DEBUG(normal, NORMAL))
			zlog_debug
			    ("%s rcvd OPEN w/ optional parameter type %u (%s) len %u",
			     peer->host, opt_type,
			     opt_type ==
			     BGP_OPEN_OPT_AUTH ? "Authentication" : opt_type ==
			     BGP_OPEN_OPT_CAP ? "Capability" : "Unknown",
			     opt_length);
		...
		case BGP_OPEN_OPT_CAP:
			ret =
			    bgp_capability_parse(peer, opt_length,
						 mp_capability, &error);
			break;
		default:
			bgp_notify_send(peer,
