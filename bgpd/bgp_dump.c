...
static int bgp_dump_interval_add(struct bgp_dump *bgp_dump, int interval)
{
	...
	if (interval > 0) {
		...
		if ((interval < 86400) && ((86400 % interval) == 0)) {
			...
			tm = localtime(&t);
			secs_into_day =
			    tm->tm_sec + 60 * tm->tm_min +
			    60 * 60 * tm->tm_hour;
			interval = interval - secs_into_day % interval;	/* always > 0 */
		}
		bgp_dump->t_interval =
		    thread_add_timer(master, bgp_dump_interval_func, bgp_dump,
				     interval);
	} else {
		/* One-off dump: execute immediately, don't affect any scheduled dumps */
		bgp_dump->t_interval =
		    thread_add_event(master, bgp_dump_interval_func, bgp_dump,
				     0);
	}
...
#if 0
char *config_time2str(unsigned int interval)
{
