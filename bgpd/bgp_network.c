...
bgp_md5_set_socket(int socket, union sockunion *su, const char *password)
{
	...
	if (ret < 0)
		zlog(NULL, LOG_WARNING,
		     "can't set TCP_MD5SIG option on socket %d: %s", socket,
		     safe_strerror(en));
...
int bgp_md5_set(struct peer *peer)
{
	...
	for (ALL_LIST_ELEMENTS_RO(bm->listen_sockets, node, listener))
		if (listener->su.sa.sa_family == peer->su.sa.sa_family) {
			ret =
			    bgp_md5_set_socket(listener->fd, &peer->su,
					       peer->password);
			break;
		}
...
static void bgp_set_socket_ttl(struct peer *peer, int bgp_sock)
{
		...
		if (ret) {
			zlog_err
			    ("%s: Can't set TxTTL on peer (rtrid %s) socket, err = %d",
			     __func__, inet_ntop(AF_INET, &peer->remote_id, buf,
						 sizeof(buf)), errno);
		}
...
static int bgp_accept(struct thread *thread)
{
	...
	listener->thread =
	    thread_add_read(master, bgp_accept, listener, accept_sock);
...
static void bgp_update_source(struct peer *peer)
{
	...
	if (peer->update_source)
		sockunion_bind(peer->fd, peer->update_source, 0,
			       peer->update_source);
}
