		     ...
		     struct bgp_nlri *packet)
{
	...
	for (; pnt < lim; pnt += psize) {
		...
		if (prefixlen < 88) {
			zlog_err("prefix length is less than 88: %d",
				 prefixlen);
			return -1;
		}
...
#if 0
		if (type == RD_TYPE_AS)
			zlog_info("prefix %ld:%ld:%ld:%s/%d", label, rd_as.as,
				  rd_as.val, inet_ntoa(p.u.prefix4),
				  p.prefixlen);
		else if (type == RD_TYPE_IP)
			zlog_info("prefix %ld:%s:%ld:%s/%d", label,
...
static int
bgp_show_mpls_vpn(struct vty *vty, struct prefix_rd *prd,
