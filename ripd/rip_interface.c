...
ipv4_multicast_join(int sock,
		    ...
		    struct in_addr ifa, unsigned int ifindex)
{
	int ret;
...
static void rip_request_interface_send(struct interface *ifp, u_char version)
{
	...
	if (if_is_pointopoint(ifp) || if_is_broadcast(ifp)) {
		...
		for (ALL_LIST_ELEMENTS
		     (ifp->connected, cnode, cnnode, connected)) {
			if (connected->address->family == AF_INET) {
				...
				if (connected->destination)
					...
					to.sin_addr =
					    connected->destination->u.prefix4;
...
static void rip_request_neighbor(struct in_addr addr)
{
	struct sockaddr_in to;
...
static void rip_request_neighbor_all(void)
{
...
static int rip_multicast_join(struct interface *ifp, int sock)
{
	...
	if (if_is_operative(ifp) && if_is_multicast(ifp)) {
		...
		for (ALL_LIST_ELEMENTS_RO(ifp->connected, cnode, ifc)) {
			...
			if (ipv4_multicast_join
			    (sock, group, p->prefix, ifp->ifindex) < 0)
				return -1;
			else
				return 0;
		}
...
int
rip_interface_delete(int command, struct zclient *zclient, zebra_size_t length)
{
	struct interface *ifp;
	struct stream *s;
      ...
      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n" "Interface name\n")
{
	...
	if (ret < 0) {
		vty_out(vty, "There is a same network configuration %s%s",
			argv[0], VTY_NEWLINE);
		return CMD_WARNING;
	}
      ...
      "neighbor A.B.C.D", "Specify a neighbor router\n" "Neighbor address\n")
{
	...
	if (ret <= 0) {
		vty_out(vty, "Please specify address by A.B.C.D%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
      ...
      "Keyed message digest\n" "Clear text authentication\n")
{
	...
	if ((argc == 2) && (auth_type != RIP_AUTH_MD5)) {
		vty_out(vty, "auth length argument only valid for md5%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
      ...
      "Authentication string\n" "Authentication string\n")
{
	...
	if (ri->key_chain) {
		vty_out(vty, "%% key-chain configuration exists%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	for (ALL_LIST_ELEMENTS_RO(iflist, node, ifp)) {
		...
		if (ri->split_horizon != ri->split_horizon_default) {
			...
			case RIP_SPLIT_HORIZON:
				vty_out(vty, " ip rip split-horizon%s",
					VTY_NEWLINE);
				break;
			case RIP_SPLIT_HORIZON_POISONED_REVERSE:
				vty_out(vty,
			...
			default:
				vty_out(vty, " no ip rip split-horizon%s",
					VTY_NEWLINE);
				break;
			}
		}
		...
		if (ri->auth_type == RIP_AUTH_SIMPLE_PASSWORD)
			vty_out(vty, " ip rip authentication mode text%s",
				VTY_NEWLINE);
