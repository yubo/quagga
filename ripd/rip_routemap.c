...
{
	if (argc == 0)
		return rip_route_match_delete(vty, vty->index, "interface",
					      NULL);
...
{
	if (argc == 0)
		return rip_route_match_delete(vty, vty->index, "ip next-hop",
					      NULL);
...
{
	return rip_route_match_add(vty, vty->index, "ip next-hop prefix-list",
				   argv[0]);
}
...
{
	if (argc == 0)
		return rip_route_match_delete(vty, vty->index,
...
{
	if (argc == 0)
		return rip_route_match_delete(vty, vty->index, "ip address",
					      NULL);
...
ALIAS(no_match_ip_address,
    ...
    DEFUN(match_ip_address_prefix_list,
      ...
      "Match entries of prefix-lists\n" "IP prefix-list name\n")
{
	return rip_route_match_add(vty, vty->index, "ip address prefix-list",
				   argv[0]);
}
      ...
      IP_STR "Match address of route\n" "Match entries of prefix-lists\n")
{
	...
	return rip_route_match_delete(vty, vty->index, "ip address prefix-list",
				      argv[0]);
}
...
ALIAS(no_match_ip_address_prefix_list,
    ...
    DEFUN(match_tag,
      match_tag_cmd,
      "match tag <0-65535>", MATCH_STR "Match tag of route\n" "Metric value\n")
{
	return rip_route_match_add(vty, vty->index, "tag", argv[0]);
}

DEFUN(no_match_tag,
      no_match_tag_cmd, "no match tag", NO_STR MATCH_STR "Match tag of route\n")
{
	if (argc == 0)
...
{
	if (argc == 0)
		return rip_route_set_delete(vty, vty->index, "ip next-hop",
					    NULL);
...
DEFUN(no_set_tag,
      no_set_tag_cmd,
      "no set tag", NO_STR SET_STR "Tag value for routing protocol\n")
{
	if (argc == 0)
