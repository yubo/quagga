...
struct vtysh_client {
	...
	{
	.fd = -1,.name = "ripngd",.flag = VTYSH_RIPNGD,.path =
		    RIPNG_VTYSH_PATH}, {
	.fd = -1,.name = "ospfd",.flag = VTYSH_OSPFD,.path =
		    OSPF_VTYSH_PATH}, {
	...
	.fd = -1,.name = "babeld",.flag = VTYSH_BABELD,.path =
		    BABEL_VTYSH_PATH}, {
.fd = -1,.name = "pimd",.flag = VTYSH_PIMD,.path =
		    PIM_VTYSH_PATH},};
...
static int vtysh_client_config(struct vtysh_client *vclient, char *line)
{
	int ret;
	char *buf;
	size_t bufsz;
...
int vtysh_config_from_file(struct vty *vty, FILE * fp)
{
	...
	while (fgets(vty->buf, VTY_BUFSIZ, fp)) {
		...
		if (ret != CMD_SUCCESS
		    && ret != CMD_SUCCESS_DAEMON && ret != CMD_WARNING) {
...
static char **vtysh_completion(char *text, int start, int end)
{
	int ret;
	vector vline;
	char **matched = NULL;
...
DEFUNSH(VTYSH_RIPNGD,
	router_ripng, router_ripng_cmd, "router ripng", ROUTER_STR "RIPng")
{
	vty->node = RIPNG_NODE;
	return CMD_SUCCESS;
}
...
DEFUNSH(VTYSH_OSPF6D,
	router_ospf6,
	router_ospf6_cmd, "router ospf6", OSPF6_ROUTER_STR OSPF6_STR)
{
	vty->node = OSPF6_NODE;
	return CMD_SUCCESS;
}
...
DEFUNSH(VTYSH_BABELD,
	router_babel, router_babel_cmd, "router babel", ROUTER_STR "Babel")
{
	vty->node = BABEL_NODE;
	return CMD_SUCCESS;
}
...
DEFUNSH(VTYSH_ALL,
	vtysh_enable,
	vtysh_enable_cmd, "enable", "Turn on privileged mode command\n")
{
	vty->node = ENABLE_NODE;
	return CMD_SUCCESS;
}
...
DEFUNSH(VTYSH_ALL,
	vtysh_disable,
	vtysh_disable_cmd, "disable", "Turn off privileged mode command\n")
{
...
/* Memory */
    DEFUN(vtysh_show_memory,
      vtysh_show_memory_cmd, "show memory", SHOW_STR "Memory statistics\n")
{
	unsigned int i;
	int ret = CMD_SUCCESS;
...
ALIAS(vtysh_write_memory,
    ...
    ALIAS(vtysh_write_terminal,
      ...
      "show running-config", SHOW_STR "Current operating configuration\n")
...
DEFUN(vtysh_show_daemons,
      ...
      "show daemons", SHOW_STR "Show list of running daemons\n")
{
	u_int i;
...
DEFUN(vtysh_ssh,
      vtysh_ssh_cmd, "ssh WORD", "Open an ssh connection\n" "[user@]host\n")
{
...
DEFUN(vtysh_start_shell,
      vtysh_start_shell_cmd, "start-shell", "Start UNIX shell\n")
{
...
DEFUN(vtysh_start_bash,
      ...
      "start-shell bash", "Start UNIX shell\n" "Start bash\n")
{
...
DEFUN(vtysh_start_zsh,
      vtysh_start_zsh_cmd,
      "start-shell zsh", "Start UNIX shell\n" "Start Z shell\n")
{
