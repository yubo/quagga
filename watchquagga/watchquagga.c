...
", progname, mode_str[0], progname, mode_str[1], progname, mode_str[2], progname, mode_str[3], progname, mode_str[4], progname, mode_str[2], mode_str[3], VTYDIR, DEFAULT_LOGLEVEL, LOG_EMERG, LOG_DEBUG, LOG_DEBUG, DEFAULT_MIN_RESTART, DEFAULT_MAX_RESTART, DEFAULT_PERIOD, DEFAULT_TIMEOUT, DEFAULT_RESTART_TIMEOUT, DEFAULT_PIDFILE);

	return status;
}

static pid_t run_background(const char *shell_cmd)
{
	pid_t child;
...
static void phase_check(void)
{
	...
	case PHASE_STOPS_PENDING:
	 ...
	 /*FALLTHRU*/ case PHASE_WAITING_DOWN:
...
static void try_restart(struct daemon *dmn)
{
	...
	/*FALLTHRU*/ case MODE_PHASED_ALL_RESTART:
		...
		/* Is it too soon for a restart? */
		{
			...
			if (time_elapsed
			    (&delay,
			     &gs.special->restart.time)->tv_sec <
		...
		for (dmn = gs.daemons; dmn; dmn = dmn->next) {
			if (dmn != gs.special)
				run_job(&dmn->restart, "stop", gs.stop_command,
					1, 1);
		}
