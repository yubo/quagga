...
static
struct acc_vals accumulate(u_char * buffer, testsz_t len, testoff_t off)
{
	u_int8_t *p;
	u_int16_t *csum;
	int i, partial_len;
...
static int /* return checksum in low-order 16 bits */ in_cksum_optimized(void
									 *parg,
									 int
									 nbytes)
{
...
static int /* return checksum in low-order 16 bits */ in_cksum_rfc(void *parg,
								   int count)
/* from RFC 1071 */
{
	u_short *addr = parg;
...
#define EXERCISESTEP 257
	...
	while (1) {
		...
		if (in_csum_res != in_csum || in_csum != in_csum_rfc)
			printf
			    ("verify: in_chksum failed in_csum:%x, in_csum_res:%x,"
			     "in_csum_rfc %x, len:%d\n", in_csum, in_csum_res,
			     in_csum_rfc, exercise);

		ospfd =
		    ospfd_checksum(buffer, exercise + sizeof(u_int16_t),
				   exercise);
		if (verify(buffer, exercise + sizeof(u_int16_t)))
			printf("verify: ospfd failed\n");
		isisd =
		    iso_csum_create(buffer, exercise + sizeof(u_int16_t),
				    exercise);
		if (verify(buffer, exercise + sizeof(u_int16_t)))
			printf("verify: isisd failed\n");
		lib =
		    fletcher_checksum(buffer, exercise + sizeof(u_int16_t),
				      exercise);
		if (verify(buffer, exercise + sizeof(u_int16_t)))
		...
		if (ospfd != lib) {
			printf("Mismatch in values at size %u\n"
			       ...
			       "lib: 0x%04x\n",
			       exercise,
			       ospfd, ospfd_vals.a.c0, ospfd_vals.a.c1,
			       ospfd_vals.x, ospfd_vals.y, isisd,
			       isisd_vals.a.c0, isisd_vals.a.c1, isisd_vals.x,
			       isisd_vals.y, lib);
			...
			if (ospfd_vals.a.c0 == isisd_vals.a.c0
				...
				for (i = 0; reducts[i].name != NULL; i++) {
					ospfd = reducts[i].f(&ospfd_vals,
							     ...
							     exercise);
					printf
					    ("%20s: x: %02x, y %02x, checksum 0x%04x\n",
					     reducts[i].name,
					     ospfd_vals.x & 0xff,
