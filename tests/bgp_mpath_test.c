...
static int run_bgp_cfg_maximum_paths(testcase_t * t)
{
	...
	for (afi = AFI_IP; afi < AFI_MAX; afi++)
		for (safi = SAFI_UNICAST; safi < SAFI_MAX; safi++) {
			...
			api_result =
			    bgp_maximum_paths_unset(bgp, afi, safi,
						    BGP_PEER_EBGP);
			EXPECT_TRUE(api_result == 0, test_result);
			api_result =
			    bgp_maximum_paths_unset(bgp, afi, safi,
...
static int setup_bgp_mp_list(testcase_t * t)
{
	...
	if ((test_mp_list_peer[0].su_remote =
	     sockunion_str2su("1.1.1.1")) == NULL)
		return -1;
	if ((test_mp_list_peer[1].su_remote =
	     sockunion_str2su("2.2.2.2")) == NULL)
		return -1;
	if ((test_mp_list_peer[2].su_remote =
	     sockunion_str2su("3.3.3.3")) == NULL)
		return -1;
	if ((test_mp_list_peer[3].su_remote =
	     sockunion_str2su("4.4.4.4")) == NULL)
		return -1;
	if ((test_mp_list_peer[4].su_remote =
