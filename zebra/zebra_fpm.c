 ...
 */
static inline struct route_node *zfpm_rnodes_iter_next(zfpm_rnodes_iter_t *
						       iter)
{
...
static int zfpm_read_cb(struct thread *thread)
{
	...
	if (already < FPM_MSG_HDR_LEN) {
		...
		nbyte =
		    stream_read_try(ibuf, zfpm_g->sock,
 ...
 */
static struct rib *zfpm_route_for_update(rib_dest_t * dest)
{
	struct rib *rib;
...
static void zfpm_build_updates(void)
{
	...
	do {
		...
		if (write_msg) {
			data_len =
			    zfpm_encode_route(dest, rib, (char *)data,
					      buf_end - data);
	...
	if (reason) {
		zfpm_debug("%s/%d triggering update to FPM - Reason: %s",
			   inet_ntop(rn->p.family, &rn->p.u.prefix, buf,
				     sizeof(buf)), rn->p.prefixlen, reason);
	}
 ...
 */
int zfpm_init(struct thread_master *master, int enable, uint16_t port)
{
