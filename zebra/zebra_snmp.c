			  ...
			  WriteMethod ** write_method)
{
	...
	if (smux_header_generic
	    (v, objid, objid_len, exact, val_len, write_method) == MATCH_FAILED)
		return NULL;
		       ...
		       int exact, struct route_node **np, struct rib **rib)
{
	...
	*rib = NULL;
	...
	if (*objid_len > (unsigned)v->namelen)
		oid2in_addr(objid + v->namelen, MIN(4, *objid_len - v->namelen),
			    &dest);
	...
	if (*objid_len > (unsigned)v->namelen + 6)
		oid2in_addr(objid + v->namelen + 6,
			    MIN(4, *objid_len - v->namelen - 6), &nexthop);
		...
		for (*np = route_top(table); *np; *np = route_next(*np)) {
			if (!in_addr_cmp(&(*np)->p.u.prefix, (u_char *) & dest)) {
				RNODE_FOREACH_RIB(*np, *rib) {
					if (!in_addr_cmp
					    ((u_char *) & (*rib)->nexthop->
					     gate.ipv4, (u_char *) & nexthop))
						if (proto ==
						    proto_trans((*rib)->type))
							return;
				}
			}
		}
