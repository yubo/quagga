 ...
 */
static void inline rta_copy(union sockunion *dest, caddr_t src)
{
	int len;
...
static int inline af_check(int family)
{
	if (family == AF_INET)
		return 1;
#ifdef HAVE_IPV6
		...
		if (ifp->ifindex != ifm->ifm_index) {
			zlog_warn
			    ("%s: index mismatch, ifname %s, ifp index %d, "
			     "ifm index %d", __func__, ifp->name, ifp->ifindex,
			     ifm->ifm_index);
			return -1;
		}
#ifdef HAVE_BSD_IFI_LINK_STATE	/* translate BSD kernel msg for link-state */
...
ifam_read_mesg(struct ifa_msghdr *ifm,
	       ...
	       union sockunion *brd, char *ifname, short *ifnlen)
{
	caddr_t pnt, end;
	union sockunion dst;
		...
		case AF_INET:
			{
				...
				zlog_debug
				    ("%s: ifindex %d, ifname %s, ifam_addrs 0x%x, "
				     ...
				     "gateway %s", __func__, ifm->ifam_index,
				     (ifnlen ? ifname : "(nil)"),
				     ifm->ifam_addrs, ifm->ifam_flags,
...
rtm_read_mesg(struct rt_msghdr *rtm,
	      ...
	      union sockunion *mask,
	      union sockunion *gate, char *ifname, short *ifnlen)
{
	caddr_t pnt, end;
...
void rtm_read(struct rt_msghdr *rtm)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL)
		zlog_debug("%s: got rtm of type %d (%s)", __func__,
			   rtm->rtm_type, lookup(rtm_type_str, rtm->rtm_type));
		...
		if (rtm->rtm_type != RTM_GET && rtm->rtm_pid == pid) {
			...
			case RTM_CHANGE:
				...
				case ZEBRA_RIB_NOTFOUND:
					zlog_debug
					    ("%s: %s %s/%d: desync: RR isn't yet in RIB, while already in FIB",
					     __func__, lookup(rtm_type_str,
							      rtm->rtm_type),
					     buf, p.prefixlen);
					break;
			...
			case RTM_DELETE:
				...
				case ZEBRA_RIB_FOUND_EXACT:
					zlog_debug
					    ("%s: %s %s/%d: desync: RR is still in RIB, while already not in FIB",
					     __func__, lookup(rtm_type_str,
							      rtm->rtm_type),
					     buf, p.prefixlen);
					rib_lookup_and_dump(&p);
				...
				case ZEBRA_RIB_FOUND_NOGATE:
					zlog_debug
					    ("%s: %s %s/%d: desync: RR is still in RIB, plus gate differs",
					     __func__, lookup(rtm_type_str,
							      rtm->rtm_type),
					     buf, p.prefixlen);
					rib_lookup_and_dump(&p);
			...
			default:
				zlog_debug
				    ("%s: %s/%d: warning: loopback RTM of type %s received",
...
static void rtmsg_debug(struct rt_msghdr *rtm)
{
	...
	zlog_debug("Kernel: pid %d, rtm_addrs 0x%x", rtm->rtm_pid,
		   rtm->rtm_addrs);
}
...
/* Make routing socket. */
static void routing_socket(void)
{
