...
static int netlink_recvbuf(struct nlsock *nl, uint32_t newsize)
{
	...
	if (ret < 0) {
		zlog(NULL, LOG_ERR, "Can't get %s receive buffer size: %s",
	...
	if (ret < 0)
		ret =
		    setsockopt(nl->sock, SOL_SOCKET, SO_RCVBUF, &nl_rcvbufsize,
			       sizeof(nl_rcvbufsize));
	if (ret < 0) {
		zlog(NULL, LOG_ERR, "Can't set %s receive buffer size: %s",
		     nl->name, safe_strerror(errno));
		return -1;
	}
...
static int netlink_socket(struct nlsock *nl, unsigned long groups)
{
	...
	namelen = sizeof snl;
	ret =
	    getsockname(sock, (struct sockaddr *)&snl, (socklen_t *) & namelen);
		   ...
		   struct nlsock *nl)
{
	...
	while (1) {
		...
		for (h = (struct nlmsghdr *)buf;
			...
			error = (*filter) (&snl, h);
			if (error < 0) {
				zlog(NULL, LOG_ERR, "%s filter function error",
				     nl->name);
				ret = error;
			}
		}
		...
		if (status) {
			zlog(NULL, LOG_ERR, "%s error: data remnant size %d",
			     nl->name, status);
			return -1;
		}
	}
...
static int netlink_routing_table(struct sockaddr_nl *snl, struct nlmsghdr *h)
{
	int len;
	struct rtmsg *rtm;
	...
	if (rtm->rtm_family == AF_INET) {
		...
		if (!tb[RTA_MULTIPATH])
			rib_add_ipv4(ZEBRA_ROUTE_KERNEL, flags, &p, gate, src,
				     index, table, metric, 0, SAFI_UNICAST);
		else {
			...
			for (;;) {
				...
				if (rtnh->rtnh_len > sizeof(*rtnh)) {
					memset(tb, 0, sizeof(tb));
					netlink_parse_rtattr(tb, RTA_MAX,
							     RTNH_DATA(rtnh),
							     rtnh->rtnh_len -
							     sizeof(*rtnh));
					if (tb[RTA_GATEWAY])
...
static int netlink_route_change(struct sockaddr_nl *snl, struct nlmsghdr *h)
{
	...
	if (rtm->rtm_family == AF_INET) {
		...
		if (h->nlmsg_type == RTM_NEWROUTE) {
			if (!tb[RTA_MULTIPATH])
				rib_add_ipv4(ZEBRA_ROUTE_KERNEL, 0, &p, gate,
					     src, index, table, metric, 0,
					     SAFI_UNICAST);
			else {
				/* This is a multipath route */

				struct rib *rib;
				struct rtnexthop *rtnh = (struct rtnexthop *)
...
netlink_information_fetch(struct sockaddr_nl *snl, struct nlmsghdr *h)
{
	...
	if (snl->nl_pid != 0) {
		zlog(NULL, LOG_ERR, "Ignoring message from pid %u",
		     snl->nl_pid);
		return 0;
	}
...
int addattr_l(struct nlmsghdr *n, size_t maxlen, int type, void *data, int alen)
{
	size_t len;
	struct rtattr *rta;
 ...
 */
static void
_netlink_route_build_singlepath(const char *routedesc,
				int bytelen,
				struct nexthop *nexthop,
				struct nlmsghdr *nlmsg,
				struct rtmsg *rtmsg, size_t req_size)
{
 ...
 */
static void
_netlink_route_build_multipath(const char *routedesc,
			       int bytelen,
			       struct nexthop *nexthop,
			       struct rtattr *rta,
			       struct rtnexthop *rtnh, union g_addr **src)
{
...
/* Routing table change via netlink interface. */
static int
netlink_route_multipath(int cmd, struct prefix *p, struct rib *rib, int family)
{
	int bytelen;
	struct sockaddr_nl snl;
	...
	if (discard) {
		if (cmd == RTM_NEWROUTE)
			for (ALL_NEXTHOPS_RO
				 ...
				 */
				if (CHECK_FLAG
				    (nexthop->flags, NEXTHOP_FLAG_RECURSIVE))
					continue;
	...
	if (nexthop_num == 1 || MULTIPATH_NUM == 1) {
		...
		for (ALL_NEXTHOPS_RO
			...
			if ((cmd == RTM_NEWROUTE
				...
				&& CHECK_FLAG(nexthop->flags,
					      NEXTHOP_FLAG_FIB))) {
				routedesc =
				    recursing ? "recursive, 1 hop" :
				    "single hop";

				_netlink_route_debug(cmd, p, nexthop, routedesc,
						     family);
				_netlink_route_build_singlepath(routedesc,
								bytelen,
								nexthop, &req.n,
								&req.r,
								sizeof req);
		...
		for (ALL_NEXTHOPS_RO
			...
			if ((cmd == RTM_NEWROUTE
				...
				&& CHECK_FLAG(nexthop->flags,
					      NEXTHOP_FLAG_FIB))) {
				routedesc =
				    recursing ? "recursive, multihop" :
				    "multihop";
				nexthop_num++;

				_netlink_route_debug(cmd, p, nexthop,
						     routedesc, family);
				_netlink_route_build_multipath(routedesc,
							       bytelen, nexthop,
							       rta, rtnh, &src);
				rtnh = RTNH_NEXT(rtnh);
		...
		if (src)
			addattr_l(&req.n, sizeof req, RTA_PREFSRC, &src->ipv4,
				  bytelen);
