		       ...
		       const char *distance_str)
{
	...
	if ((gate_str != NULL)
		...
		if (add_cmd)
			static_add_ipv4_safi(safi, &p, NULL, NULL,
					     ZEBRA_FLAG_BLACKHOLE, distance, 0);
		else
			static_delete_ipv4_safi(safi, &p, NULL, NULL, distance,
						0);
		return CMD_SUCCESS;
	}
	...
	if (gate_str == NULL) {
		if (add_cmd)
			static_add_ipv4_safi(safi, &p, NULL, NULL, flag,
					     distance, 0);
		else
			static_delete_ipv4_safi(safi, &p, NULL, NULL, distance,
						0);
      ...
      "Nexthop address\n" "Nexthop interface name\n" "Distance\n")
{
	...
	return zebra_static_ipv4_safi(vty, SAFI_MULTICAST, 1, argv[0], NULL,
				      argv[1], NULL, argc > 2 ? argv[2] : NULL);
}
...
{
	VTY_WARN_EXPERIMENTAL();
	return zebra_static_ipv4_safi(vty, SAFI_MULTICAST, 0, argv[0], NULL,
				      argv[1], NULL, argc > 2 ? argv[2] : NULL);
}
...
DEFUN(ip_route_mask_flags_distance2,
      ...
      "Silently discard pkts when matched\n" "Distance value for this route\n")
{
	return zebra_static_ipv4(vty, 1, argv[0], argv[1], NULL, argv[2],
				 argv[3]);
}
...
DEFUN(no_ip_route_mask_flags_distance2,
      ...
      "Silently discard pkts when matched\n" "Distance value for this route\n")
{
	return zebra_static_ipv4(vty, 0, argv[0], argv[1], NULL, argv[2],
				 argv[3]);
}
...
DEFUN(ip_protocol,
      ...
      NO_STR "Apply route map to PROTO\n" "Protocol name\n" "Route map name\n")
{
	int i;
	...
	if (i < 0) {
		vty_out(vty, "invalid protocol name \"%s\"%s",
...
DEFUN(no_ip_protocol,
      ...
      NO_STR "Remove route map from PROTO\n" "Protocol name\n")
{
	int i;
...
#define ONE_WEEK_SECOND 60*60*24*7
		...
		for (ALL_NEXTHOPS_RO
			...
			case NEXTHOP_TYPE_IPV6_IFNAME:
				if (!IPV6_ADDR_SAME
				    (&nexthop->src.ipv6, &in6addr_any)) {
					if (inet_ntop
					    (AF_INET6, &nexthop->src.ipv6,
					     addrstr, sizeof addrstr))
						vty_out(vty, ", src %s",
							addrstr);
				}
...
DEFUN(show_ip_route,
      show_ip_route_cmd, "show ip route", SHOW_STR IP_STR "IP routing table\n")
{
...
DEFUN(show_ip_protocol,
      ...
      "show ip protocol", SHOW_STR IP_STR "IP protocol filtering status\n")
{
	int i;
	...
	for (i = 0; i < ZEBRA_ROUTE_MAX; i++) {
		if (proto_rm[AFI_IP][i])
			...
			vty_out(vty, "%-10s  : none%s", zebra_route_string(i),
				VTY_NEWLINE);
	}
	if (proto_rm[AFI_IP][i])
...
DEFUN(show_ip_mroute,
      show_ip_mroute_cmd,
      "show ip mroute", SHOW_STR IP_STR "IP Multicast routing table\n")
{
	struct route_table *table;
	struct route_node *rn;
	struct rib *rib;
...
static void
vty_show_ipv6_route(struct vty *vty, struct route_node *rn, struct rib *rib)
{
	struct nexthop *nexthop, *tnexthop;
	int recursing;
	int len = 0;
...
DEFUN(show_ipv6_route,
      show_ipv6_route_cmd,
      "show ipv6 route", SHOW_STR IP_STR "IPv6 routing table\n")
{
	struct route_table *table;
	struct route_node *rn;
	struct rib *rib;
...
DEFUN(show_ipv6_mroute,
      ...
      "show ipv6 mroute", SHOW_STR IP_STR "IPv6 Multicast routing table\n")
{
	struct route_table *table;
	struct route_node *rn;
	struct rib *rib;
	...
	for (rn = route_top(stable); rn; rn = route_next(rn))
		for (si = rn->info; si; si = si->next) {
			...
			case STATIC_IPV6_GATEWAY:
				vty_out(vty, " %s",
					inet_ntop(AF_INET6, &si->ipv6, buf,
						  BUFSIZ));
				break;
...
static int config_write_vty(struct vty *vty)
{
	...
	if (ipv4_multicast_mode != MCAST_NO_CONFIG)
		vty_out(vty, "ip multicast rpf-lookup-mode %s%s",
			...
			ipv4_multicast_mode ==
			MCAST_MIX_MRIB_FIRST ? "mrib-then-urib" :
			ipv4_multicast_mode ==
			MCAST_MIX_DISTANCE ? "lower-distance" : "longer-prefix",
			VTY_NEWLINE);

	for (i = 0; i < ZEBRA_ROUTE_MAX; i++) {
		if (proto_rm[AFI_IP][i])
			vty_out(vty, "ip protocol %s route-map %s%s",
