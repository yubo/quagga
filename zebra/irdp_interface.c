...
static int
if_group(struct interface *ifp, int sock, u_int32_t group, int add_leave)
{
	struct ip_mreq m;
	struct prefix *p;
	int ret;
	...
	if (ret < 0)
		zlog_warn("IRDP: %s can't setsockopt %s: %s",
			  add_leave ==
			  IP_ADD_MEMBERSHIP ? "join group" : "leave group",
...
static int if_drop_group(struct interface *ifp)
{
	...
	ret =
	    if_group(ifp, irdp_sock, INADDR_ALLRTRS_GROUP, IP_DROP_MEMBERSHIP);
	if (ret < 0)
		return ret;
...
DEFUN(ip_irdp_broadcast,
      ...
      "ip irdp broadcast",
      IP_STR "ICMP Router discovery on this interface using broadcast\n")
{
	struct interface *ifp;
...
DEFUN(ip_irdp_shutdown,
      ...
      "ip irdp shutdown",
      IP_STR "ICMP Router discovery shutdown on this interface\n")
{
	struct interface *ifp;
...
DEFUN(no_ip_irdp_shutdown,
      ...
      "no ip irdp shutdown",
      NO_STR IP_STR "ICMP Router discovery no shutdown on this interface\n")
{
	struct interface *ifp;
