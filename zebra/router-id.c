...
void router_id_get(struct prefix *p)
{
	struct listnode *node;
	struct connected *c;
...
static void router_id_set(struct prefix *p)
{
	struct prefix p2;
	struct listnode *node;
	struct zserv *client;
...
void router_id_write(struct vty *vty)
{
	if (rid_user_assigned.u.prefix4.s_addr)
		vty_out(vty, "router-id %s%s",
			inet_ntoa(rid_user_assigned.u.prefix4), VTY_NEWLINE);
}
...
DEFUN(no_router_id,
      no_router_id_cmd,
      "no router-id", NO_STR "Remove the manually configured router-id\n")
{
	struct prefix rid;
