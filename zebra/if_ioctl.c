...
int if_get_addr(struct interface *ifp)
{
	...
	if (ret < 0) {
		if (errno != EADDRNOTAVAIL) {
			zlog_warn("SIOCGIFNETMASK fail: %s",
				  safe_strerror(errno));
			return ret;
		}
		return 0;
	...
	if (ret < 0) {
		if (errno != EADDRNOTAVAIL)
			zlog_warn("SIOCGIFDSTADDR fail: %s",
				  safe_strerror(errno));
	...
	if (!dest_pnt) {
		...
		if (ret < 0) {
			if (errno != EADDRNOTAVAIL)
				zlog_warn("SIOCGIFBRDADDR fail: %s",
					  safe_strerror(errno));
		} else
		    if (!IPV4_ADDR_SAME
			(&addr.sin_addr, &ifreq.ifr_broadaddr.sin_addr)) {
			memcpy(&dest, &ifreq.ifr_broadaddr,
