...
static const struct {
	...
	[ZEBRA_ROUTE_SYSTEM] = {
	ZEBRA_ROUTE_SYSTEM, 0},[ZEBRA_ROUTE_KERNEL] = {
	ZEBRA_ROUTE_KERNEL, 0},[ZEBRA_ROUTE_CONNECT] = {
	ZEBRA_ROUTE_CONNECT, 0},[ZEBRA_ROUTE_STATIC] = {
	ZEBRA_ROUTE_STATIC, 1},[ZEBRA_ROUTE_RIP] = {
	ZEBRA_ROUTE_RIP, 120},[ZEBRA_ROUTE_RIPNG] = {
	ZEBRA_ROUTE_RIPNG, 120},[ZEBRA_ROUTE_OSPF] = {
	ZEBRA_ROUTE_OSPF, 110},[ZEBRA_ROUTE_OSPF6] = {
	ZEBRA_ROUTE_OSPF6, 110},[ZEBRA_ROUTE_ISIS] = {
	ZEBRA_ROUTE_ISIS, 115},[ZEBRA_ROUTE_BGP] = {
	    ...
	    const char *msgfmt, ...)
{
	...
	if (rn) {
		...
		bptr = buf + strlen(buf);
		snprintf(bptr, buf + sizeof(buf) - bptr, "/%d%s",
			 rn->p.prefixlen,
 ...
 * vrf_table_create
 */
static void vrf_table_create(struct vrf *vrf, afi_t afi, safi_t safi)
{
...
static void _nexthop_add(struct nexthop **target, struct nexthop *nexthop)
{
	struct nexthop *last;
...
struct nexthop *nexthop_ipv4_add(struct rib *rib, struct in_addr *ipv4,
				 struct in_addr *src)
{
	struct nexthop *nexthop;
...
struct nexthop *nexthop_ipv4_ifindex_add(struct rib *rib, struct in_addr *ipv4,
					 struct in_addr *src,
...
static struct nexthop *nexthop_ipv6_ifname_add(struct rib *rib,
...
static struct nexthop *nexthop_ipv6_ifindex_add(struct rib *rib,
						struct in6_addr *ipv6,
						unsigned int ifindex)
{
	struct nexthop *nexthop;
...
struct rib *rib_match_ipv4_safi(struct in_addr addr, safi_t safi, int skip_bgp,
...
struct rib *rib_match_ipv4_multicast(struct in_addr addr,
				     struct route_node **rn_out)
{
	...
	case MCAST_MRIB_ONLY:
		return rib_match_ipv4_safi(addr, SAFI_MULTICAST, skip_bgp,
					   rn_out);
	case MCAST_URIB_ONLY:
		return rib_match_ipv4_safi(addr, SAFI_UNICAST, skip_bgp,
					   rn_out);
	case MCAST_NO_CONFIG:
	case MCAST_MIX_MRIB_FIRST:
		rib = mrib =
		    rib_match_ipv4_safi(addr, SAFI_MULTICAST, skip_bgp, &m_rn);
		if (!mrib)
			rib = urib =
			    rib_match_ipv4_safi(addr, SAFI_UNICAST, skip_bgp,
						&u_rn);
		break;
	...
	case MCAST_MIX_PFXLEN:
		...
		if (mrib && urib)
			rib =
			    u_rn->p.prefixlen > m_rn->p.prefixlen ? urib : mrib;
		else if (mrib)
			rib = mrib;
		else if (urib)
			rib = urib;
		     ...
		     struct nexthop *nexthop, int set)
{
	...
	if (!rmap && proto_rm[family][ZEBRA_ROUTE_MAX])
		rmap =
		    route_map_lookup_by_name(proto_rm[family][ZEBRA_ROUTE_MAX]);
	if (rmap) {
...
static void rib_install_kernel(struct route_node *rn, struct rib *rib)
{
	...
	if (info->safi != SAFI_UNICAST) {
		for (ALL_NEXTHOPS_RO
		     (rib->nexthop, nexthop, tnexthop, recursing))
...
static void rib_process(struct route_node *rn)
{
	...
	if (select && select == fib) {
		...
		if (CHECK_FLAG(select->flags, ZEBRA_FLAG_CHANGED)) {
			...
			for (ALL_NEXTHOPS_RO
			     (select->nexthop, nexthop, tnexthop, recursing))
				if (CHECK_FLAG
				    (nexthop->flags, NEXTHOP_FLAG_FIB)) {
					installed = 1;
					break;
				}
			if (!installed)
	     ...
	     u_int32_t metric, u_char distance, safi_t safi)
{
	...
	if (IS_ZEBRA_DEBUG_RIB)
		zlog_debug("%s: calling rib_addnode (%p, %p)", __func__, rn,
			   rib);
	rib_addnode(rn, rib);
	...
	if (same) {
		if (IS_ZEBRA_DEBUG_RIB)
			zlog_debug("%s: calling rib_delnode (%p, %p)", __func__,
				   rn, rib);
		rib_delnode(rn, same);
	}
	       ...
	       union prefix46constptr pp, const struct rib *rib)
{
	...
	zlog_debug("%s: dumping RIB entry %p for %s/%d", func, rib, straddr,
		   p->prefixlen);
	zlog_debug("%s: refcnt == %lu, uptime == %lu, type == %u, table == %d",
		   func, rib->refcnt, (unsigned long)rib->uptime, rib->type,
		   rib->table);
	zlog_debug
	    ("%s: metric == %u, distance == %u, flags == %u, status == %u",
	     func, rib->metric, rib->distance, rib->flags, rib->status);
	zlog_debug
		...
		zlog_debug
		    ("%s: %s %s with flags %s%s%s",
		     ...
		     (CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE) ?
		      "ACTIVE " : ""), (CHECK_FLAG(nexthop->flags,
						   NEXTHOP_FLAG_FIB) ? "FIB " :
...
void rib_lookup_and_dump(struct prefix_ipv4 *p)
{
	...
	/* No route for this prefix. */
	if (!rn) {
		zlog_debug("%s: lookup failed for %s/%d", __func__, prefix_buf,
			   p->prefixlen);
		return;
	}
	...
	RNODE_FOREACH_RIB(rn, rib) {
		zlog_debug
		    ("%s: rn %p, rib %p: %s, %s",
		     ...
		     rn,
		     rib,
		     (CHECK_FLAG(rib->status, RIB_ENTRY_REMOVED) ? "removed" :
		      "NOT removed"), (CHECK_FLAG(rib->flags,
						  ZEBRA_FLAG_SELECTED) ?
	...
	RNODE_FOREACH_RIB(rn, rib) {
		if (CHECK_FLAG(rib->flags, ZEBRA_FLAG_SELECTED) &&
			...
			if (IS_ZEBRA_DEBUG_RIB) {
				...
				inet_ntop(rn->p.family, &p->prefix, buf,
					  INET_ADDRSTRLEN);
				zlog_debug
				    ("%s: freeing way for connected prefix %s/%d",
		...
		safi_t safi)
{
	...
	if (IS_ZEBRA_DEBUG_KERNEL) {
		if (gate)
			zlog_debug
			    ("rib_delete_ipv4(): route delete %s/%d via %s ifindex %d",
			     inet_ntop(AF_INET, &p->prefix, buf1,
				       INET_ADDRSTRLEN), p->prefixlen,
			     inet_ntoa(*gate), ifindex);
		else
			zlog_debug
			    ("rib_delete_ipv4(): route delete %s/%d ifindex %d",
			     inet_ntop(AF_INET, &p->prefix, buf1,
	...
	if (!rn) {
		if (IS_ZEBRA_DEBUG_KERNEL) {
			if (gate)
				zlog_debug
				    ("route %s/%d via %s ifindex %d doesn't exist in rib",
				     inet_ntop(AF_INET, &p->prefix, buf1,
					       INET_ADDRSTRLEN), p->prefixlen,
				     inet_ntop(AF_INET, gate, buf2,
					       INET_ADDRSTRLEN), ifindex);
			else
				zlog_debug
				    ("route %s/%d ifindex %d doesn't exist in rib",
				     inet_ntop(AF_INET, &p->prefix, buf1,
					       INET_ADDRSTRLEN), p->prefixlen,
				     ifindex);
		}
	...
	if (!same) {
		if (fib && type == ZEBRA_ROUTE_KERNEL) {
			...
			if (IS_ZEBRA_DEBUG_KERNEL) {
				if (gate)
					zlog_debug
					    ("route %s/%d via %s ifindex %d type %d doesn't exist in rib",
					     inet_ntop(AF_INET, &p->prefix,
						       buf1, INET_ADDRSTRLEN),
					     p->prefixlen, inet_ntop(AF_INET,
								     gate, buf2,
								     INET_ADDRSTRLEN),
					     ifindex, type);
				else
					zlog_debug
					    ("route %s/%d ifindex %d type %d doesn't exist in rib",
					     inet_ntop(AF_INET, &p->prefix,
						       buf1, INET_ADDRSTRLEN),
		     ...
		     u_int32_t vrf_id)
{
	...
	/* Distance changed.  */
	if (update)
		static_delete_ipv4_safi(safi, p, gate, ifname, update->distance,
					vrf_id);
	...
	for (pp = NULL, cp = rn->info; cp; pp = cp, cp = cp->next) {
		...
		if (si->type == STATIC_IPV4_GATEWAY
			...
			if (ntohl(si->gate.ipv4.s_addr) <
			    ntohl(cp->gate.ipv4.s_addr))
				break;
			if (ntohl(si->gate.ipv4.s_addr) >
			    ntohl(cp->gate.ipv4.s_addr))
				continue;
				...
				zlog_debug
				    ("route %s/%d via %s ifindex %d doesn't exist in rib",
				     inet_ntop(AF_INET6, &p->prefix, buf1,
					       INET6_ADDRSTRLEN), p->prefixlen,
				     inet_ntop(AF_INET6, gate, buf2,
					       INET6_ADDRSTRLEN), ifindex);
			else
				zlog_debug
				    ("route %s/%d ifindex %d doesn't exist in rib",
				     inet_ntop(AF_INET6, &p->prefix, buf1,
				...
				if (gate)
					zlog_debug
					    ("route %s/%d via %s ifindex %d type %d doesn't exist in rib",
					     ...
					     p->prefixlen, inet_ntop(AF_INET6,
								     gate, buf2,
								     INET6_ADDRSTRLEN),
					     ifindex, type);
				else
 ...
 */
static inline int vrf_id_get_next(uint32_t id, uint32_t * next_id_p)
{
	...
	case RIB_TABLES_ITER_S_ITERATING:
		...
		while (1) {
			...
			while (iter->afi_safi_ix < (int)ZEBRA_NUM_OF(afi_safis)) {
				table =
				    vrf_table(afi_safis[iter->afi_safi_ix].afi,
