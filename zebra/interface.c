	...
	if (!CHECK_FLAG(ifp->status, ZEBRA_INTERFACE_ACTIVE)) {
		...
		if (if_data && if_data->shutdown == IF_ZEBRA_SHUTDOWN_ON) {
			if (IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug
				    ("interface %s index %d is shutdown. Won't wake it up.",
		...
		if (IS_ZEBRA_DEBUG_KERNEL)
			zlog_debug("interface %s index %d is added.", ifp->name,
				   ifp->ifindex);
	}
}
...
void if_delete_update(struct interface *ifp)
{
	...
	if (ifp->connected) {
		...
		while ((node = (last ? last->next : listhead(ifp->connected)))) {
			...
			if (p->family == AF_INET
			    && (rn =
				route_node_lookup(zebra_if->ipv4_subnets, p))) {
	...
	if (rtadv->AdvSendAdvertisements) {
		...
		if (interval % 1000)
			vty_out(vty,
				...
				"%d milliseconds%s", interval, VTY_NEWLINE);
		else
			vty_out(vty,
				"  ND router advertisements are sent every "
				"%d seconds%s", interval / 1000, VTY_NEWLINE);
		...
		if (rtadv->AdvHomeAgentFlag) {
			vty_out(vty, "  ND router advertisements with "
				"Home Agent flag bit set.%s", VTY_NEWLINE);
...
static void if_dump_vty(struct vty *vty, struct interface *ifp)
{
	...
	if (sdl != NULL && sdl->sdl_alen != 0) {
		...
		vty_out(vty, "  HWaddr: ");
		for (i = 0, ptr = (u_char *) LLADDR(sdl); i < sdl->sdl_alen;
		     i++, ptr++)
	...
	for (rn = route_top(zebra_if->ipv4_subnets); rn; rn = route_next(rn)) {
		...
		for (ALL_LIST_ELEMENTS_RO
		     ((struct list *)rn->info, node, connected))
...
DEFUN(multicast,
      multicast_cmd, "multicast", "Set multicast flag to interface\n")
{
	int ret;
	struct interface *ifp;
	struct zebra_if *if_data;
...
DEFUN(no_multicast,
      no_multicast_cmd,
      "no multicast", NO_STR "Unset multicast flag to interface\n")
{
	int ret;
	struct interface *ifp;
	struct zebra_if *if_data;
...
DEFUN(linkdetect,
      linkdetect_cmd, "link-detect", "Enable link detection on interface\n")
{
	struct interface *ifp;
...
DEFUN(no_linkdetect,
      no_linkdetect_cmd,
      "no link-detect", NO_STR "Disable link detection on interface\n")
{
	struct interface *ifp;
	int if_was_operative;
...
DEFUN(shutdown_if,
      shutdown_if_cmd, "shutdown", "Shutdown the selected interface\n")
{
	int ret;
	struct interface *ifp;
	struct zebra_if *if_data;
...
DEFUN(no_shutdown_if,
      no_shutdown_if_cmd,
      "no shutdown", NO_STR "Shutdown the selected interface\n")
{
	int ret;
	struct interface *ifp;
	struct zebra_if *if_data;
...
DEFUN(no_bandwidth_if,
      no_bandwidth_if_cmd,
      "no bandwidth", NO_STR "Set bandwidth informational parameter\n")
{
	struct interface *ifp;
	...
	if (!ifc) {
		...
		*p = cp;
		...
		if (p->prefixlen <= IPV4_MAX_PREFIXLEN - 2) {
			...
			*p = cp;
			p->prefix.s_addr =
			    ipv4_broadcast_addr(p->prefix.s_addr, p->prefixlen);
