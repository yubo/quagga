...
{
	if (argc == 0)
		return zebra_route_match_delete(vty, vty->index, "interface",
						NULL);
...
{
	if (argc == 0)
		return zebra_route_match_delete(vty, vty->index, "ip next-hop",
						NULL);
...
{
	if (argc == 0)
		return zebra_route_match_delete(vty, vty->index, "ip address",
						NULL);
...
/* set functions */
    DEFUN(set_src,
      set_src_cmd,
      "set src A.B.C.D", SET_STR "src address for route\n" "src address\n")
{
	struct in_addr src;
...
DEFUN(no_set_src,
      no_set_src_cmd, "no set src", NO_STR SET_STR "Source address for route\n")
{
	if (argc == 0)
