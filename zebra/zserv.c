...
static int zebra_server_send_message(struct zserv *client)
{
	if (client->t_suicide)
		return -1;
	switch (buffer_write
		(client->wb, client->sock, STREAM_DATA(client->obuf),
	...
	case BUFFER_ERROR:
		...
		/* Schedule a delayed close since many of the functions that call this
		   ...
		   deleted. */
		client->t_suicide =
		    thread_add_event(zebrad.master, zserv_delayed_close, client,
				     0);
		return -1;
	case BUFFER_EMPTY:
...
static void zserv_create_header(struct stream *s, uint16_t cmd)
{
	...
	for (nexthop = rib->nexthop; nexthop; nexthop = nexthop->next) {
		if (CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_FIB)
			...
			case NEXTHOP_TYPE_IPV6_IFNAME:
				stream_write(s, (u_char *) & nexthop->gate.ipv6,
					     16);
				break;
#endif
			default:
...
static int zsend_ipv4_nexthop_lookup(struct zserv *client, struct in_addr addr)
{
	struct stream *s;
	struct rib *rib;
	unsigned long nump;
	...
	if (rib) {
		...
		if (IS_ZEBRA_DEBUG_PACKET && IS_ZEBRA_DEBUG_RECV)
			zlog_debug("%s: No matching rib entry found.",
				   __func__);
		stream_putl(s, 0);
		stream_putc(s, 0);
	}
		...
		for (ALL_LIST_ELEMENTS(ifp->connected, cnode, cnnode, c)) {
			if (CHECK_FLAG(c->conf, ZEBRA_IFC_REAL) &&
			    (zsend_interface_address
			     (ZEBRA_INTERFACE_ADDRESS_ADD, client, ifp, c) < 0))
				return -1;
		}
	}
	return 0;
	...
	if (CHECK_FLAG(message, ZAPI_MESSAGE_NEXTHOP)) {
		...
		for (i = 0; i < nexthop_num; i++) {
			...
			case ZEBRA_NEXTHOP_IPV4_IFINDEX:
				nexthop.s_addr = stream_get_ipv4(s);
				ifindex = stream_getl(s);
				nexthop_ipv4_ifindex_add(rib, &nexthop, NULL,
							 ifindex);
				break;
			case ZEBRA_NEXTHOP_IPV6:
...
static int zread_ipv4_nexthop_lookup(struct zserv *client, u_short length)
{
	struct in_addr addr;
	char buf[BUFSIZ];
...
/* Nexthop lookup for IPv4. */
static int zread_ipv4_import_lookup(struct zserv *client, u_short length)
{
	struct prefix_ipv4 p;
...
static int zread_ipv6_nexthop_lookup(struct zserv *client, u_short length)
{
...
static void zebra_event(enum event event, int sock, struct zserv *client)
{
	...
	case ZEBRA_READ:
		client->t_read =
		    thread_add_read(zebrad.master, zebra_client_read, client,
				    sock);
		break;
	case ZEBRA_WRITE:
		 /**/ break;
...
DEFUN(show_table,
      show_table_cmd,
      "show table", SHOW_STR "default routing table to use for all clients\n")
{
...
DEFUN(ip_forwarding,
      ip_forwarding_cmd, "ip forwarding", IP_STR "Turn on IP forwarding")
{
	int ret;
...
DEFUN(no_ip_forwarding,
      ...
      "no ip forwarding", NO_STR IP_STR "Turn off IP forwarding")
{
	int ret;
...
DEFUN(show_zebra_client,
      ...
      "show zebra client", SHOW_STR "Zebra information" "Client information")
{
...
DEFUN(show_ip_forwarding,
      ...
      "show ip forwarding", SHOW_STR IP_STR "IP forwarding status\n")
{
	int ret;
...
DEFUN(ipv6_forwarding,
      ipv6_forwarding_cmd,
      "ipv6 forwarding", IPV6_STR "Turn on IPv6 forwarding")
{
	int ret;
...
DEFUN(no_ipv6_forwarding,
      ...
      "no ipv6 forwarding", NO_STR IPV6_STR "Turn off IPv6 forwarding")
{
	int ret;
