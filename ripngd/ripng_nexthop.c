	       ...
	       struct sockaddr_in6 *to)
{
	...
	for (ALL_LIST_ELEMENTS(ripng_rte_list, node, nnode, data)) {
		...
		if (!IPV6_ADDR_SAME(&last_nexthop, NEXTHOP_OUT_PTR(data))) {
			...
			if (num == (rtemax - 1)) {
				ret =
				    ripng_send_packet((caddr_t) STREAM_DATA(s),
						      stream_get_endp(s), to,
						      ifp);

				if (ret >= 0 && IS_RIPNG_DEBUG_SEND)
					ripng_packet_dump((struct ripng_packet
							   *)STREAM_DATA(s),
							  stream_get_endp(s),
							  "SEND");
				num = 0;
			...
			num =
			    ripng_write_rte(num, s, NULL, &last_nexthop, 0,
			...
			if ((num == 0)
				...
				num =
				    ripng_write_rte(num, s, NULL, &last_nexthop,
