...
{
	if (argc == 0)
		return ripng_route_match_delete(vty, vty->index, "metric",
						NULL);
...
{
	if (argc == 0)
		return ripng_route_match_delete(vty, vty->index, "interface",
						NULL);
      ...
      "IPv6 local address\n" "IPv6 address of next hop\n")
{
	...
	if (ret < 0) {
		vty_out(vty, "%% Malformed next-hop local address%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}

	return ripng_route_set_add(vty, vty->index, "ipv6 next-hop local",
				   argv[0]);
}
...
{
	if (argc == 0)
		return ripng_route_set_delete(vty, vty->index,
					      "ipv6 next-hop local", NULL);

	return ripng_route_set_delete(vty, vty->index, "ipv6 next-hop local",
				      argv[0]);
}
