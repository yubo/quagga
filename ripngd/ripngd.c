...
static int ripng_make_socket(void)
{
	...
	if (ret < 0) {
		zlog(NULL, LOG_ERR, "Can't bind ripng socket: %s.",
		     safe_strerror(errno));
...
ripng_nexthop_rte(struct rte *rte,
		  struct sockaddr_in6 *from, struct ripng_nexthop *nexthop)
{
	...
	if (IS_RIPNG_DEBUG_RECV)
		zlog_debug("RIPng nexthop RTE address %s tag %d prefixlen %d",
			   inet6_ntoa(rte->addr), ntohs(rte->tag),
			   rte->prefixlen);
	...
	if (ntohs(rte->tag) != 0)
		zlog_warn
		    ("RIPng nexthop RTE with non zero tag value %d from %s",
...
ripng_route_process(struct rte *rte, struct sockaddr_in6 *from,
		    struct ripng_nexthop *ripng_nexthop, struct interface *ifp)
{
	int ret;
	...
	if (rp->info == NULL) {
		...
		if (rte->metric != RIPNG_METRIC_INFINITY) {
			...
			ripng_zebra_ipv6_add(&p, &rinfo->nexthop,
					     rinfo->ifindex, rinfo->metric);
			rinfo->flags |= RIPNG_RTF_FIB;
		...
		if ((same && rinfo->metric != rte->metric) ||
			...
			if (oldmetric != RIPNG_METRIC_INFINITY) {
				ripng_zebra_ipv6_delete(&p, &rinfo->nexthop,
							rinfo->ifindex);
				ripng_zebra_ipv6_add(&p, nexthop, ifp->ifindex,
						     rinfo->metric);
				rinfo->flags |= RIPNG_RTF_FIB;
		...
		if (!nexthop)
			...
			zlog_debug
			    ("Redistribute new prefix %s/%d with nexthop %s on the interface %s",
			     inet6_ntoa(p->prefix), p->prefixlen,
			     inet6_ntoa(*nexthop), ifindex2ifname(ifindex));
	}
		...
		struct in6_addr *nexthop, u_int16_t tag, u_char metric)
{
	...
	if (!nexthop)
		stream_write(s, (u_char *) & p->prefix,
	...
	for (rp = route_top(ripng->table); rp; rp = route_next(rp)) {
		if ((rinfo = rp->info) != NULL && rinfo->suppress == 0) {
			...
			if (ripng->route_map[rinfo->type].name) {
				...
				ret =
				    route_map_apply(ripng->route_map
						    [rinfo->type].map,
						    (struct prefix *)p,
						    RMAP_RIPNG, rinfo);
		...
		if ((aggregate = rp->aggregate) != NULL &&
		    aggregate->count > 0 && aggregate->suppress == 0) {
			/* If no route-map are applied, the RTE will be these following
			 * informations.
			 */
			p = (struct prefix_ipv6 *)&rp->p;
	...
	case RIPNG_TRIGGERED_UPDATE:
		if (ripng->t_triggered_interval)
			...
			ripng->t_triggered_update =
			    thread_add_event(master, ripng_triggered_update,
					     NULL, 0);
		break;
	default:
		break;
...
DEFUN(show_ipv6_ripng,
      show_ipv6_ripng_cmd,
      "show ipv6 ripng", SHOW_STR IPV6_STR "Show RIPng routes\n")
{
	struct route_node *rp;
	struct ripng_info *rinfo;
	struct ripng_aggregate *aggregate;
...
/* RIPng update timer setup. */
    DEFUN(ripng_update_timer,
      ...
      "update-timer SECOND", "Set RIPng update timer in seconds\n" "Seconds\n")
{
...
DEFUN(no_ripng_update_timer,
      ...
      NO_STR "Unset RIPng update timer in seconds\n" "Seconds\n")
{
	ripng->update_time = RIPNG_UPDATE_TIMER_DEFAULT;
	ripng_event(RIPNG_UPDATE_EVENT, 0);
	return CMD_SUCCESS;
...
DEFUN(no_ripng_timeout_timer,
      ...
      NO_STR "Unset RIPng timeout timer in seconds\n" "Seconds\n")
{
	ripng->timeout_time = RIPNG_TIMEOUT_TIMER_DEFAULT;
	return CMD_SUCCESS;
}
...
DEFUN(no_ripng_garbage_timer,
      ...
      NO_STR "Unset RIPng garbage timer in seconds\n" "Seconds\n")
{
	ripng->garbage_time = RIPNG_GARBAGE_TIMER_DEFAULT;
	return CMD_SUCCESS;
}
...
DEFUN(no_ripng_timers,
      no_ripng_timers_cmd,
      "no timers basic", NO_STR "RIPng timers setup\n" "Basic timer\n")
{
...
ALIAS(no_ripng_timers,
    ...
    DEFUN(show_ipv6_protocols, show_ipv6_protocols_cmd,
      "show ipv6 protocols", SHOW_STR IPV6_STR "Routing protocol information")
{
	if (!ripng)
		return CMD_SUCCESS;
	...
	vty_out(vty,
		...
		ripng->update_time, 0, VTY_NEWLINE);

	vty_out(vty, "Timerout after %ld seconds, garbage correct %ld%s",
		ripng->timeout_time, ripng->garbage_time, VTY_NEWLINE);
...
static void ripng_distribute_update(struct distribute *dist)
{
	...
	if (dist->prefix[DISTRIBUTE_IN]) {
		plist =
		    prefix_list_lookup(AFI_IP6, dist->prefix[DISTRIBUTE_IN]);
		if (plist)
	...
	if (dist->prefix[DISTRIBUTE_OUT]) {
		plist =
		    prefix_list_lookup(AFI_IP6, dist->prefix[DISTRIBUTE_OUT]);
		if (plist)
	...
	if (ripng) {
		...
		for (rp = route_top(ripng->table); rp; rp = route_next(rp)) {
			if ((rinfo = rp->info) != NULL) {
				if ((rinfo->type == ZEBRA_ROUTE_RIPNG) &&
				    (rinfo->sub_type == RIPNG_ROUTE_RTE))
					ripng_zebra_ipv6_delete((struct
								 prefix_ipv6 *)
								&rp->p,
								&rinfo->nexthop,
								rinfo->metric);
		...
		for (i = 0; i < ZEBRA_ROUTE_MAX; i++) {
			if (ripng->route_map[i].name)
				ripng->route_map[i].map =
				    route_map_lookup_by_name(ripng->route_map
							     [i].name);
		}
	}
}
