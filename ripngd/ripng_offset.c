...
static struct ripng_offset_list *ripng_offset_list_new()
{
	...
	new =
	    XCALLOC(MTYPE_RIPNG_OFFSET_LIST, sizeof(struct ripng_offset_list));
	return new;
}
			   ...
			   u_char * metric)
{
	...
	if (offset && OFFSET_LIST_IN_NAME(offset)) {
		alist =
		    access_list_lookup(AFI_IP6, OFFSET_LIST_IN_NAME(offset));

		if (alist
		    && access_list_apply(alist,
					 (struct prefix *)p) == FILTER_PERMIT) {
			    ...
			    u_char * metric)
{
	...
	if (offset && OFFSET_LIST_OUT_NAME(offset)) {
		alist =
		    access_list_lookup(AFI_IP6, OFFSET_LIST_OUT_NAME(offset));
		...
		if (!offset->ifname) {
			if (offset->direct[RIPNG_OFFSET_LIST_IN].alist_name)
				vty_out(vty, " offset-list %s in %d%s",
					offset->
					direct[RIPNG_OFFSET_LIST_IN].alist_name,
					offset->
					direct[RIPNG_OFFSET_LIST_IN].metric,
					VTY_NEWLINE);
			if (offset->direct[RIPNG_OFFSET_LIST_OUT].alist_name)
				vty_out(vty, " offset-list %s out %d%s",
					offset->
					direct
					[RIPNG_OFFSET_LIST_OUT].alist_name,
					offset->
					direct[RIPNG_OFFSET_LIST_OUT].metric,
					VTY_NEWLINE);
		} else {
			if (offset->direct[RIPNG_OFFSET_LIST_IN].alist_name)
