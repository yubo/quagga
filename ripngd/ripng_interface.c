		...
		for (rp = route_top(ripng->table); rp; rp = route_next(rp))
			if ((rinfo = rp->info) != NULL) {
				...
				if (rinfo->ifindex == ifp->ifindex
					...
					ripng_redistribute_delete(rinfo->type,
								  rinfo->sub_type,
								  (struct
								   prefix_ipv6
								   *)&rp->p,
...
int
ripng_interface_down(int command, struct zclient *zclient, zebra_size_t length)
{
	struct stream *s;
	struct interface *ifp;
...
static void ripng_connect_set(struct interface *ifp, int set)
{
	...
	for (ALL_LIST_ELEMENTS(ifp->connected, node, nnode, connected)) {
		...
		if (set) {
			...
			ripng_redistribute_delete(ZEBRA_ROUTE_CONNECT,
						  RIPNG_ROUTE_INTERFACE,
						  &address,
						  connected->ifp->ifindex);
