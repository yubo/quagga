	...
	if (size <= 0 || (size_t) size > array_size(array)) {
		zlog_err
		    ("Cannot get backtrace, returned invalid # of frames %d "
		     "(valid range is between 1 and %lu)", size,
		       ...
		       const char *none, const char *mesname)
{
	...
	zlog_err("message index %d not found in %s (max is %d)", index, mesname,
		 max);
	assert(none);
	return none;
}
