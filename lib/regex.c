...
unsigned char *end;
{
	...
	while (p < pend) {
		...
		case set_number_at:
			...
			p1 = p + mcnt;
			extract_number_and_incr(&mcnt2, &p);
			printf("/set_number_at location %d to %d", p1 - start,
			       mcnt2);
			break;
...
succeed_label:
			...
			if (regs && !bufp->no_sub) {
				...
				/* If the regs structure we return has more elements than
				   ...
				   -1 at the end.  */
				for (mcnt = num_regs;
				     (unsigned)mcnt < regs->num_regs; mcnt++)
...
size_t errbuf_size;
{
	...
	if (err < 0 || err >= (int)(sizeof(re_error_msgid_idx)
