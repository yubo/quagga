...
static int quagga_get_relative(struct timeval *tv)
{
...
#elif defined(__APPLE__)
	{
		...
		useconds =
		    ticks * timebase_info.numer / timebase_info.denom / 1000;
...
static void cpu_record_print(struct vty *vty, thread_type filter)
{
	...
	hash_iterate(cpu_record, (void (*)(struct hash_backet *, void *))
		     cpu_record_hash_print, args);
...
static struct thread *funcname_thread_add_timer_timeval(struct thread_master *m,
							int (*func) (struct
								     thread *),
							int type, void *arg,
							struct timeval
					 ...
					 struct timeval *timer_val)
{
	if (queue->size) {
		...
		*timer_val =
		    timeval_subtract(next_timer->u.sands, relative_time);
		return timer_val;
	}
	return NULL;
}
...
void thread_call(struct thread *thread)
{
	...
	if (!thread->hist) {
		...
		thread->hist = hash_get(cpu_record, &tmp, (void *(*)(void *))
