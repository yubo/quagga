...
int zclient_send_message(struct zclient *zclient)
{
	if (zclient->sock < 0)
		return -1;
	switch (buffer_write
		(zclient->wb, zclient->sock, STREAM_DATA(zclient->obuf),
...
void zclient_create_header(struct stream *s, uint16_t command)
{
 ...
 * sending client
 */
int zebra_redistribute_send(int command, struct zclient *zclient, int type)
{
	struct stream *s;
...
struct connected *zebra_interface_address_read(int type, struct stream *s)
{
	...
	if (ifp == NULL) {
		zlog_warn("zebra_interface_address_read(%s): "
			  ...
			  (type ==
			   ZEBRA_INTERFACE_ADDRESS_ADD ? "ADD" : "DELETE"),
			  ifindex);
		return NULL;
	}
	...
	if (type == ZEBRA_INTERFACE_ADDRESS_ADD) {
		...
		ifc =
		    connected_add_by_prefix(ifp, &p,
					    (memconstant(&d.u.prefix, 0, plen) ?
					     NULL : &d));
		if (ifc != NULL) {
			ifc->flags = ifc_flags;
			if (ifc->destination)
...
static int zclient_read(struct thread *thread)
{
	...
	case ZEBRA_IPV4_ROUTE_DELETE:
		if (zclient->ipv4_route_delete)
			(*zclient->ipv4_route_delete) (command, zclient,
						       length);
		break;
	...
	case ZEBRA_IPV6_ROUTE_DELETE:
		if (zclient->ipv6_route_delete)
			(*zclient->ipv6_route_delete) (command, zclient,
						       length);
		break;
	default:
		break;
...
void zclient_redistribute(int command, struct zclient *zclient, int type)
{
...
static void zclient_event(enum event event, struct zclient *zclient)
{
	...
	case ZCLIENT_SCHEDULE:
		if (!zclient->t_connect)
			zclient->t_connect =
			    thread_add_event(master, zclient_connect, zclient,
					     0);
		break;
	...
	case ZCLIENT_READ:
		zclient->t_read =
		    thread_add_read(master, zclient_read, zclient,
				    zclient->sock);
		break;
	}
}
