...
static void if_dump(const struct interface *ifp)
{
	...
	for (ALL_LIST_ELEMENTS_RO(ifp->connected, node, c))
		zlog_info("Interface %s index %d metric %d mtu %d "
			  ...
			  "mtu6 %d "
#endif /* HAVE_IPV6 */
			  "%s", ifp->name, ifp->ifindex, ifp->metric, ifp->mtu,
#ifdef HAVE_IPV6
			  ifp->mtu6,
#endif /* HAVE_IPV6 */
	   ...
	   "Delete a pseudo interface's configuration\n" "Interface's name\n")
{
	...
	if (ifp == NULL) {
		vty_out(vty, "%% Interface %s does not exist%s", argv[0],
			VTY_NEWLINE);
		return CMD_WARNING;
	}
...
/* Find the IPv4 address on our side that will be used when packets
   are sent to dst. */
struct connected *connected_lookup_address(struct interface *ifp,
					   struct in_addr dst)
{
	struct prefix addr;
	struct listnode *cnode;
...
unsigned int if_nametoindex(const char *name)
{
	...
	return ((ifp =
		 if_lookup_by_name_len(name, strnlen(name, IFNAMSIZ))) != NULL)
	    ? ifp->ifindex : 0;
}
#endif
...
static void ifaddr_ipv4_add(struct in_addr *ifaddr, struct interface *ifp)
{
...
static void ifaddr_ipv4_delete(struct in_addr *ifaddr, struct interface *ifp)
{
	struct route_node *rn;
	struct prefix_ipv4 p;
...
static struct interface *ifaddr_ipv4_lookup(struct in_addr *addr,
