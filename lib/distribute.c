...
static struct distribute *distribute_list_set(const char *ifname,
...
/* Set access-list name to the distribute list. */
static struct distribute *distribute_list_prefix_set(const char *ifname,
						     enum distribute_type type,
...
int config_show_distribute(struct vty *vty)
{
	...
	if (dist
	    && (dist->list[DISTRIBUTE_OUT] || dist->prefix[DISTRIBUTE_OUT])) {
		vty_out(vty,
		...
		vty_out(vty,
			"  Outgoing update filter list for all interface is not set%s",
			VTY_NEWLINE);
	...
	if (dist && (dist->list[DISTRIBUTE_IN] || dist->prefix[DISTRIBUTE_IN])) {
		...
		vty_out(vty,
			"  Incoming update filter list for all interface is not set%s",
			VTY_NEWLINE);
