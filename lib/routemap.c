...
static void
route_map_rule_delete(struct route_map_rule_list *, struct route_map_rule *);
...
static struct route_map_index *route_map_index_add(struct route_map *map,
						   enum route_map_type type,
						   int pref)
{
	struct route_map_index *index;
...
/* Get route map index. */
static struct route_map_index *route_map_index_get(struct route_map *map,
						   enum route_map_type type,
						   int pref)
{
	struct route_map_index *index;
		     ...
		     const char *set_arg)
{
	...
	for (rule = index->set_list.head; rule; rule = rule->next)
		    ...
		    (rulecmp(rule->rule_str, set_arg) == 0 || set_arg == NULL))
		{
			...
			if (route_map_master.event_hook)
				(*route_map_master.event_hook)
				    (RMAP_EVENT_SET_DELETED, index->map->name);
			return 0;
		}
	/* Can't find matched rule. */
	return 1;
		...
		route_map_object_t type, void *object)
{
	...
	for (index = map->head; index; index = index->next) {
		/* Apply this index. */
		ret =
		    route_map_apply_match(&index->match_list, prefix, type,
					  object);
      ...
      "Sequence to insert to/delete from existing route-map entry\n")
{
	...
	if (strncmp(argv[1], "permit", strlen(argv[1])) == 0)
		...
		vty_out(vty, "the third field must be [permit|deny]%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	if (pref == 0 || pref > 65535) {
		vty_out(vty, "the fourth field must be <1-65535>%s",
			VTY_NEWLINE);
		return CMD_WARNING;
	}
      ...
      "Exit policy on matches\n" "Goto Clause number\n" "Number\n")
{
	...
	if (index) {
		if (argc == 1 && argv[0])
			VTY_GET_INTEGER_RANGE("route-map index", d, argv[0], 1,
					      65536);
		else
			d = index->pref + 1;
...
DEFUN(no_rmap_description,
      no_rmap_description_cmd, "no description", NO_STR "Route-map comment\n")
{
	struct route_map_index *index;
