	    ...
	    const char *format, struct timestamp_control *ctl, va_list va)
{
	...
	if (!ctl->already_rendered) {
		ctl->len =
		    quagga_timestamp(ctl->precision, ctl->buf,
				     sizeof(ctl->buf));
	...
	if (level)
		ret =
		    snprintf(buf + len, sizeof(buf) - len, "%s: %s: ", level,
			     proto_str);
	else
		...
		if (vty->fail >= 3) {
			if (vty->node == AUTH_NODE) {
				...
				vty_out(vty,
					...
					VTY_NEWLINE);
				vty->node =
				    restricted_mode ? RESTRICTED_NODE :
				    VIEW_NODE;
			}
		}
	}
...
static int vty_command(struct vty *vty, char *buf)
{
...
#ifdef CONSUMED_TIME_CHECK
	{
		...
		GETRUSAGE(&after);
		if ((realtime =
		     thread_consumed_time(&after, &before,
					  &cputime)) > CONSUMED_TIME_CHECK)
...
static void vty_describe_command(struct vty *vty)
{
	...
	if ((token = token_cr)) {
		if (!token->desc)
			vty_out(vty, "  %-s%s",
				token->cmd[0] ==
				'.' ? token->cmd + 1 : token->cmd, VTY_NEWLINE);
		else if (desc_width >= strlen(token->desc))
			vty_out(vty, "  %-*s  %s%s", width,
				token->cmd[0] ==
				'.' ? token->cmd + 1 : token->cmd, token->desc,
				VTY_NEWLINE);
		else
	...
	case SE:
		{
			...
			case TELOPT_NAWS:
				if (vty->sb_len != TELNET_NAWS_SB_LEN)
					...
					vty->width =
					    ((vty->
					      sb_buf[1] << 8) | vty->sb_buf[2]);
					vty->height =
					    ((vty->
					      sb_buf[3] << 8) | vty->sb_buf[4]);
#ifdef TELNET_OPTION_DEBUG
					vty_out(vty,
...
vty_serv_sock_family(const char *addr, unsigned short port, int family)
{
	...
	if (naddr)
		...
		case -1:
			...
			break;
		case 0:
			zlog_err("error translating address %s: %s", addr,
				 safe_strerror(errno));
			naddr = NULL;
		}
	...
	case VTY_READ:
		...
		if (vty->v_timeout) {
			if (vty->t_timeout)
				thread_cancel(vty->t_timeout);
			vty->t_timeout =
			    thread_add_timer(master, vty_timeout, vty,
					     vty->v_timeout);
		}
		break;
	case VTY_WRITE:
...
DEFUN(vty_no_restricted_mode,
      ...
      "no anonymous restricted", NO_STR "Enable password checking\n")
{
	restricted_mode = 0;
	return CMD_SUCCESS;
}
