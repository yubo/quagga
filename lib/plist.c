...
static struct prefix_master *prefix_master_get(afi_t afi)
{
	if (afi == AFI_IP)
...
static struct prefix_list_entry *prefix_list_entry_new(void)
{
	...
	new =
	    XCALLOC(MTYPE_PREFIX_LIST_ENTRY, sizeof(struct prefix_list_entry));
	return new;
}
...
static struct prefix_list *prefix_list_insert(afi_t afi, const char *name)
{
	unsigned int i;
	long number;
	struct prefix_list *plist;
...
static struct prefix_list *prefix_list_get(afi_t afi, const char *name)
{
	struct prefix_list *plist;
...
static struct prefix_list_entry *prefix_list_entry_make(struct prefix *prefix,
...
static struct prefix_list_entry *prefix_list_entry_lookup(struct prefix_list
							  *plist,
							  struct prefix *prefix,
							  enum prefix_list_type
							  type, int seq, int le,
...
static struct prefix_list_entry *prefix_entry_dup_check(struct prefix_list
							*plist, struct prefix_list_entry
							*new)
{
	struct prefix_list_entry *pentry;
	int seq = 0;
...
DEFUN(show_ip_prefix_list_name_seq,
      ...
      "sequence number of an entry\n" "Sequence number\n")
{
	return vty_show_prefix_list(vty, AFI_IP, argv[0], argv[1],
				    sequential_display);
}
...
{
	return vty_show_prefix_list(vty, AFI_IP, argv[0], NULL,
				    summary_display);
}
...
DEFUN(no_ipv6_prefix_list_seq,
      ...
      "Any prefix match.  Same as \"::0/0 le 128\"\n")
{
	return vty_prefix_list_uninstall(vty, AFI_IP6, argv[0], argv[1],
...
{
	return vty_show_prefix_list(vty, AFI_IP6, argv[0], NULL,
				    normal_display);
}
...
DEFUN(show_ipv6_prefix_list_name_seq,
      ...
      "sequence number of an entry\n" "Sequence number\n")
{
	return vty_show_prefix_list(vty, AFI_IP6, argv[0], argv[1],
				    sequential_display);
}
...
DEFUN(show_ipv6_prefix_list_prefix,
      ...
      "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n")
{
	return vty_show_prefix_list_prefix(vty, AFI_IP6, argv[0], argv[1],
					   normal_display);
}
...
static void prefix_list_reset_orf(void)
{
	struct prefix_list *plist;
...
static void prefix_list_reset_ipv4(void)
{
	struct prefix_list *plist;
	struct prefix_list *next;
	struct prefix_master *master;
...
static void prefix_list_reset_ipv6(void)
{
	struct prefix_list *plist;
	struct prefix_list *next;
	struct prefix_master *master;
