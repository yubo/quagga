...
static int smux_socket(void)
{
	...
	ret =
	    connect(sock, (struct sockaddr *)&serv, sizeof(struct sockaddr_in));
	if (ret < 0) {
		close(sock);
		smux_sock = -1;
		zlog_warn("Can't connect to SNMP agent with SMUX");
		  ...
		  long errindex, u_char val_type, void *arg, size_t arg_len)
{
	...
	ptr = asn_build_int(ptr, &len,
			    (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE |
				      ASN_INTEGER), &reqid, sizeof(reqid));
...
static u_char *smux_var(u_char * ptr, size_t len, oid objid[],
			size_t * objid_len, size_t * var_val_len,
			u_char * var_val_type, void **var_value)
{
	u_char type;
	u_char val_type;
	size_t val_len;
	...
	for (ALL_LIST_ELEMENTS(treelist, node, nnode, subtree)) {
		...
		if (subresult <= 0) {
			...
			for (j = 0; j < subtree->variables_num; j++) {
				...
				if (result == 0)
					result =
					    oid_compare_part(suffix, suffix_len,
							     v->name,
							     v->namelen);
...
static void smux_parse_get(u_char * ptr, size_t len, int exact)
{
	...
	if (exact)
		...
		ret =
		    smux_getnext(oid, &oid_len, exact, &val_type, &val,
				 &val_len);
...
static int smux_open(int sock)
{
	...
	ptr = asn_build_int(ptr, &len,
			    (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE |
				      ASN_INTEGER), &version, sizeof(version));

	/* SMUX connection oid. */
	ptr = asn_build_objid(ptr, &len, (u_char)
			      (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OBJECT_ID),
			      smux_oid, smux_oid_len);

	/* SMUX connection description. */
	ptr = asn_build_string(ptr, &len, (u_char)
...
smux_trap(struct variable *vp, size_t vp_len,
	  ...
	  const struct trap_object *trapobj, size_t trapobjlen, u_char sptrap)
{
	unsigned int i;
	u_char buf[BUFSIZ];
	u_char *ptr;
	...
	ptr = asn_build_int(ptr, &len,
			    (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE |
				      ASN_INTEGER), (long *)&val, sizeof(val));
...
static int smux_register(int sock)
{
	...
	for (ALL_LIST_ELEMENTS(treelist, node, nnode, subtree)) {
		...
		len = BUFSIZ;
		asn_build_header(buf, &len, (u_char) SMUX_RREQ,
				 (ptr - buf) - 2);
...
void smux_event(enum smux_event event, int sock)
{
	...
	case SMUX_SCHEDULE:
		smux_connect_thread =
		    thread_add_event(master, smux_connect, NULL, 0);
		break;
	case SMUX_CONNECT:
		smux_connect_thread =
		    thread_add_timer(master, smux_connect, NULL, 10);
		break;
	case SMUX_READ:
		smux_read_thread =
...
smux_register_mib(const char *descr, struct variable *var,
		  size_t width, int num, oid name[], size_t namelen)
{
	struct subtree *tree;
