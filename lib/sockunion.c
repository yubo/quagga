...
static int sockunion_sizeof(union sockunion *su)
{
	int ret;
...
static const char *sockunion_log(union sockunion *su, char *buf, size_t len)
{
	switch (su->sa.sa_family) {
	case AF_INET:
		return inet_ntop(AF_INET, &su->sin.sin_addr, buf, len);
...
int sockopt_minttl(int family, int sock, int minttl)
{
	...
	if (family == AF_INET6) {
		int ret =
		    setsockopt(sock, IPPROTO_IPV6, IPV6_MINHOPCNT, &minttl,
			       sizeof(minttl));
		if (ret < 0)
			zlog(NULL, LOG_WARNING,
...
#ifdef HAVE_IPV6
static int in6addr_cmp(struct in6_addr *addr1, struct in6_addr *addr2)
{
	unsigned int i;
	u_char *p1, *p2;
