...
cmd_word_match(struct cmd_token *token,
	       enum filter_type filter, const char *word)
{
	const char *str;
...
cmd_matcher_record_match(struct cmd_matcher *matcher,
			 enum match_type match_type, struct cmd_token *token)
{
			  ...
			  struct cmd_token *token, vector args_vector)
{
	...
	while (1) {
		...
		for (i = 0; i < vector_active(token->keyword); i++) {
			...
			word_match =
			    cmd_word_match(word_token, matcher->filter, word);
			if (word_match == no_match)
				continue;

			cmd_matcher_record_match(matcher, word_match,
						 word_token);
		...
		for (i = 1; i < vector_active(keyword_vector); i++) {
			...
			case TOKEN_TERMINAL:
				rv = cmd_matcher_match_terminal(matcher,
								word_token,
								&keyword_argc,
								keyword_argv);
				break;
			case TOKEN_MULTIPLE:
				rv = cmd_matcher_match_multiple(matcher,
								word_token,
								&keyword_argc,
								keyword_argv);
				break;
...
cmd_matcher_match_keyword(struct cmd_matcher *matcher,
			  struct cmd_token *token, int *argc, const char **argv)
{
	vector keyword_args_vector;
	...
	reader_rv =
	    cmd_matcher_read_keywords(matcher, token, keyword_args_vector);
	builder_rv =
	    cmd_matcher_build_keyword_args(matcher, token, argc, argv,
					   keyword_args_vector);
...
cmd_matcher_init(struct cmd_matcher *matcher,
		 ...
		 vector vline,
		 unsigned int index,
		 enum match_type *match_type, vector * match)
{
...
cmd_element_match(struct cmd_element *cmd_element,
		  ...
		  unsigned int index,
		  enum match_type *match_type,
		  vector * match, int *argc, const char **argv)
{
	...
	for (token_index = 0;
	     token_index < vector_active(cmd_element->tokens); token_index++) {
		struct cmd_token *token =
		    vector_slot(cmd_element->tokens, token_index);

		switch (token->type) {
		case TOKEN_TERMINAL:
			rv = cmd_matcher_match_terminal(&matcher, token, argc,
							argv);
			break;
		case TOKEN_MULTIPLE:
			rv = cmd_matcher_match_multiple(&matcher, token, argc,
							argv);
			break;
		case TOKEN_KEYWORD:
			rv = cmd_matcher_match_keyword(&matcher, token, argc,
						       argv);
		}
...
cmd_vector_filter(vector commands,
		  ...
		  vector vline,
		  unsigned int index,
		  enum match_type *match_type, vector * matches)
{
	unsigned int i;
...
cmd_parse(struct cmd_element *cmd_element,
	  vector vline, int *argc, const char **argv)
{
...
is_cmd_ambiguous(vector cmd_vector,
		 const char *command, vector matches, enum match_type type)
{
	unsigned int i;
	unsigned int j;
	const char *str = NULL;
	...
	for (i = 0; i < vector_active(matches); i++)
		if ((match_vector = vector_slot(matches, i)) != NULL) {
			...
			for (j = 0; j < vector_active(match_vector); j++)
				if ((cmd_token =
				     vector_slot(match_vector, j)) != NULL) {
					enum match_type ret;

					assert(cmd_token->type ==
					       TOKEN_TERMINAL);
...
static int cmd_try_do_shortcut(enum node_type node, char *first_word)
{
	if (first_word != NULL &&
	    ...
	    node != ENABLE_NODE &&
	    node != RESTRICTED_NODE && 0 == strcmp("do", first_word))
		return 1;
	return 0;
}
...
#define INIT_MATCHVEC_SIZE 10
	...
	for (i = 0; i <= index; i++) {
		...
		if (match == vararg_match) {
			...
			for (j = 0; j < vector_active(matches); j++)
				if ((match_vector =
				     vector_slot(matches, j)) != NULL)
					for (k = 0;
					     k < vector_active(match_vector);
					     k++) {
						struct cmd_token *token =
	...
	for (i = 0; i < vector_active(matches); i++)
		if ((cmd_element = vector_slot(cmd_vector, i)) != NULL) {
			...
			last_word =
			    vector_slot(vline, vector_active(vline) - 1);
			if (last_word == NULL || !strlen(last_word)) {
				vline_trimmed = vector_copy(vline);
				vector_unset(vline_trimmed,
					     vector_active(vline_trimmed) - 1);
			...
			if (match_vector)
				for (j = 0; j < vector_active(match_vector);
				     j++) {
					struct cmd_token *token =
					    vector_slot(match_vector, j);
					const char *string;
...
cmd_execute_command_real(vector vline,
			 enum filter_type filter,
			 struct vty *vty, struct cmd_element **cmd)
{
	unsigned int i;
	unsigned int index;
	vector cmd_vector;
	...
	for (index = 0; index < vector_active(vline); index++) {
		...
		ret = cmd_vector_filter(cmd_vector,
					filter, vline, index, &match, &matches);
		    ...
		    int vtysh)
{
	...
	if (cmd_try_do_shortcut(vty->node, vector_slot(vline, 0))) {
		...
		ret =
		    cmd_execute_command_real(shifted_vline, FILTER_RELAXED, vty,
					     cmd);
	...
	if (token->keyword) {
		for (i = 0; i < vector_active(token->keyword); i++) {
			...
			for (j = 0; j < vector_active(keyword_vect); j++)
				cmd_terminate_token(vector_slot
						    (keyword_vect, j));
