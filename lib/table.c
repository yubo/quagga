 ...
 */
struct route_table *route_table_init_with_delegate(route_table_delegate_t *
						   delegate)
{
...
static void route_common(struct prefix *n, struct prefix *p, struct prefix *new)
{
	int i;
	u_char diff;
	u_char mask;
 ...
 */
static struct route_node *route_node_create(route_table_delegate_t * delegate,
 ...
 */
static struct route_node *route_table_get_next_internal(const struct route_table
							*table,
							struct prefix *p)
{
	...
	while (node) {
		...
		if (match) {
			...
			tmp_node =
			    node->
			    link[prefix_bit(&p->u.prefix, node->p.prefixlen)];
